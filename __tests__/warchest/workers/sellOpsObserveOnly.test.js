...
const { evaluatePosition } = require('../../../lib/warchest/workers/sellOps/evaluationEngine');
const { createSellOpsController } = require('../../../lib/warchest/workers/sellOps/controller');
...
describe('sellOps observeOnly defaults', () => {
  ...
  test('defaults to execute mode in production env', async () => {
    ...
    const controller = createSellOpsController(
      ...
      { client: { close: jest.fn() }, dataClient: { close: jest.fn() }, db: {}, track: jest.fn(), env: { NODE_ENV: 'production' } }
    );
    ...
    expect(args.payload.observeOnly).toBe(false);
  ...
  test('honors explicit observeOnly override', async () => {
    ...
    const controller = createSellOpsController(
      { wallet: { alias: 'alpha', pubkey: 'pub' }, pollIntervalMs: 1000, observeOnly: true },
      { client: { close: jest.fn() }, dataClient: { close: jest.fn() }, db: {}, track: jest.fn(), env: { NODE_ENV: 'production' } }
    );
