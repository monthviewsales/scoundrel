...
// Env:
//   SOLANATRACKER_APIKEY (preferred) or SOLANATRACKER_APIKEY -> API key for the Data API
//   SOLANATRACKER_BASE_URL (optional) -> override SDK base URL

require('dotenv').config();

const API_KEY = process.env.SOLANATRACKER_APIKEY || process.env.SOLANATRACKER_APIKEY || null;
const BASE_URL = process.env.SOLANATRACKER_BASE_URL || null;
...
class SolanaTrackerClient {
  constructor({ baseUrl = BASE_URL || undefined, apiKey = API_KEY } = {}) {
    if (!apiKey) {
      throw new Error('[SolanaTrackerDataClient] Missing SOLANATRACKER_APIKEY');
    }
  ...
  /**
   * Fetch wallet portfolio chart data with historical values and PnL.
   * Mirrors SDK: client.getWalletChart('walletAddress')
   * @param {string} wallet
   * @returns {Promise<any>} chart payload (sdk-shaped)
   */
  async getWalletChart(wallet) {
    if (!wallet || typeof wallet !== 'string') {
      throw new Error('[SolanaTrackerDataClient.getWalletChart] wallet (string) is required');
    }
    const ctx = { op: 'getWalletChart', wallet };
    const run = () => this.client.getWalletChart(wallet);
    log('info', 'fetching wallet chart', ctx);
    const res = await withRetries(run, { ctx });
    return res?.chart || res?.chartData || res || [];
  }
  /**
...
module.exports = { SolanaTrackerDataClient: SolanaTrackerClient };
