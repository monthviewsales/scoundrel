 ...
 * readline code across CLI entrypoints and enables an Ink TUI picker.
 */
...
async function promptKeySource(rl) {
  while (true) {
    const raw = await rl.question('Key source (k=keychain, e=env, n=none) [k]: ');
...
async function promptEnvVar(rl) {
  while (true) {
    const raw = await rl.question('Env var name (e.g. WALLET_KEY_ALPHA): ');
...
async function promptSecret(rl) {
  while (true) {
    const raw = await rl.question('Paste private key (base58 or id.json array): ');
    const value = (raw || '').trim();
    if (value) return value;
...
async function maybeImportDefaultFundingWallet(rl) {
  ...
  let hasPrivateKey = await promptYesNo(
    rl,
  ...
  if (hasPrivateKey && keySource === 'keychain') {
    try {
