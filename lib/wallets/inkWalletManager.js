...
function createWalletManagerComponents(ink, TextInput) {
  ...
  function OptionsEditor({ wallets, onUpdated, onCancel }) {
    ...
    useInput((input, key) => {
      ...
      // Focus management for Strategy ID input: when focused, let TextInput capture keystrokes.
    ...
    return h(
      Box,
      { flexDirection: 'column' },
      h(Text, null, 'Select wallet (↑/↓). Toggle a=auto-attach, d=default funding. Tab/t to edit Strategy ID. Enter to save.'),
      h(
        Box,
        { marginTop: 1, flexDirection: 'column' },
        wallets.map((w, idx) => {
