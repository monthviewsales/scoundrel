    ...
    React.useEffect(() => {
      ...
      registry
        ...
        .then((result) => {
          ...
          list.sort((a, b) => {
            const aDefault = a?.isDefaultFunding || a?.is_default_funding;
            const bDefault = b?.isDefaultFunding || b?.is_default_funding;
            if (!!aDefault !== !!bDefault) return aDefault ? -1 : 1;

            const aAuto = a?.autoAttachWarchest || a?.auto_attach_warchest;
            const bAuto = b?.autoAttachWarchest || b?.auto_attach_warchest;
            if (!!aAuto !== !!bAuto) return aAuto ? -1 : 1;

            const aLabel = String(a?.alias || '').toLowerCase();
            const bLabel = String(b?.alias || '').toLowerCase();
  ...
  function AutopsyPrompt({ onSubmit, registry = walletRegistry, defaultMint = '' }) {
    ...
    return h(
      ...
      stage === 'mint'
        ...
        : null,
      stage !== 'walletList' && !isValidWalletAddress(walletAddress) && walletAddress.length > 0
        ? h(Text, { color: 'yellow' }, 'Wallet address does not look like base58.')
        : null,
      !isValidMintAddress(mint) && mint.length > 0 ? h(Text, { color: 'yellow' }, 'Mint does not look like base58.') : null,
      h(
        Box,
        { marginTop: 1 },
