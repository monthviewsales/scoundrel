...
async function harvestWallet({ wallet, traderName, startTime, endTime, limit = DEFAULT_LIMIT, concurrency = 6, includeOutcomes = false, featureMintCount = FEATURE_MINT_COUNT_DEFAULT, runAnalysis = true }) {
  ...
  try {
    ...
    function writeDataArtifact(subdirs, filename, data) {
      const dir = path.join(dataDir, ...subdirs);
      try { fs.mkdirSync(dir, { recursive: true }); } catch (_) {}
      const fullPath = path.join(dir, filename);
      fs.writeFileSync(fullPath, J(data));
      return fullPath;
    }
    ...
    try {
      ...
      if (ENV === 'development') {
        ...
        try {
          ...
          if (SAVE_RAW) {
            const tradesPath = path.join(process.cwd(), 'data', traderAlias, 'raw', `trades-${runId}.json`);
            const chartPath = path.join(process.cwd(), 'data', traderAlias, 'raw', `chart-${runId}.json`);
            try { fs.unlinkSync(tradesPath); } catch (_) {}
