...
function printAutopsyToConsole({ walletLabel, walletAddress, mint, aiResult }) {
  ...
  if (header || mint) {
    logger.log(`Wallet: ${header || 'n/a'}`);
    logger.log(`Mint  : ${mint}`);
    logger.log('');
  }

  if (grade) {
    logger.log(`Grade: ${grade}${gradeReason ? ` — ${gradeReason}` : ''}`);
    logger.log('');
  }

  if (summary) {
    logger.log('Summary:\n');
    logger.log(normalizeText(summary));
    logger.log('');
  }

  if (entryAnalysis) {
    logger.log('Entry Analysis:\n');
    logger.log(normalizeText(entryAnalysis));
    logger.log('');
  }

  if (exitAnalysis) {
    logger.log('Exit Analysis:\n');
    logger.log(normalizeText(exitAnalysis));
    logger.log('');
  }

  if (riskManagement) {
    logger.log('Risk Management:\n');
    logger.log(normalizeText(riskManagement));
    logger.log('');
  }

  if (profitability) {
    logger.log('Profitability:\n');
    logger.log(normalizeText(profitability));
    logger.log('');
  }

  if (whereYouLeftEV) {
    logger.log('Where You Left EV:\n');
    logger.log(normalizeText(whereYouLeftEV));
    logger.log('');
  }

  if (Array.isArray(lessons) && lessons.length) {
    logger.log('Key Lessons:\n');
