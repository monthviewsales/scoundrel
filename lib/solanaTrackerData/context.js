...
function createCallWithRetry({ attempts = DEFAULT_ATTEMPTS, baseDelayMs = DEFAULT_BASE_DELAY_MS }) {
  ...
  return async function call(opName, exec, opts = {}) {
    ...
    for (let attemptIndex = 0; attemptIndex < localAttempts; attemptIndex += 1) {
      ...
      try {
        logger.debug('data api call', { op: opName, attempt: attemptNo });
