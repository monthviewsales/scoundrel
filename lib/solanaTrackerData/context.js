...
const defaultLogger = typeof rootLogger.solanaTrackerData === 'function'
  ? rootLogger.solanaTrackerData()
  : rootLogger.child({ scope: 'SolanaTrackerDataClient' });
...
function createCallWithRetry({ logger, attempts = DEFAULT_ATTEMPTS, baseDelayMs = DEFAULT_BASE_DELAY_MS }) {
  ...
  const debug = typeof log.debug === 'function' ? log.debug.bind(log) : defaultLogger.debug.bind(defaultLogger);
  const warn = typeof log.warn === 'function' ? log.warn.bind(log) : defaultLogger.warn.bind(defaultLogger);
  const errorLog = typeof log.error === 'function' ? log.error.bind(log) : defaultLogger.error.bind(defaultLogger);
