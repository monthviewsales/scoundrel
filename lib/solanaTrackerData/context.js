...
function createCallWithRetry({ logger, attempts = DEFAULT_ATTEMPTS, baseDelayMs = DEFAULT_BASE_DELAY_MS }) {
  ...
  const info = typeof log.info === 'function' ? log.info.bind(log) : fallbackLogger.info.bind(fallbackLogger);
  ...
  return async function call(opName, exec, opts = {}) {
    ...
    for (let attemptIndex = 0; attemptIndex < localAttempts; attemptIndex += 1) {
      ...
      try {
        ...
        info('data api success', { op: opName, attempt: attemptNo, durationMs });
        if (typeof result === 'object' && result !== null) {
          const keys = Object.keys(result);
          debug('data api success detail', { op: opName, attempt: attemptNo, durationMs, resultKeys: keys.slice(0, 8) });
        } else {
          debug('data api success detail', { op: opName, attempt: attemptNo, durationMs, resultType: typeof result });
        }
        return result;
      } catch (error) {
        lastError = error;
