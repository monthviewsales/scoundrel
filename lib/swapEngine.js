...
async function performTrade(request) {
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      logger.debug(
        chalk.bgGray('[swapEngine] Full swap response:', swapResp)
      );
    }
    ...
    try {
      ...
      logger.debug('[swapEngine] performSwapWithDetails result:', result);
      ...
      logger.info('[swapEngine] Transaction successful:', txid);
    } catch (err) {
