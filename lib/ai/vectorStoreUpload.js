 ...
 */
async function queueVectorStoreUpload(params = {}, options = {}) {
  const signal = options && options.signal ? options.signal : null;
  const onSpawn = options && typeof options.onSpawn === 'function' ? options.onSpawn : null;
  if (signal && signal.aborted) {
    return { queued: false, reason: 'aborted' };
  }
  ...
  if (!jsonPath) {
    ...
    const filename = `${sanitizePrefix(source)}-${randomUUID()}.json`;
