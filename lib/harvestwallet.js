...
async function harvestWallet({ wallet, startTime, endTime, limit = DEFAULT_LIMIT, concurrency = 6, includeOutcomes = false }) {
  console.log(`[harvestWallet] start wallet=${wallet} start=${startTime || 'N/A'} end=${endTime || 'N/A'} limit=${limit} concurrency=${concurrency}`);
  ...
  try {
    ...
    console.log(`[harvestWallet] parsed ${parsed.length} trades`);

    if (SAVE_PARSED) {
      try { saveAsJson(`${wallet}-parsed`, parsed); } catch (e) { console.warn('[harvestWallet] warn: failed to save parsed JSON:', e?.message || e); }
    }

    // 3) Enrich with t0 snapshots + features
    const { enriched, count, errors } = await enrichTrades({ trades: parsed, client: st, concurrency, includeOutcomes });
    ...
    return { wallet, startTime: startTime || null, endTime: endTime || null, count, errors, openAiResult };
  } catch (err) {
