  ...
  await activePool.query(
    `INSERT INTO sc_wallet_analyses (
     ...
     ON DUPLICATE KEY UPDATE
       ...
       json_version = VALUES(json_version),
...
async function getWalletAnalysisById(analysisId) {
  const activePool = requirePool();
  const [rows] = await activePool.query(
    "SELECT * FROM sc_wallet_analyses WHERE analysis_id = ?",
    [analysisId]
  );
  return rows && rows[0] ? rows[0] : null;
}

async function listWalletAnalysesByWallet(wallet, { limit = 20 } = {}) {
  const activePool = requirePool();
  const [rows] = await activePool.query(
    `SELECT * FROM sc_wallet_analyses WHERE wallet = ? ORDER BY created_at DESC LIMIT ?`,
    [wallet, Number.isFinite(limit) ? limit : 20]
  );
  return rows || [];
}

async function recordTradeAutopsy({
  autopsyId,
  wallet,
  mint,
  symbol,
  ...
  if (!autopsyId || !wallet || !mint) {
    throw new Error("recordTradeAutopsy: autopsyId, wallet, and mint are required");
  }
  const activePool = requirePool();
  await activePool.query(
...
async function getTradeAutopsyById(autopsyId) {
  const activePool = requirePool();
  const [rows] = await activePool.query(
    "SELECT * FROM sc_trade_autopsies WHERE autopsy_id = ?",
    [autopsyId]
  );
  return rows && rows[0] ? rows[0] : null;
}

async function listTradeAutopsiesByWallet(wallet, { limit = 20 } = {}) {
  const activePool = requirePool();
  const [rows] = await activePool.query(
    `SELECT * FROM sc_trade_autopsies WHERE wallet = ? ORDER BY created_at DESC LIMIT ?`,
    [wallet, Number.isFinite(limit) ? limit : 20]
  );
  return rows || [];
}
