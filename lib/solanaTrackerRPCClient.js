...
const { classifySolanaError, shouldRetryRpcError } = require('./solana/errors');

const KIT_LOG_SLOW_MS = Number(process.env.KIT_LOG_SLOW_MS || 500);
const KIT_RPC_MAX_RETRIES = Number(process.env.KIT_RPC_MAX_RETRIES || 1);
const KIT_RPC_RETRY_BASE_MS = Number(process.env.KIT_RPC_RETRY_BASE_MS || 200);
const KIT_RPC_RETRY_MAX_MS = Number(process.env.KIT_RPC_RETRY_MAX_MS || 2000);
...
function computeBackoffMs(attempt) {
  const base = KIT_RPC_RETRY_BASE_MS * Math.pow(2, attempt);
  const jitter = Math.floor(Math.random() * Math.min(250, base));
  return Math.min(KIT_RPC_RETRY_MAX_MS, base + jitter);
}
...
function ensureProxyAwareFetch(url) {
  ...
  logger.info(`http using proxy ${redactUrl(proxyUrl)}`);
}
  ...
  class ProxyAwareWebSocket extends BaseWebSocket {
    constructor(address, protocols, options) {
      ...
      if (proxyUrl && mergedOptions !== baseOptions) {
        logger.info(`ws using proxy ${redactUrl(proxyUrl)}`);
      }
      ...
      return (...args) => {
        ...
        if (req && typeof req.send === 'function') {
          ...
          req.send = async (...sendArgs) => {
            ...
            const rawPayload = args && args.length ? args[0] : undefined;
            const rawOptions = args && args.length > 1 ? args[1] : undefined;
            const payload =
            ...
            const logFullPayload = String(process.env.KIT_RPC_LOG_PAYLOAD || '').toLowerCase() === 'full';
            let attempt = 0;
            while (true) {
              const started = Date.now();
              try {
                logger.debug('rpc.send start', {
                  method,
                  payloadLength: payloadLen,
                  payloadPreview: logFullPayload ? payload : payloadPreview,
                ...
                const ms = Date.now() - started;
                const summary = classifySolanaError(error, { method });
                logger.error('rpc.send error', {
                  method,
                  ms,
                ...
                const canRetry = shouldRetryRpcError(method, error) && attempt < KIT_RPC_MAX_RETRIES;
                if (!canRetry) {
                  throw error;
                }
                const delayMs = computeBackoffMs(attempt);
                logger.warn('rpc.send retry', {
                  method,
                  attempt: attempt + 1,
                  delayMs,
                  reason: summary.userMessage,
                });
                await sleep(delayMs);
                attempt += 1;
              }
        ...
        if (result && typeof result.subscribe === 'function') {
          ...
          result.subscribe = async (opts = {}) => {
            ...
            try {
              ...
              const summary = classifySolanaError(e, { method: subType });
