...
const KIT_LOG_LEVEL = (process.env.KIT_LOG_LEVEL || kitLogLevelFromNodeEnv()).toLowerCase();
...
function ensureProxyAwareFetch(url) {
  ...
  kitLog('info', 'http using proxy', { proxy: redactUrl(proxyUrl) });
}

function ensureProxyAwareWebSocket() {
  if (globalThis.__scoundrelProxyWebSocket) return globalThis.WebSocket || WebSocket;
  ...
  class ProxyAwareWebSocket extends BaseWebSocket {
    constructor(address, protocols, options) {
      ...
      if (proxyUrl && mergedOptions !== baseOptions) {
        kitLog('info', 'ws using proxy', { proxy: redactUrl(proxyUrl) });
      }
      ...
      if (typeof this.on === 'function') {
        this.on('unexpected-response', (request, response) => {
          kitLog('error', 'ws unexpected response', {
      ...
      return (...args) => {
        ...
        if (result && typeof result.subscribe === 'function') {
          ...
          result.subscribe = async (opts = {}) => {
            ...
            try {
              kitLog('debug', 'subscribe', { type: subType, subId });
              ...
              const wrapped = {
                ...
                async return(value) {
                  try {
                    ...
                    kitLog('debug', 'subscription closed', { subId, count });
                  }
...
function createSolanaTrackerRPCClient({ httpUrl = KIT_HTTP_URL, wsUrl = KIT_WS_URL } = {}) {
  ...
  if (wsUrl) ensureProxyAwareWebSocket(wsUrl);
