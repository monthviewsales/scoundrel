...
const { loadConfig } = require('./swap/swapConfig');     // or wherever Scoundrel stores swap config
...
async function makeTrackerClient(walletAlias) {
  ...
  const apiKey = cfg.swapApiKey || cfg.swapAPIKey || undefined;
...
async function buyToken({ walletAlias, mint, amount }) {
  ...
  const priorityFeeArg = cfg.priorityFee === undefined ? undefined : cfg.priorityFee;
