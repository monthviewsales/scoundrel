...
class WalletManagerV2 {
  ...
  async processSignature(signature) {
    ...
    const executedAtCandidate = Number(info.executedAt);
    const executedAt =
      Number.isFinite(executedAtCandidate) && executedAtCandidate > 0
        ? executedAtCandidate
        : Date.now();
    ...
    logger.debug(
      chalk.bgYellow.black(`[WalletManagerV2] Built ScTradeEvent for ${signature}: ` + 
  ...
  async persistTradeEvent(tradeEvent) {
    ...
    try {
      ...
      logger.error(
        chalk.bgRed.whiteBright('[WalletManagerV2] Failed to record sc_trades row for %s (wallet=%s, mint=%s, side=%s): %s'),
        tradeEvent.txid,
   ...
   * @returns {Promise<void>}
   */
  async applyTradeEventToPositions() {
    // DEPRECATED: WalletManagerV2 no longer applies positions directly.
    // BootyBox.recordScTradeEvent() is the single-writer entry point and is responsible for keeping sc_positions in sync.
    logger.warn(
      '[WalletManagerV2] applyTradeEventToPositions() is deprecated and is now a no-op; positions are updated by recordScTradeEvent().',
    );
  }
}
