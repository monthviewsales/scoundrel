...
function colorForLevel(level) {
  ...
  if (normalized === "debug") return ANSI.blue;
  if (normalized === "verbose") return ANSI.blue;
  if (normalized === "silly") return ANSI.blue;
  return ANSI.white;
}

function splitJsonSegments(text) {
  const segments = [];
  ...
  for (let i = 0; i < text.length; i += 1) {
    ...
    if (ch === "{") {
      if (depth === 0) {
        if (i > lastIndex) {
          segments.push({ type: "text", value: text.slice(lastIndex, i) });
        }
        jsonStart = i;
      }
      depth += 1;
    ...
    if (ch === "}" && depth > 0) {
      ...
      if (depth === 0 && jsonStart >= 0) {
        segments.push({ type: "json", value: text.slice(jsonStart, i + 1) });
        lastIndex = i + 1;
        jsonStart = -1;
      }
    }
...
function colorizeBracketSegments(text, enableColor) {
  if (!enableColor || typeof text !== "string") return text;
  const bracketRegex = /\[[^\]\r\n]+\]/g;
  ...
  return segments
    .map((segment) => {
      if (segment.type === "json") return segment.value;
      return segment.value.replace(bracketRegex, (match) => {
        return `${ANSI.white}${match}${ANSI.reset}`;
      });
    })
    .join("");
}

function formatConsoleLine(info, enableColor) {
  ...
  const combined = `${scope}${message}`;

  if (!enableColor) {
    return `${timestamp} ${level}: ${combined}`;
  }

  const tsText = `${ANSI.white}${timestamp}${ANSI.reset}`;
  const levelText = `${colorForLevel(level)}${level}${ANSI.reset}`;
  const coloredMessage = colorizeBracketSegments(combined, true);
  return `${tsText} ${levelText}: ${coloredMessage}`;
}
...
const consoleFormat = winston.format.combine(
  ...
  winston.format.printf((info) => formatConsoleLine(info, supportsConsoleColor()))
);
...
const logger = winston.createLogger({
  ...
  transports: [
      ...
      format: winston.format.combine(
        dropScopes(
          isHudMode() || isInkMode()
            ...
            : ["worker", "metrics", "BootyBox", "SolanaTrackerDataClient", "tokenBouncer", "targetList"]
        ),
        consoleFormat
      ),
    }),
    ...
    // Dedicated buyOps stream (buyOps worker traces).
    new winston.transports.DailyRotateFile({
      filename: path.join(LOG_ROOT_DIR, "BuyOps_%DATE%.log"),
      datePattern: "YYYY-MM-DD",
      maxSize: "50m",
      maxFiles: "7d",
      level: "debug",
      format: buildScopedMetaFormat("buyOps"),
    }),
...
// BuyOps workflow traces (kept out of console in Ink/HUD mode; routed to BuyOps_%DATE%.log).
logger.buyOps = function buyOps() {
  return logger.child({ scope: "buyOps" });
};
