 ...
 * @returns {{ mints: string[], runAnalysis: boolean, concurrency: number, skipVectorStore: boolean, forceTokenRefresh: boolean, sendVectorStore: boolean, client?: Object }}
 */
...
function appendTargetAddedEvent({
  ...
  const summary = suffix ? `Target added: ${label} ${suffix}` : `Target added: ${label}`;
  try {
    appendHubEvent({
      type: 'targetAdded',
      summary,
 ...
 *
 * @param {{ mint: string, payload: Object, analysis: Object|null }} params
 * @returns {Promise<{ existingTarget: Object|null, updatedTarget: Object|null }|null>}
 */
async function upsertTargetFromScan({ mint, payload, analysis }) {
  if (!mint) return null;
...
const runTargetScanFlow = createAnalysisFlow({
  ...
  persist: async ({ payload, analysis, buildResult, options }) => {
    ...
    let existingTarget = null;
    if (mint) {
      const upsertResult = await upsertTargetFromScan({ mint, payload, analysis });
  ...
  const runOne = async () => {
    while (true) {
      ...
      try {
        ...
        if (!res.analysis) {
          await upsertTargetFromScan({ mint, payload: res.payload, analysis: null });
        }
        results[idx] = {
          mint,
          payload: res.payload,
