 ...
 * @returns {{ mints: string[], runAnalysis: boolean, concurrency: number, skipVectorStore: boolean, client?: Object }}
 */
function normalizeTargetScanOptions(options = {}) {
  ...
  const skipVectorStore = options.skipVectorStore === true;
...
function buildTargetUpdateFromScan({
  ...
  const now = Date.now();
  const analysisRating = typeof analysis?.rating === 'string' ? analysis.rating.trim() : '';
  ...
  return {
    ...
    rating: analysisRating || existing?.rating || null,
    ...
    if (mint) {
      try {
        ...
        if (typeof BootyBox.addUpdateTarget === 'function') {
          const existing = typeof BootyBox.getTarget === 'function' ? BootyBox.getTarget(mint) : null;
          BootyBox.addUpdateTarget(buildTargetUpdateFromScan({
            mint,
            payload,
            analysis,
            existing,
