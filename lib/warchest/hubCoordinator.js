...
const { buildWorkerEnv, forkWorkerWithPayload } = require('./workers/harness');
 ...
 * @returns {{runSwap:Function,runTxMonitor:Function,publishStatus:Function,publishHudEvent:Function,close:Function,on:Function,once:Function,paths:Object}}
 */
function createHubCoordinator(options = {}) {
  ...
  async function runWorker(command, payload) {
    const ns = namespaceFor(command, payload || {});
    if (active.has(ns)) {
      throw new Error(`Command already running for namespace ${ns}`);
    }

    const workerPath = command === 'txMonitor' ? txMonitorWorkerPath : swapWorkerPath;
    const envInput = { ...envBase, ...(commandEnv[command] || {}) };
    const env = buildWorkerEnv({ ...envInput, extraEnv: envInput });
    const lockTag = options.lockPrefix ? `${options.lockPrefix}-${ns}` : ns;

    emitter.emit('start', { command, namespace: ns, payload });

    const promise = forkWorkerWithPayload(workerPath, {
      payload,
  ...
  return {
    runSwap: (payload) => runWorker('swap', payload),
    runTxMonitor: (payload) => runWorker('txMonitor', payload),
    publishStatus,
    publishHudEvent,
