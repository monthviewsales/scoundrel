...
function createHubCoordinator(options = {}) {
  ...
  const logger = rootLogger && typeof rootLogger.child === 'function'
    ? rootLogger.child({ scope: 'hubCoordinator' })
    : rootLogger;
  ...
  emitter.on('error', (payload) => {
    ...
    logger.error('[hubCoordinator] worker error event', {
      command: payload && payload.command ? payload.command : null,
      namespace: payload && payload.namespace ? payload.namespace : null,
