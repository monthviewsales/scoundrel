 ...
 * @param {{wsSupervisor?: {restarts?:number,lastRestartAt?:number|null,lastRestartReason?:string|null,lastError?:string|null,lastErrorAt?:number|null,backoffMs?:number,restartInFlight?:boolean}, socketCount?: number|null}} [serviceStats]
 */
function updateHealth(state, rpcStats, serviceStats) {
  const now = Date.now();
  ...
  const wsSupervisor = serviceStats && serviceStats.wsSupervisor ? serviceStats.wsSupervisor : null;
  const socketCount =
    serviceStats && typeof serviceStats.socketCount === 'number'
      ? Math.max(0, Math.trunc(serviceStats.socketCount))
      : null;
  ...
  const health = {
    ...
    service:
      wsSupervisor || socketCount != null
        ? {
            wsSupervisor: wsSupervisor
              ? {
                  restarts: typeof wsSupervisor.restarts === 'number' ? wsSupervisor.restarts : 0,
                  lastRestartAt: wsSupervisor.lastRestartAt ?? null,
                  lastRestartReason: wsSupervisor.lastRestartReason ?? null,
                  lastError: wsSupervisor.lastError ?? null,
                  lastErrorAt: wsSupervisor.lastErrorAt ?? null,
                  backoffMs: typeof wsSupervisor.backoffMs === 'number' ? wsSupervisor.backoffMs : 0,
                  restartInFlight: wsSupervisor.restartInFlight === true,
                }
              : undefined,
            sockets: socketCount,
          }
