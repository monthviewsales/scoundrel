...
const {
  getStrategyDocs,
  getStrategyList,
  resolveStrategyOverride: resolveStrategyOverrideBase,
} = require("../strategyRegistry");
const {
  chooseStrategy,
  evalQualify,
  recommendAction,
} = require("../sellOps/decisionEngine");
...
const STRATEGY_LIST = getStrategyList();
...
function resolveStrategyOverride(value) {
  const resolved = resolveStrategyOverrideBase(value);
...
function resolveStrategyChoice({ payload, position, target }) {
  ...
  const positionStrategy = resolveStrategyOverride(
    position?.strategyName || position?.strategyId
  );
  if (positionStrategy) {
    return { strategy: positionStrategy, source: "position" };
  }

  const targetStrategy = resolveStrategyOverride(
...
function resolveExpectedNotional(strategy, payload) {
  const payloadUsd = toFiniteNumber(
    payload?.expectedNotionalUsd ??
      payload?.plannedNotionalUsd ??
      payload?.notionalUsd
  );
  const payloadSol = toFiniteNumber(
    payload?.expectedNotionalSol ??
      payload?.plannedNotionalSol ??
      payload?.notionalSol
  );

  const expectedNotionalUsd =
    payloadUsd != null && payloadUsd > 0 ? payloadUsd : null;
  const expectedNotionalSol =
    payloadSol != null && payloadSol > 0 ? payloadSol : null;
  ...
  const strategySol =
    baseUnitSol != null && baseUnitSol > 0 ? baseUnitSol : null;
...
function shouldSkipExitabilityGate(position, evaluation) {
  const hasTokens =
    Number.isFinite(Number(position?.currentTokenAmount)) &&
    Number(position.currentTokenAmount) > 0;
  const hasExitability = Number.isFinite(
    Number(evaluation?.derived?.liquidityToPositionRatio)
  );
...
function evalQualifyForBuy(strategy, evaluation, skipExitabilityGate) {
  ...
  const gates = Array.isArray(strategy?.qualify?.gates)
    ? strategy.qualify.gates
    : [];
  const filtered = gates.filter(
    (gate) => gate && gate.id !== "gate.liquidity.exitability"
  );
  if (filtered.length === gates.length)
    return evalQualify(strategy, evaluation);
  const patched = {
    ...strategy,
    qualify: {
      ...(strategy?.qualify || {}),
...
function resolveBuyDecision(target, qualify, minScore) {
  ...
  const score = Number(target?.score);
  const meetsScore =
    Number.isFinite(score) && Number.isFinite(minScore)
      ? score >= minScore
      : false;
  ...
  if (isBuyStatus && meetsScore) {
    if (
      qualify?.worstSeverity === "exit" ||
      qualify?.worstSeverity === "degrade"
    ) {
      return "skip";
    }
    return "buy";
 ...
 * @returns {{ eventIntervals?: string[], ohlcv?: Object|null, indicators?: Object|null, freshness?: Object|null, includeCandles: boolean, includeOhlcv?: boolean }}
 */
function resolveEvaluationConfig(strategy, payload) {
  ...
  const eventIntervals =
    Array.isArray(payload?.eventIntervals) && payload.eventIntervals.length
  ...
  const ohlcv =
    includeOhlcv && (defaults.ohlcv || payload?.ohlcv)
      ? { ...(defaults.ohlcv || {}), ...(payload?.ohlcv || {}) }
      : null;

  const indicators =
    includeOhlcv && (defaults.indicators || payload?.indicators)
  ...
  return {
    ...
    indicators,
    freshness,
    includeCandles: includeOhlcv ? Boolean(payload?.includeCandles) : false,
    includeOhlcv,
  };
}
...
// Helper to build a compact, explainable signals block for regime explainability
function buildRegimeSignals(evaluation) {
  // Support multiple possible indicator locations
  const ind =
    evaluation?.indicators || evaluation?.ta || evaluation?.technical || null;
  const price =
    evaluation?.coin?.priceUsd ??
    evaluation?.coin?.price_usd ??
    evaluation?.pool?.priceUsd ??
    evaluation?.pool?.price_usd ??
    null;

  const emaFast = ind?.emaFast ?? ind?.ema_fast ?? null;
  const emaSlow = ind?.emaSlow ?? ind?.ema_slow ?? null;
  const slope = ind?.slopePctPerCandle ?? ind?.slope_pct_per_candle ?? null;

  const vwap = ind?.vwap ?? null;
  const macd = ind?.macd || null;
  const macdHist =
    macd && typeof macd === "object"
      ? macd.hist ?? macd.histogram ?? null
      : null;

  const emaTrend =
    Number.isFinite(Number(emaFast)) && Number.isFinite(Number(emaSlow))
      ? Number(emaFast) > Number(emaSlow)
        ? "up"
        : "down"
      : null;

  const aboveVwap =
    Number.isFinite(Number(price)) && Number.isFinite(Number(vwap))
      ? Number(price) >= Number(vwap)
      : null;

  const slopePct = Number.isFinite(Number(slope)) ? Number(slope) : null;

  return {
    priceUsd: Number.isFinite(Number(price)) ? Number(price) : null,
    emaFast: Number.isFinite(Number(emaFast)) ? Number(emaFast) : null,
    emaSlow: Number.isFinite(Number(emaSlow)) ? Number(emaSlow) : null,
    emaTrend,
    slopePctPerCandle: slopePct,
    vwap: Number.isFinite(Number(vwap)) ? Number(vwap) : null,
    aboveVwap,
    macdHist: Number.isFinite(Number(macdHist)) ? Number(macdHist) : null,
  };
}
...
function buildPositionForEvaluation(position, expectedNotional) {
  ...
  return {
    ...position,
    ...(expectedNotional.expectedNotionalUsd != null
      ? { expectedNotionalUsd: expectedNotional.expectedNotionalUsd }
      : {}),
    ...(expectedNotional.expectedNotionalSol != null
      ? { expectedNotionalSol: expectedNotional.expectedNotionalSol }
      : {}),
  };
}
...
async function runBuyOpsEvaluation(payload) {
  ...
  try {
    ...
    const initialExpectedNotional = resolveExpectedNotional(
      forcedStrategy?.strategy || null,
      payload
    );
    const initialConfig = resolveEvaluationConfig(
      forcedStrategy?.strategy || null,
      payload
    );
    const initialPosition = buildPositionForEvaluation(
      position,
      initialExpectedNotional
    );
    ...
    const buildSnapshot = async (config, positionOverride) => {
      const args = {
        ...
        includeOhlcv: config.includeOhlcv,
      };
    ...
    let { evaluation, warnings } = await buildSnapshot(
      initialConfig,
      initialPosition
    );
    ...
    const chosenConfig = resolveEvaluationConfig(
      chosen?.strategy || null,
      payload
    );
    const chosenExpectedNotional = resolveExpectedNotional(
      chosen?.strategy || null,
      payload
    );
    const needsConfigRefresh =
      fingerprintEvaluationConfig(chosenConfig) !==
      fingerprintEvaluationConfig(initialConfig);
    const needsNotionalRefresh =
      fingerprintExpectedNotional(chosenExpectedNotional) !==
      fingerprintExpectedNotional(initialExpectedNotional);

    if (needsConfigRefresh || needsNotionalRefresh) {
      const positionForChosen = buildPositionForEvaluation(
        position,
        chosenExpectedNotional
      );
    ...
    const qualify = evalQualifyForBuy(
      chosen.strategy,
      evaluation,
      skipExitabilityGate
    );
    ...
    const decision = resolveBuyDecision(
      target,
      evaluation.qualify,
    ...
    try {
      regime.signals = buildRegimeSignals(evaluation);
    } catch (_) {
      // ignore
    }
