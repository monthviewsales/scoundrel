...
async function runBuyOpsEvaluation(payload) {
  ...
  try {
    ...
    let { evaluation, warnings } = await buildSnapshot(initialConfig);
    ...
    if (!chosen) {
      ...
      if (fingerprintEvaluationConfig(chosenConfig) !== fingerprintEvaluationConfig(initialConfig)) {
        const rebuilt = await buildSnapshot(chosenConfig);
