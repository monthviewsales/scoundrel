...
const { buildHudChart, buildHudIndicators, buildHudMetrics } = require('../sellOps/indicatorAdapter');
...
function persistBuyOpsEvaluation({ bootyBox, target, snapshot, logger }) {
  if (!bootyBox || typeof bootyBox.insertBuyOpsEvaluation !== 'function') return;
  ...
  try {
    ...
    logger?.warn?.(
      `[buyOps] persist tick failed mint=${snapshot.mint || 'n/a'} wallet=${snapshot.walletAlias || 'n/a'}: ${
        err?.message || err
      }`
    );
  }
