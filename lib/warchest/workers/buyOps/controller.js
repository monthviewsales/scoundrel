...
function createBuyOpsController(payload = {}, tools = {}, log) {
  ...
  const targetListTimeoutMs = Number.isFinite(Number(payload.targetListTimeoutMs))
    ? Number(payload.targetListTimeoutMs)
    : DEFAULT_TARGET_LIST_TIMEOUT_MS;
  const targetScanTimeoutMs = Number.isFinite(Number(payload.targetScanTimeoutMs))
    ? Number(payload.targetScanTimeoutMs)
    : DEFAULT_TARGET_SCAN_TIMEOUT_MS;
  const evalTimeoutMs = Number.isFinite(Number(payload.evalTimeoutMs))
    ? Number(payload.evalTimeoutMs)
  ...
  function resolveDbHandle() {
    ...
    if (!db && ctx && typeof ctx.getDb === 'function') {
      try {
        ...
        logger.warn(`[buyOps] ctx.getDb() failed: ${err?.message || err}`);
      }
    }
    if (!db) {
      logger.warn('[buyOps] sqlite db handle unavailable; evaluation will be skipped.');
    }
    return db;
  }
  ...
  async function runTargetListTick() {
    ...
    try {
      logger.info('[buyOps] target list tick starting.');
      ...
      logger.info(
        `[buyOps] target list tick completed: mints=${summary.uniqueMints ?? 'n/a'} ` +
  ...
  async function runTargetScanTick() {
    ...
    try {
      logger.info('[buyOps] target scan tick starting.');
      ...
      if (!mints.length) {
        logger.info('[buyOps] targetscan skipped: no targets.');
      } else if (activeTargetScan) {
        ...
        if (!activeTargetScanWarned && Number.isFinite(ageMs) && ageMs > targetScanTimeoutMs) {
          ...
          logger.warn(
            `[buyOps] targetscan still running after ${Math.round(ageMs / 1000)}s; skipping new scan.`
          );
        } else {
          logger.info('[buyOps] targetscan already running; skipping new scan.');
        }
      } else {
        logger.info(`[buyOps] targetscan running: mints=${mints.length} concurrency=${scanConcurrency}`);
        ...
        activeTargetScan
          .then(() => {
            ...
            logger.info(`[buyOps] targetscan completed: mints=${mints.length}`);
          })
          .catch((err) => {
            logger.warn(`[buyOps] targetscan failed: ${err?.message || err}`);
          })
          .finally(() => {
      ...
      logger.info('[buyOps] evaluation tick starting.');
      lastEvaluationTickAt = Date.now();
      runEvaluationTick().then((evalStats) => {
        if (!evalStats) return;
        logger.info(
      ...
      logger.info('[buyOps] target scan tick completed.');
  ...
  async function runEvaluationTick() {
    ...
    let evaluated = 0;
    let errors = 0;
    const resolvedDb = resolveDbHandle();
    ...
    async function bootstrap() {
      ...
      heartbeatTimer = setInterval(() => {
        ...
        emitToParent('buyOps:heartbeat', {
          ...
          lastEvaluationTickAt,
          note: activeTargetScan ? 'targetscan running' : 'targetscan idle',
        });
        logger.info(
          `[buyOps] heartbeat alive targetList=${lastTargetListCompletedAt ? 'ok' : 'pending'} ` +
            `targetScan=${lastTargetScanCompletedAt ? 'ok' : 'pending'}`
        );
      }, 60_000);

      logger.info(
        `[buyOps] started targetListIntervalMs=${targetListIntervalMs ?? 'disabled'} ` +
          `targetScanIntervalMs=${targetScanIntervalMs ?? 'disabled'} minScore=${minScore} ` +
