...
function buildAutopsyMetricsPayload(event) {
  const details = event?.result || event?.payload || {};
  const wallet = details.walletAddress || details.wallet || null;

  return {
    worker: event?.worker || 'autopsyWorker',
...
function createAutopsyMetricsReporter() {
  if (!metricsLogger || typeof metricsLogger.debug !== 'function') return null;
  return (event) => {
    const payload = buildAutopsyMetricsPayload(event);
...
createWorkerHarness(runAutopsyWorker, {
  workerName: 'autopsyWorker',
  logger,
  metricsReporter: createAutopsyMetricsReporter(),
});
