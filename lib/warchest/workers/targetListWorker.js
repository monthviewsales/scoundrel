  ...
  try {
    spawnWorkerDetached(workerPath, {
      payload: {
        ...
        // NOTE: skipVectorStore prevents bulk target-list scans from uploading to the vector store.
...
async function fetchTargetList({ dataClient }) {
  ...
  if (bootyBoxReady && typeof BootyBox.pruneTargets === 'function') {
    try {
      targetsPruned = BootyBox.pruneTargets({
