 ...
 * @returns {Promise<{ runId: string, volume: any, trending: any, mints: string[], artifacts: { volumePath: string|null, trendingPath: string|null, performerPath: string|null } }>}
 */
async function fetchTargetList({ dataClient }) {
  const artifacts = createArtifactWriter({ command: ARTIFACT_COMMAND, logger });
  const [volumeRaw, trendingRaw, performerRaw] = await Promise.all([
    dataClient.getTokensByVolumeWithTimeframe({ timeframe: "1h" }),
    dataClient.getTrendingTokens({ timeframe: "6h" }),
    dataClient.getTopPerformersByTimeframe({ timeframe: "6h" }),
  ]);

  const countsRaw = {
    volume: countTokens(volumeRaw),
    trending: countTokens(trendingRaw),
    performing: countTokens(performerRaw),
  };

  const volume = applyTokenBouncer(volumeRaw);
  const trending = applyTokenBouncer(trendingRaw);
  const performer = applyTokenBouncer(performerRaw);

  const countsFiltered = {
    volume: countTokens(volume),
    trending: countTokens(trending),
    performing: countTokens(performer),
  };
  const totals = {
    raw: sumCounts(countsRaw),
    filtered: sumCounts(countsFiltered),
  };

  const volumeEntries = extractTokenEntries(volume);
  const trendingEntries = extractTokenEntries(trending);
  const performerEntries = extractTokenEntries(performer);
  ...
  const volumePath = artifacts.write("raw", "tokens-by-volume-1hr", volume);
  const trendingPath = artifacts.write("raw", "trending-tokens-6h", trending);
  const performingPath = artifacts.write(
    "raw",
    "performing-tokens-6h",
    performer
  );
  ...
  return {
    ...
    artifacts: { volumePath, trendingPath, performingPath },
    countsRaw,
    countsFiltered,
    totals,
  };
 ...
 *
 * @param {{ dataClient: ReturnType<typeof createSolanaTrackerDataClient>, skipTargetScan?: boolean, progress?: Function }} deps
 * @returns {Promise<object>}
 */
async function runTargetListOnce({ dataClient, skipTargetScan = false, progress }) {
  const startedAt = Date.now();
  const { runId, volume, trending, performer, artifacts, summary, mints, countsRaw, countsFiltered, totals } =
    await fetchTargetList({ dataClient });
  const endedAt = Date.now();
  if (typeof progress === "function") {
    progress("targetlist:counts", {
      totals,
      countsRaw,
      countsFiltered,
  ...
  return {
    ...
    totals,
    counts: {
      volume: countsFiltered?.volume ?? countTokens(volume),
      trending: countsFiltered?.trending ?? countTokens(trending),
      performing: countsFiltered?.performing ?? countTokens(performer),
    },
  };
}
 ...
 * @returns {Promise<object>}
 */
async function runTargetListWorker(payload, tools = {}) {
  ...
  try {
    const progress =
      tools && typeof tools.progress === "function" ? tools.progress : null;
    return await runTargetListOnce({ dataClient, skipTargetScan, progress });
  } finally {
