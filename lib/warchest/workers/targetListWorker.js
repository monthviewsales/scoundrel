...
const { createWorkerHarness, safeSerializePayload, spawnWorkerDetached } = require('./harness');
...
async function ensureBootyBoxReady() {
  if (!BootyBox || typeof BootyBox.init !== 'function') {
    logger.warn('[targetList] BootyBox client unavailable; skipping target persistence.');
    return false;
  }
  try {
    await BootyBox.init();
    ...
    logger.warn(`[targetList] BootyBox init failed; skipping target persistence: ${msg}`);
    return false;
  }
  if (typeof BootyBox.addUpdateTarget !== 'function') {
    logger.warn('[targetList] BootyBox.addUpdateTarget missing; skipping target persistence.');
    return false;
  }
  return true;
}
...
function extractTokenEntries(payload) {
  ...
  if (payload && Array.isArray(payload.tokens)) return payload.tokens.filter((row) => row && typeof row === 'object');
  if (payload && payload.token && typeof payload.token === 'object') return [payload];
  return [];
}

function extractMint(entry) {
  if (!entry || typeof entry !== 'object') return null;
  const token = entry.token && typeof entry.token === 'object' ? entry.token : entry;
  return token.mint || token.address || entry.mint || entry.address || null;
}

function extractTokenLabel(entry) {
  const token = entry && entry.token && typeof entry.token === 'object' ? entry.token : entry || {};
  return {
...
function spawnTargetScanWorker(mints, runId) {
  ...
  const prefix = runId ? `targetscan-targetlist-${runId}` : 'targetscan-targetlist';
  try {
    ...
    logger.warn(`[targetList] failed to spawn targetscan worker: ${err?.message || err}`);
  }
}
...
async function fetchTargetList({ dataClient }) {
  ...
  for (const [mint, entry] of tokensByMint.entries()) {
    ...
    try {
      ...
      logger.warn(`[targetList] token info fetch failed for ${mint}: ${err?.message || err}`);
    }

    if (bootyBoxReady) {
      try {
        const label = extractTokenLabel(info || entry);
        const existingTarget = typeof BootyBox.getTarget === 'function'
        ...
        BootyBox.addUpdateTarget({
          ...
          strategyId: existingTarget?.strategy_id || existingTarget?.strategyId || null,
          source: existingTarget?.source || ARTIFACT_COMMAND,
          tags: existingTarget?.tags || null,
          notes: existingTarget?.notes || null,
          vectorStoreId: existingTarget?.vector_store_id || existingTarget?.vectorStoreId || null,
          vectorStoreFileId: existingTarget?.vector_store_file_id || existingTarget?.vectorStoreFileId || null,
          vectorStoreUpdatedAt: Number.isFinite(existingTarget?.vector_store_updated_at)
            ? existingTarget.vector_store_updated_at
            : (Number.isFinite(existingTarget?.vectorStoreUpdatedAt) ? existingTarget.vectorStoreUpdatedAt : null),
          confidence: Number.isFinite(existingTarget?.confidence) ? existingTarget.confidence : null,
          score: Number.isFinite(existingTarget?.score) ? existingTarget.score : null,
          mintVerified: existingTarget?.mint_verified === 1 || existingTarget?.mintVerified === true,
          createdAt: Number.isFinite(existingTarget?.created_at) ? existingTarget.created_at : Date.now(),
          updatedAt: Date.now(),
        ...
        targetsFailed += 1;
        logger.warn(`[targetList] failed to upsert target ${mint}: ${err?.message || err}`);
      }
    }
  }
  ...
  if (bootyBoxReady) {
    try {
      ...
      logger.warn(`[targetList] failed to prune targets: ${err?.message || err}`);
    }
  }
...
async function runTargetListOnce({ dataClient, skipTargetScan = false }) {
  ...
  const { runId, volume, trending, artifacts, summary, mints } = await fetchTargetList({ dataClient });
  ...
  try {
    ...
    logger.warn(`[targetList] failed to append hub event: ${err?.message || err}`);
  }
...
async function runTargetListWorker(payload) {
  const { runOnce, intervalMs, skipTargetScan } = validateTargetListPayload(payload || {});
