...
// --- DB helpers (prepared for extraction) ---
function getSellOpsDb() {
  try {
    // Optional external module (to be created later). Must export:
    // { getDbFromSqliteContext, dbQuery, normalizeRows, loadOpenPositions }
    // eslint-disable-next-line global-require
    const mod = require('./sellOpsDb');
...
// SellOps runs in its own process; it must open the DB itself.
// The `db` package exports service methods but does not expose a raw db handle.
function getDbFromSqliteContext() {
  try {
    // eslint-disable-next-line global-require
    const ctx = require('../../../db/src/adapters/sqlite/context');

    // Common export shapes
    if (ctx && ctx.db) return ctx.db;
    if (ctx && ctx.connection) return ctx.connection;
    if (ctx && typeof ctx.getContext === 'function') {
      const c = ctx.getContext();
      if (c?.db) return c.db;
      if (c?.connection) return c.connection;
      return c || null;
    }

    return ctx || null;
  } catch (err) {
    logger.error(`[sellOps] failed to load sqlite context: ${err?.message || err}`);
    return null;
  }
}
...
async function dbQuery(db, sql, params) {
  ...
  if (typeof db.prepare === 'function') {
    ...
    // Normalize params: allow undefined/null, array, or object.
    const p = params == null ? [] : params;
    // Default to `.all()` (we only use SELECTs in SellOps right now).
    return stmt.all(p);
  }

  // sqlite-style adapters that expose `all(sql, params)`
  if (typeof db.all === 'function') {
    return db.all(sql, params);
  }

  throw new Error(
    `sellOps DB handle does not support query/execute/prepare/all (keys=${Object.keys(db || {}).slice(0, 15).join(',')})`
  );
}
...
const localSellOpsDb = {
  getDbFromSqliteContext,
  dbQuery,
  normalizeRows,
  loadOpenPositions,
};

function resolveSellOpsDb() {
  return getSellOpsDb() || localSellOpsDb;
}
...
async function loadOpenPositions(db, walletAlias) {
  ...
  const sellOpsDb = resolveSellOpsDb();
  const res = await sellOpsDb.dbQuery(db, sql, [walletAlias]);
  const rows = sellOpsDb.normalizeRows(res);
  ...
  const finalPromise = new Promise((resolve, reject) => {
    ...
    async function tick() {
      ...
      try {
        ...
        const sqliteCtxDb = !tools.db ? sellOpsDb.getDbFromSqliteContext() : null;
        db = db || tools.db || sqliteCtxDb || null;

        logger.debug(
          `[sellOps] db resolved source=${tools.db ? 'tools.db' : sqliteCtxDb ? 'sqliteContext' : 'none'} ` +
            `keys=${Object.keys(db || {}).slice(0, 15).join(',') || 'none'} ` +
            `hasQuery=${db && typeof db.query === 'function'} hasExecute=${db && typeof db.execute === 'function'} hasPrepare=${db && typeof db.prepare === 'function'}`
        );
        ...
        const { rows } = await resolveSellOpsDb().loadOpenPositions(db, wallet.alias);
