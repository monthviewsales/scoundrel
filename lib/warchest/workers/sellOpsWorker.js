  ...
  createWorkerHarness(
    async (payload, { track, env }) => {
      ...
      const walletAlias =
        payload?.walletAlias ||
        payload?.alias ||
        payload?.wallet?.alias ||
        payload?.wallet?.walletAlias;
      const walletPubkey =
        payload?.walletPubkey || payload?.pubkey || payload?.wallet?.pubkey;

      sellOpsLogger.debug(
        `[sellOps] IPC payload received keys=${
          Object.keys(payload || {}).join(",") || "none"
        } ` +
          `walletAlias=${walletAlias || "n/a"} walletPubkey=${
            walletPubkey ? String(walletPubkey).slice(0, 6) + "…" : "n/a"
          }`
      );

      sellOpsLogger.debug(
        `[sellOps] IPC payload snapshot ${JSON.stringify(
          redact(payload || {})
        )}`
      );

      sellOpsLogger.debug(
        `[sellOps] env presence WARCHEST_DATA_ENDPOINT=${
          env?.WARCHEST_DATA_ENDPOINT ? "yes" : "no"
        } ` +
