      ...
      try {
        ...
        if (!rows.length) {
          ...
          for (const row of rows) {
            ...
            if (tokenPoolOhlcv) {
              const poolRow = evalResult.evaluation?.pool;
              const poolAddress =
                poolRow?.poolAddress ||
              ...
              if (poolRow && !poolAddress) {
                logger.warn(
                  `[sellOps] missing pool address for OHLCV wallet=${wallet.alias} mint=${summary.mint} market=${poolRow.market || 'n/a'} quoteToken=${poolRow.quoteToken || 'n/a'} (pools table does not include a pool address column)`
                );
              }
            ...
            const poolMarket = evalResult.evaluation?.pool?.market;
            const poolQuote = evalResult.evaluation?.pool?.quoteToken;
            ...
            logger.info(
              `[sellOps] wallet=${wallet.alias} ${tradeTag} ${mintTag} decision=${evalResult.decision} reasons=${evalResult.reasons.join(',')} ` +
                ...
                `ohlcv=${ohlcvType && ohlcvPoints != null ? `${ohlcvType}:${ohlcvPoints}@${poolMarket || 'n/a'}:${poolQuote || 'n/a'}` : 'n/a'} ` +
