...
function resolveDb(client) {
  ...
  // Prefer an actual DB connection/adapter with query/execute/prepare.
  return (
    client?.db ||
    client?.bootybox?.db ||
    client?.bootyBox?.db ||
    client?.bootybox?.connection ||
    client?.bootyBox?.connection ||
    client?.bootybox?.pool ||
    client?.bootyBox?.pool ||
    // Fall back to adapters only if they look queryable.
    (client?.bootybox && (typeof client.bootybox.query === 'function' || typeof client.bootybox.execute === 'function' || typeof client.bootybox.prepare === 'function')
      ? client.bootybox
      : null) ||
    (client?.bootyBox && (typeof client.bootyBox.query === 'function' || typeof client.bootyBox.execute === 'function' || typeof client.bootyBox.prepare === 'function')
      ? client.bootyBox
  ...
  const finalPromise = new Promise((resolve, reject) => {
    ...
    async function tick() {
      ...
      try {
        ...
        logger.debug(
          `[sellOps] db resolved keys=${Object.keys(db || {}).slice(0, 15).join(',') || 'none'} ` +
