...
const ALERT_DEBOUNCE_MS = 30_000; // throttle HUD/log alerts for repeated edge cases
...
/**
 * Extract avg cost USD from a PnL row.
 * @param {any} row
 * @returns {number|null}
 */
function extractAvgCostUsdFromPnl(row) {
  if (!row) return null;
  const avg = Number(row.avg_cost_usd);
  return Number.isFinite(avg) && avg > 0 ? avg : null;
}

/**
 * Build stop-loss/trailing-stop context for HUD display.
 * @param {Object} args
 * @param {number|null} args.roiPct
 * @param {number|null} args.priceUsd
 * @param {any|null} args.trailingState
 * @param {number|null} args.hardStopLossPct
 * @param {number|null} args.activationPct
 * @param {number|null} args.trailPct
 * @returns {Object|null}
 */
function buildStopContext({ roiPct, priceUsd, trailingState, hardStopLossPct, activationPct, trailPct }) {
  const roi = Number.isFinite(Number(roiPct)) ? Number(roiPct) : null;
  const hardStop = Number.isFinite(Number(hardStopLossPct)) ? Math.abs(Number(hardStopLossPct)) : null;
  const stopLossTriggerPct = hardStop != null ? -hardStop : null;
  const stopLossDistancePct = roi != null && hardStop != null ? roi + hardStop : null;

  const state = trailingState || null;
  const armed = Boolean(state && state.active);
  const activation = Number.isFinite(Number(state?.activationPct))
    ? Number(state.activationPct)
    : (activationPct != null && Number.isFinite(Number(activationPct)) ? Number(activationPct) : null);
  const trail = Number.isFinite(Number(state?.trailPct))
    ? Number(state.trailPct)
    : (trailPct != null && Number.isFinite(Number(trailPct)) ? Number(trailPct) : null);
  const stopUsd = Number.isFinite(Number(state?.stopUsd)) ? Number(state.stopUsd) : null;
  const highWaterUsd = Number.isFinite(Number(state?.highWaterUsd)) ? Number(state.highWaterUsd) : null;
  const price = Number.isFinite(Number(priceUsd)) ? Number(priceUsd) : null;

  const trailingStopDistancePct =
    price != null && stopUsd != null && price > 0 ? ((price - stopUsd) / price) * 100 : null;
  const rawArmDistance = !armed && roi != null && activation != null ? activation - roi : null;
  const trailingDistanceToArmPct = rawArmDistance != null && rawArmDistance > 0 ? rawArmDistance : null;

  return {
    roiPct: roi,
    hardStopLossPct: hardStop,
    stopLossTriggerPct,
    stopLossDistancePct,
    trailing: {
      armed,
      activationPct: activation ?? null,
      trailPct: trail ?? null,
      highWaterUsd,
      stopUsd,
      distancePct: trailingStopDistancePct,
      distanceToArmPct: trailingDistanceToArmPct,
    },
  };
}
...
function createSellOpsController(
  ...
  const stopDefaults = getTrailingStopConfig(STRATEGY_DOCS?.flash || null);
  ...
  const lastCostBasisAttemptByTradeUuid = new Map();
  // tradeUuid -> {
  //   active: boolean,
  ...
  const finalPromise = new Promise((resolve, reject) => {
    function shouldThrottleAlert(key, nowMs) {
      const last = lastAlertByKey.get(key) || 0;
      if (nowMs - last < ALERT_DEBOUNCE_MS) return true;
      lastAlertByKey.set(key, nowMs);
      return false;
    }

    function emitAlert(level, code, message, extra = {}) {
      const now = Date.now();
      const key = `${code}:${extra.tradeUuid || extra.mint || 'unknown'}`;
      if (shouldThrottleAlert(key, now)) return;
      ...
      if (level === 'error') {
        log.error(`[sellOps] ${code}: ${message}`);
      } else if (level === 'warn') {
        log.warn(`[sellOps] ${code}: ${message}`);
      } else {
        log.info(`[sellOps] ${code}: ${message}`);
      }

      emitToParent('sellOps:alert', payload);
    }

    async function seedCostBasisCache(positions, reason) {
      const missing = (positions || []).filter((pos) => {
        if (!pos || !pos.tradeUuid) return false;
        const cached = costUsdByTradeUuid.get(pos.tradeUuid);
        return !(Number.isFinite(Number(cached)) && Number(cached) > 0);
      });
      if (!missing.length) return;

      let pnlByTradeUuid = null;
      let pnlByMint = null;
      const walletId = missing.find((p) => p && p.walletId != null)?.walletId;

      if (walletId && typeof BootyBox.getPnlPositionsLive === 'function') {
        try {
          const rows = await BootyBox.getPnlPositionsLive({ walletId: Number(walletId) });
          pnlByTradeUuid = new Map();
          pnlByMint = new Map();
          for (const row of rows || []) {
            if (row && row.trade_uuid) pnlByTradeUuid.set(row.trade_uuid, row);
            if (row && row.coin_mint) pnlByMint.set(row.coin_mint, row);
          }
        } catch (err) {
          emitAlert('error', 'pnl_live_fetch_failed', `Failed to load live PnL view for cost basis (${err?.message || err})`, {
            reason,
            walletId,
          });
        }
      ...
      for (const pos of missing) {
        let costUsd = null;
        if (pnlByTradeUuid || pnlByMint) {
          const pnlRow =
            (pnlByTradeUuid && pnlByTradeUuid.get(pos.tradeUuid)) ||
            (pnlByMint && pnlByMint.get(pos.mint)) ||
            null;
          costUsd = extractAvgCostUsdFromPnl(pnlRow);
        }
        if (!costUsd) {
          costUsd = resolveAvgCostUsd(pos, null);
        }

        if (Number.isFinite(Number(costUsd)) && Number(costUsd) > 0) {
          costUsdByTradeUuid.set(pos.tradeUuid, Number(costUsd));
        } else {
          emitAlert(
            'error',
            'cost_basis_missing',
            `Missing cost basis for ${pos.mint || 'mint?'}; fast loop cannot compute ROI`,
            {
              tradeUuid: pos.tradeUuid,
              mint: pos.mint,
              reason,
            }
          );
    ...
    function buildSellAllSwapPayload({ pos, tradeUuid, mint, reason, hardStopLossPct }) {
      const amtNum = Number(pos.currentTokenAmount);
      const amountDecimal = Number.isFinite(amtNum) && amtNum > 0 ? String(amtNum) : null;
      const amount = amountDecimal || '100%';

      if (!amountDecimal) {
        emitAlert(
          'warn',
          'swap_amount_fallback',
          `Missing currentTokenAmount; falling back to sell 100% to satisfy swap contract`,
          {
            tradeUuid,
            mint,
            currentTokenAmount: pos.currentTokenAmount ?? null,
      ...
      return {
        ...
        side: 'sell',
        amount,
        ...(amountDecimal ? { amountDecimal, fromAmountDecimal: amountDecimal } : {}),
        percent: 1,
        sellPercent: 100,
        sellPct: 100,
        sellAll: true,
        ...
        ...(hardStopLossPct != null ? { hardStopLossPct } : {}),
      };
    }
      ...
      async function trailingTick() {
        ...
        try {
          ...
          for (const pos of previousOpenPositions.values()) {
            ...
            const amtMissing = amtRaw === null || amtRaw === undefined || (typeof amtRaw === 'string' && amtRaw.trim() === '');
            if (amtMissing || !Number.isFinite(amt)) {
              emitAlert(
                'error',
                'current_token_amount_missing',
                `currentTokenAmount missing or invalid for ${pos.mint || 'mint?'} in fast loop`,
                {
                  tradeUuid: pos.tradeUuid,
                  mint: pos.mint,
                  currentTokenAmount: amtRaw ?? null,
                }
              );
            }
          ...
          for (const pos of previousOpenPositions.values()) {
            ...
            if (!costUsd || !Number.isFinite(Number(costUsd)) || Number(costUsd) <= 0) {
              const lastAttempt = lastCostBasisAttemptByTradeUuid.get(tradeUuid) || 0;
              if (now - lastAttempt >= ALERT_DEBOUNCE_MS) {
                lastCostBasisAttemptByTradeUuid.set(tradeUuid, now);
                await seedCostBasisCache([pos], 'fast_loop');
              }
              const refreshed = costUsdByTradeUuid.get(tradeUuid) || null;
              if (!refreshed || !Number.isFinite(Number(refreshed)) || Number(refreshed) <= 0) {
                // If we don't know cost, we can't do ROI-based trailing. Skip with alert.
                missingCostSkips += 1;
                continue;
              }
            }

            const resolvedCostUsd = costUsdByTradeUuid.get(tradeUuid) || costUsd;
            const roiPct = ((priceUsd / Number(resolvedCostUsd)) - 1) * 100;
    ...
    async function tick() {
      ...
      try {
        ...
        for (const [tradeUuid, summary] of currentPositions.entries()) {
          if (!previousOpenPositions.has(tradeUuid)) {
            newPositions.push(summary);
          }
        }
        ...
        if (newPositions.length) {
          await seedCostBasisCache(newPositions, 'new_position');
        }
        ...
        if (!rows.length) {
          ...
          for (const summary of summaries) {
            ...
            const roiPct = snapshot.evaluation?.derived?.roiUnrealizedPct ?? null;
            const priceUsd = snapshot.evaluation?.coin?.priceUsd ?? snapshot.evaluation?.coin?.price_usd ?? null;
            const hardStopLossPct = Number.isFinite(Number(payload?.hardStopLossPct))
              ? Number(payload.hardStopLossPct)
              : stopDefaults.hardStopLossPct;
            const trailingState = summary.tradeUuid ? trailingStateByTradeUuid.get(summary.tradeUuid) : null;
            const stopContext = buildStopContext({
              roiPct,
              priceUsd,
              trailingState,
              hardStopLossPct,
              activationPct: stopDefaults.activationPct,
              trailPct: stopDefaults.trailPct,
            });
            const hudPayload = {
              ts: snapshot.ts,
