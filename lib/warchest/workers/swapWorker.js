...
const logger = typeof baseLogger.swap === 'function' ? baseLogger.swap() : baseLogger;
...
async function ensureBootyBoxReady() {
  ...
  try {
    ...
    (INK_MODE ? logger.debug : logger.warn)(`[swapWorker] BootyBox module not available: ${msg}`);
...
function spawnTxMonitorDetached(context) {
  ...
  [
    'TX_MONITOR_TEST_LOG',
    'TX_MONITOR_RPC_FACTORY',
    'TX_MONITOR_EVENT_PATH',
    'SC_INK_MODE',
    'SC_INK_ALLOW_CONSOLE',
    'LOG_LEVEL',
  ].forEach((key) => {
...
async function executeSwap(payload, tools = {}) {
  ...
  if (executor) {
    ...
    try {
      ...
      logger.error('[swapWorker] swap RPC call failed', {
        side: normalized.side,
        fromMint,
        toMint,
        walletPubkey,
        rpcEndpoint,
        swapApiBaseUrl,
        swapApiKeyPresent: Boolean(swapApiKey),
        errorMessage: msg,
  ...
  if (!isDryRun && result.txid) {
    ...
    const monitorContext = {
      txid: result.txid,
      dryRun: isDryRun,
      wallet: walletPubkey || resolvedWalletAlias || normalized.walletPrivateKey || null,
    ...
    if (normalized.detachMonitor) {
      try {
        ...
        (INK_MODE ? logger.debug : logger.warn)(
          `[swapWorker] failed to detach tx monitor for ${monitorContext.txid}: ${err?.message || err}`
        );
        monitorResult = null;
      }
    } else {
...
async function spawnTxMonitor(context) {
  ...
  try {
    ...
    const msg = err && err.message ? err.message : err;
    (INK_MODE ? logger.debug : logger.warn)(`[swapWorker] tx monitor failed to start for ${context.txid}: ${msg}`);
  }
  return null;
}
...
function startHarness() {
  ...
  createWorkerHarness(async (payload, tools) => {
    try {
      ...
      (INK_MODE ? logger.debug : logger.error)('[swapWorker] swap failed:', err?.message || err);
      throw err;
    }
  });
}
