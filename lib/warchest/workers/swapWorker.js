...
async function executeSwap(payload, tools = {}) {
  ...
  const normalizedTxid = normalizeTxid(swapV3Resp.txid);

  const result = {
    txid: normalizedTxid,
    signature: normalizedTxid,
    dryRun: isDryRun,
    ...
    totalFees: swapV3Resp.totalFeesSol != null ? Number(swapV3Resp.totalFeesSol) : undefined,
