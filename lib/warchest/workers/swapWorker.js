...
async function executeSwap(payload, tools = {}) {
  ...
  if (executor) {
    ...
    const rpcHttpUrl = process.env.SOLANATRACKER_RPC_HTTP_URL || (swapConfig && swapConfig.rpcUrl) || null;
    ...
    try {
      if (swapProvider === 'raptor') {
        ...
        swapV3Resp = await executeSwapRaptor({
          ...
          swapApiKey,
          slippagePercent,
          priorityFee: priorityFee != null ? priorityFee : priorityFeeLevel || null,
          txVersion: normalizeTxVersionForProvider(txVersion, swapProvider),
          preflight,
