...
function buildSwapMetricsPayload(event) {
  const details = event?.result || event?.payload || {};
  const txid = details.txid || details.signature || null;
  const wallet = details.walletPubkey || details.wallet || details.walletAddress || null;

  return {
    worker: event?.worker || 'swapWorker',
    event: event?.event || null,
    requestId: event?.requestId || null,
    durationMs: event?.durationMs ?? null,
    ...(wallet ? { wallet } : {}),
    ...(details.walletAlias ? { walletAlias: details.walletAlias } : {}),
    ...(details.walletId != null ? { walletId: details.walletId } : {}),
...
function createSwapMetricsReporter() {
  if (!metricsLogger || typeof metricsLogger.debug !== 'function') return null;
  return (event) => {
    const payload = buildSwapMetricsPayload(event);
...
function startHarness() {
  ...
  createWorkerHarness(async (payload, tools) => {
    ...
    workerName: 'swapWorker',
    logger,
    metricsReporter: createSwapMetricsReporter(),
  });
}
