 ...
 * @param {Function} [tools.metricsReporter] - Optional metrics hook for lifecycle/retry events.
 * @param {Object} [tools.retryOptions] - Optional retry tuning for RPC calls.
 * @param {Function} [tools.retryDelayFn] - Optional override for retry delays (primarily for tests).
 * @throws {Error} When token accounts cannot be hydrated after retrying.
...
function createCoinMonitorController(payload, tools = {}) {
  ...
  const metrics = typeof tools.metricsReporter === 'function'
    ? (event) => tools.metricsReporter({ worker: 'coinMonitor', ...event })
    : null;
    ...
    async function refreshAccounts() {
      ...
      try {
        const res = await withRetry(
          () => rpcMethods.getTokenAccountsByOwner(wallet.pubkey, { mint, encoding: 'jsonParsed' }),
          {
            attempts: retryOptions.attempts || 4,
            baseMs: retryOptions.baseMs || 300,
            maxMs: retryOptions.maxMs || 2500,
            shouldRetry: retryOptions.shouldRetry,
            delayFn: retryOptions.delayFn || retryDelayFn,
            onRetry: (err, attempt) => {
              logger.warn(
                `[coinMonitor] retrying getTokenAccountsByOwner attempt ${attempt}: ${err?.message || err}`
              );
              if (typeof retryOptions.onRetry === 'function') {
                retryOptions.onRetry(err, attempt);
              }
              if (metrics) metrics({ event: 'retry:getTokenAccountsByOwner', attempt });
            },
          }
        );
        ...
        logger.error(`[coinMonitor] failed to fetch token accounts: ${err?.message || err}`);
        if (metrics) metrics({ event: 'error:getTokenAccountsByOwner', message: err?.message || String(err) });
        throw err;
      }
    }
...
function startHarness() {
  ...
  createWorkerHarness(
    ...
    {
      exitOnComplete: true,
      workerName: 'coinMonitor',
      metricsReporter: (event) => {
        logger.debug?.(`[coinMonitor][metrics] ${JSON.stringify(event)}`);
      },
      onClose: async () => {
