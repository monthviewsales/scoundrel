...
function loadBootyBox() {
  ...
  try {
    ...
    (INK_MODE ? logger.debug : logger.warn)(`[txMonitor] BootyBox module unavailable: ${msg}`);
...
async function ensureBootyBoxReady() {
  ...
  if (!bootyInitPromise) {
    bootyInitPromise = client
      ...
      .catch((err) => {
        const msg = err && err.message ? err.message : err;
        (INK_MODE ? logger.debug : logger.warn)(`[txMonitor] BootyBox init failed: ${msg}`);
...
async function persistSwapOutcome(payload, finalResult, insight, txDetails) {
  ...
  try {
    ...
    const msg = err && err.message ? err.message : err;
    (INK_MODE ? logger.warn : logger.error)(`[txMonitor] Failed to record sc_trades row for ${payload.txid}: ${msg}`);
  }
}
...
async function watchViaSignature(txid, rpcMethods, track, metricsReporter, { commitment = 'confirmed' } = {}) {
  ...
  return new Promise((resolve) => {
    ...
    const onError = (err) => {
      (INK_MODE ? logger.debug : logger.warn)(`[txMonitor] signature subscription error for ${txid}: ${err?.message || err}`);
    ...
    subscribePromise
      ...
      .catch((err) => {
        (INK_MODE ? logger.debug : logger.warn)(`[txMonitor] failed to subscribe to signature ${txid}: ${err?.message || err}`);
...
async function fetchTransactionOnce(txid, rpcMethods, { commitment = 'confirmed' } = {}) {
  ...
  try {
    ...
    (INK_MODE ? logger.debug : logger.warn)(`[txMonitor] getTransaction failed for ${txid}: ${err?.message || err}`);
...
async function monitorTransaction(payload, tools = {}) {
  ...
  try {
    ...
    try {
      ...
      (INK_MODE ? logger.debug : logger.warn)(`[txMonitor] insight recovery failed for ${txid}: ${err?.message || err}`);
    }
    ...
    try {
      ...
      (INK_MODE ? logger.debug : logger.warn)(`[txMonitor] failed to write HUD event: ${err?.message || err}`);
    ...
    try {
      ...
      (INK_MODE ? logger.debug : logger.warn)(`[txMonitor] failed to write HUD event after error: ${writeErr?.message || writeErr}`);
      if (metricsReporter) metricsReporter({ event: 'hud:write:error', txid });
    }
    (INK_MODE ? logger.debug : logger.warn)(`[txMonitor] ❌ monitor error for ${txid}: ${err?.message || err}`);
...
if (require.main === module) {
  ...
  if (standaloneArgs.payloadBase64 || standaloneArgs.payloadFile) {
    (async () => {
      ...
      (INK_MODE ? logger.warn : logger.error)(`[txMonitor] standalone failed: ${err?.message || err}`);
      process.exit(1);
    });
  } else {
    startHarness();
