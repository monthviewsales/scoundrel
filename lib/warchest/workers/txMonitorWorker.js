...
async function persistSwapOutcome(payload, finalResult, insight, txDetails) {
  ...
  try {
    ...
    logger.debug(
      chalk.bgYellow.black('[txMonitor][ScTradeEvent] ') +
        JSON.stringify(toJsonSafe(tradeEvent), null, 2),
    );
...
async function monitorTransaction(payload, tools = {}) {
  ...
  try {
    ...
    const statusEmoji = finalResult.status === 'confirmed' ? '✅' : finalResult.status === 'failed' ? '❌' : '⚠️';
    ...
    return toJsonSafe({ ...finalResult, insight });
  } catch (err) {
