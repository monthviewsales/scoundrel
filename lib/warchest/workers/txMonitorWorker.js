...
const txLog = createWorkerLogger({
...
const logger = txLog;
const metricsLogger = typeof baseLogger.metrics === 'function' ? baseLogger.metrics() : baseLogger;
let inkMode = false;
...
const { classifySolanaError, formatSolanaErrorMessage } = require('../../solana/errors');
try {
  process.title = 'scoundrel-txMonitorWorker';
} catch (_) {
  // ignore
}
const { isStableMint } = require('../../solana/stableMints');

// Canonical SOL mint and a tiny cache for on-demand SOL/USD lookups (Data API fallback).
const SOL_MINT = 'So11111111111111111111111111111111111111112';
const SOL_USD_CACHE_TTL_MS = 60 * 1000;
let solPriceDataClient = null;
const solUsdCache = {
  value: null,
  fetchedAt: 0,
  inFlight: null,
...
function extractSwapMints({ seed, payload, insight, swapQuote }) {
  const out = new Set();

  const add = (v) => {
    const m = normalizeMint(v);
    if (m) out.add(m);
  };

  // Seed (from swapWorker) may include richer mint hints.
  add(seed && seed.mint);
  add(seed && seed.inputMint);
  add(seed && seed.outputMint);
  add(seed && seed.inMint);
  add(seed && seed.outMint);
  add(seed && seed.baseMint);
  add(seed && seed.quoteMint);

  // Payload context.
  add(payload && payload.mint);

  // Insight may include mint hints.
  add(insight && insight.mint);
  add(insight && insight.inputMint);
  add(insight && insight.outputMint);
  add(insight && insight.inMint);
  add(insight && insight.outMint);
  add(insight && insight.baseMint);
  add(insight && insight.quoteMint);

  // Quote payloads (support multiple shapes)
  const quote = swapQuote && (swapQuote.quote || swapQuote);
  const raw = swapQuote && (swapQuote.rawQuoteResponse || quote?.rawQuoteResponse || swapQuote.rawQuoteResponse);

  add(quote && (quote.inputMint || quote.inMint || quote.baseMint));
  add(quote && (quote.outputMint || quote.outMint || quote.quoteMint));

  add(raw && (raw.inputMint || raw.inMint || raw.baseMint));
  add(raw && (raw.outputMint || raw.outMint || raw.quoteMint));

  return Array.from(out);
}

function pickDisplayMint(mints, fallbackMint) {
  const fallback = normalizeMint(fallbackMint);
  const list = Array.isArray(mints) ? mints.map(normalizeMint).filter(Boolean) : [];

  // If exactly two mints and one is stable, show the non-stable side.
  if (list.length === 2) {
    const [a, b] = list;
    const aStable = isStableMint(a);
    const bStable = isStableMint(b);
    if (aStable && !bStable) return b;
    if (!aStable && bStable) return a;
  }

  // Otherwise, prefer a non-stable mint if present.
  for (const m of list) {
    if (!isStableMint(m)) return m;
  }
...
function getSolPriceDataClient() {
  if (solPriceDataClient) return solPriceDataClient;
  try {
    solPriceDataClient = createSolanaTrackerDataClient();
  } catch (err) {
    traceTxMonitor('solUsdPrice-fallback-client-failed', { err: err?.message || err });
    solPriceDataClient = null;
  }
  return solPriceDataClient;
}

/**
 * On-demand SOL/USD fetch with a short-lived cache so we don't block every trade.
 *
 * @returns {Promise<number|null>}
 */
async function getCachedSolUsdPrice() {
  const now = Date.now();
  if (solUsdCache.value != null && now - solUsdCache.fetchedAt < SOL_USD_CACHE_TTL_MS) {
    return solUsdCache.value;
  }
  if (solUsdCache.inFlight) {
    return solUsdCache.inFlight;
  }

  const client = getSolPriceDataClient();
  if (!client || typeof client.getMultipleTokenPrices !== 'function') {
    return solUsdCache.value;
  }

  solUsdCache.inFlight = (async () => {
    try {
      const resp = await client.getMultipleTokenPrices({ mints: [SOL_MINT] });
      const maybePrice = resp && resp[SOL_MINT] && resp[SOL_MINT].price;
      const price = Number(maybePrice);
      if (Number.isFinite(price) && price > 0) {
        solUsdCache.value = price;
        solUsdCache.fetchedAt = Date.now();
        return price;
      }
      return solUsdCache.value;
    } catch (err) {
      traceTxMonitor('solUsdPrice-fallback-failed', { err: err?.message || err });
      return solUsdCache.value;
    } finally {
...
function loadBootyBox() {
  ...
  try {
    ...
    (inkMode ? logger.debug : logger.warn)(`[txMonitor] BootyBox module unavailable: ${msg}`);
...
async function ensureBootyBoxReady() {
  ...
  if (!bootyInitPromise) {
    bootyInitPromise = client
      ...
      .catch((err) => {
        const msg = err && err.message ? err.message : err;
        (inkMode ? logger.debug : logger.warn)(`[txMonitor] BootyBox init failed: ${msg}`);
 ...
 * @returns {object|null}
 */
async function buildTradeEventFromInsight(payload, insight, txDetails) {
  if (!payload || !insight) return null;
  if (insight.kind === 'transfer') return null;
  ...
  let solUsdPrice = deriveSolUsdPriceFromQuote(swapQuote, priceSolPerToken, priceUsdPerToken);
  if (solUsdPrice == null) {
    solUsdPrice = await getCachedSolUsdPrice();
  }
...
async function persistSwapOutcome(payload, finalResult, insight, txDetails) {
  ...
  const tradeEvent = await buildTradeEventFromInsight(payload, insight, txDetails);
  ...
  try {
    ...
    const msg = err && err.message ? err.message : err;
    (inkMode ? logger.warn : logger.error)(`[txMonitor] Failed to record sc_trades row for ${payload.txid}: ${msg}`);
  }
}
...
async function watchViaSignature(txid, rpcMethods, track, metricsReporter, { commitment = 'confirmed' } = {}) {
  ...
  return new Promise((resolve) => {
    ...
    const onError = (err) => {
      (inkMode ? logger.debug : logger.warn)(`[txMonitor] signature subscription error for ${txid}: ${err?.message || err}`);
    ...
    subscribePromise
      ...
      .catch((err) => {
        (inkMode ? logger.debug : logger.warn)(`[txMonitor] failed to subscribe to signature ${txid}: ${err?.message || err}`);
...
async function fetchTransactionOnce(txid, rpcMethods, { commitment = 'confirmed' } = {}) {
  ...
  try {
    ...
    (inkMode ? logger.debug : logger.warn)(`[txMonitor] getTransaction failed for ${txid}: ${err?.message || err}`);
...
function extractTxLogs(tx) {
  if (!tx || typeof tx !== 'object') return null;
  const meta = tx.meta || null;
  if (meta && Array.isArray(meta.logMessages)) return meta.logMessages;
  return null;
}
...
function finalizeTxSummary(seed, { txid, finalResult, txDetails, insight, payload, swapQuote }) {
  ...
  const errorSummary = finalResult && finalResult.errorSummary ? finalResult.errorSummary : null;
  const errMessage = errorSummary ? errorSummary.userMessage : err ? formatMonitorError(err) : '';

  const status = confirmed ? 'ok' : failed ? 'failed' : processed ? 'processed' : 'unknown';

  const insightKind = insight && insight.kind ? String(insight.kind) : null;
  ...
  if (insightKind === 'transfer' && (label === 'transaction' || !label)) {
    label = 'transfer';
  }

  return {
    kind: base.kind || insightKind || (base.side ? 'swap' : 'tx'),
    status,
    statusCategory: confirmed ? 'confirmed' : failed ? 'failed' : processed ? 'processed' : 'unknown',
    label,
    side: base.side || null,
    // Prefer a stablecoin-aware display mint when we can see both sides of a swap.
    mint: pickDisplayMint(
      extractSwapMints({ seed: base, payload, insight, swapQuote }),
      base.mint || (payload && payload.mint) || (insight && insight.mint) || null
    ),
    inputMint: base.inputMint || base.inMint || null,
    outputMint: base.outputMint || base.outMint || null,

    txid: base.txid || txid,
    explorerUrl: base.explorerUrl || buildExplorerUrl(base.txid || txid, payload && payload.explorerBaseUrl),
...
async function monitorTransaction(payload, tools = {}) {
  ...
  try {
    ...
    const txLogs = extractTxLogs(txDetails);
    ...
    const errorSummary = errPayload ? classifySolanaError(errPayload, { logs: txLogs }) : null;
    finalResult = { status, err: errPayload, slot, errorSummary };
    ...
    const statusEmoji =
      finalResult.status === 'confirmed'
        ...
        : finalResult.status === 'failed'
          ? '🔴'
          : '🟡';
    const errMessage = finalResult.err ? ` reason=${formatSolanaErrorMessage(finalResult.err)}` : '';
    ...
    try {
      ...
      (inkMode ? logger.debug : logger.warn)(`[txMonitor] insight recovery failed for ${txid}: ${err?.message || err}`);
    }

    const txSummary = finalizeTxSummary(txSummarySeed, { txid, finalResult, txDetails, insight, payload, swapQuote: payload.swapQuote || null });
    ...
    const hudEvent = {
      ...
      errorSummary: finalResult.errorSummary || null,
      context: hudContext,
      insight,
    ...
    try {
      ...
      (inkMode ? logger.debug : logger.warn)(`[txMonitor] failed to write HUD event: ${err?.message || err}`);
    ...
    const errorSummary = classifySolanaError(err, { logs: null });
    const fallback = finalResult || { status: 'failed', err, slot: null, errorSummary };
    const txSummary = finalizeTxSummary(txSummarySeed, { txid, finalResult: fallback, txDetails: null, insight: null, payload, swapQuote: payload?.swapQuote || null });
    ...
    const hudEvent = {
      ...
      errorSummary: fallback.errorSummary || errorSummary,
      context: hudContext,
      insight: null,
      swapQuote: payload.swapQuote || null,
      observedAt: new Date().toISOString(),
    ...
    try {
      ...
      (inkMode ? logger.debug : logger.warn)(`[txMonitor] failed to write HUD event after error: ${writeErr?.message || writeErr}`);
      if (metricsReporter) metricsReporter({ event: 'hud:write:error', txid });
    }
    (inkMode ? logger.debug : logger.warn)(`[txMonitor] ❌ monitor error for ${txid}: ${err?.message || err}`);
...
function startHarness() {
  createWorkerHarness(async (payload, { track }) => {
    inkMode = Boolean(payload && payload.inkMode);
    return monitorTransaction(payload, { track });
  }, {
    exitOnComplete: true,
    workerName: 'txMonitor',
    logger,
    metricsReporter: (event) => {
      metricsLogger.debug?.(`txMonitor ${JSON.stringify(event)}`);
    },
  });
}
...
if (require.main === module) {
  ...
  if (standaloneArgs.payloadBase64 || standaloneArgs.payloadFile) {
    (async () => {
      ...
      (inkMode ? logger.warn : logger.error)(`[txMonitor] standalone failed: ${err?.message || err}`);
      process.exit(1);
    });
  } else {
    startHarness();
