...
async function monitorTransaction(payload, tools = {}) {
  ...
  const signatureResultPromise = rpcMethods
    ? watchViaSignature(txid, rpcMethods, track, metricsReporter, { commitment: 'confirmed' })
  ...
  const signatureResult = await signatureResultPromise;
  let finalResult = signatureResult;
