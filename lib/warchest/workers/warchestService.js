...
} = require("../../services/tokenInfoService");
const {
...
async function fetchCoinMeta(mint) {
  ...
  try {
    const info = await ensureTokenInfo({
      mint,
      client: dataClient,
      forceRefresh: hudMetaRefreshTick,
    });
    if (info) {
...
async function main() {
  ...
  const targetScanConcurrency = Math.max(
    1,
    parseNumber(process.env.WARCHEST_TARGET_SCAN_CONCURRENCY, 5)
  );
  ...
  const buyOpsEvalConcurrency = Math.max(
    1,
    parseNumber(process.env.WARCHEST_BUYOPS_EVAL_CONCURRENCY, 6)
  );
  ...
  Promise.resolve()
    .then(async () => {
      try {
        ...
        await hub.runTargetList({ runOnce: true }, { timeoutMs: 60_000 });
  ...
  try {
    ...
    eventFollower.onEvent((event) => {
      ...
      pushServiceAlert(
        serviceAlerts,
        "info",
        `Target list updated (vol=${volume}, trend=${trending})`,
        {
