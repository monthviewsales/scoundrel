...
async function main() {
  ...
  try {
    buyOpsWorkerHandle = forkWorkerWithPayload(BUYOPS_WORKER_PATH, {
      ...
      waitForExit: false,
      payload: {
        targetListIntervalMs: process.env.WARCHEST_TARGET_LIST_INTERVAL_MS || 300000,
        targetScanIntervalMs: 60_000,
        scanConcurrency: 5,
        minScore: 65,
      },
      onProgress: (msg) => {
        ...
        const now = Date.now();
        if (now - buyOpsLastHeartbeatAt >= 60_000) {
    ...
    buyOpsWorkerHandle
      .then(({ result }) => {
        logger.info(`[HUD] BuyOps worker started: ${JSON.stringify(result)}`);
      })
      .catch((err) => {
        const msg = err && err.message ? err.message : err;
        logger.warn(`[HUD] BuyOps worker failed: ${msg}`);
      });
    pushServiceAlert(serviceAlerts, 'info', 'BuyOps worker started');
  } catch (err) {
    const msg = err && err.message ? err.message : err;
