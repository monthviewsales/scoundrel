    ...
    aliases.map(async (alias) => {
      ...
      try {
        ...
        for (const [mint, balance] of aggregated.entries()) {
          ...
          // Compact line for HUD: "+3.2% ($12.34)" or "-11.9% (-$0.62)"
          const positionLine = (() => {
            const p = position;
            if (!p) return prev && prev.positionLine ? prev.positionLine : null;
            const pct = p.roiPct;
            const up = p.unrealizedPnlUsd;
            const pctText = pct != null && Number.isFinite(pct) ? `${pct >= 0 ? '+' : ''}${pct.toFixed(2)}%` : null;
            const usdText = up != null && Number.isFinite(up) ? `(${up >= 0 ? '+' : ''}${up.toFixed(2)})` : null;
            if (pctText && usdText) return `${pctText} ${usdText}`;
            if (pctText) return pctText;
            if (usdText) return usdText;
            return prev && prev.positionLine ? prev.positionLine : null;
          })();
