...
async function ingestTxEvents(events, hudStore) {
  ...
  // taking from the tail is robust for an append-only event stream.
  const recent = events.slice(-HUD_MAX_TX).reverse();
...
async function fetchCoinMeta(mint) {
  ...
  try {
    const info = await ensureTokenInfo({ mint, client: dataClient, forceRefresh: true });
    if (info) {
