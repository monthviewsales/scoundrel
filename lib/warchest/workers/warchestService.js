...
async function ingestTxEvents(events, hudStore) {
  ...
  const mapped = await Promise.all(events.slice(0, HUD_MAX_TX).map((ev) => buildTxDisplay(ev)));
...
async function fetchCoinMeta(mint) {
  ...
  try {
    const info = await ensureTokenInfo(mint, { client: dataClient, refresh: true });
    if (info) {
...
function deriveStatusCategory(event) {
  ...
  const status = event.status || event.txSummary?.status || 'unknown';
...
async function buildTxDisplay(event) {
  if (!event || typeof event !== 'object') return null;
  const txSummary = event.txSummary || {};
  const statusCategory = deriveStatusCategory(event);
  const statusEmoji = event.statusEmoji || txSummary.statusEmoji || deriveStatusEmoji(statusCategory);
  const side = txSummary.side || (event.context && event.context.side) || null;
  const mint = txSummary.mint || (event.context && event.context.mint) || null;
  const wallet = event.context && event.context.wallet ? event.context.wallet : null;
  const slot = txSummary.slot != null ? txSummary.slot : event.slot != null ? event.slot : null;
  const observedAt = event.observedAt ? Date.parse(event.observedAt) : Date.now();
  const blockTimeIso = txSummary.blockTimeIso || null;
  const tokens = txSummary.tokens != null ? txSummary.tokens : null;
  const sol = txSummary.sol != null ? txSummary.sol : null;
  const priceImpactPct = txSummary.priceImpactPct != null ? txSummary.priceImpactPct : null;
  const totalFeesSol = txSummary.totalFeesSol != null ? txSummary.totalFeesSol : null;
  const explorerUrl = txSummary.explorerUrl || null;

  let coinMeta = null;
  if (mint) {
    const info = await fetchCoinMeta(mint);
    coinMeta = mapCoinMeta(info);
  }

  return {
    txid: event.txid || txSummary.txid || null,
    statusCategory,
    statusEmoji,
    side,
    mint,
    ...
    totalFeesSol,
    explorerUrl,
    label: txSummary.label || null,
    errMessage: txSummary.errMessage || (event.err && (event.err.message || JSON.stringify(event.err))) || null,
    coin: coinMeta,
  };
}
