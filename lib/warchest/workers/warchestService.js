      ...
      try {
        ...
        for (const [mint, balance] of aggregated.entries()) {
          ...
          if (tokenMeta === undefined) {
            if (doMeta) {
              try {
                // Full meta refresh tick: force refresh so we pick up pools/risk/events changes.
                tokenMeta = await ensureTokenInfo({ mint, client: dataClient, forceRefresh: true });
              } catch (metaErr) {
                const msg =
              ...
              // Price tick: avoid spending per-mint credits; reuse whatever we had previously.
              tokenMeta = null;
            }
