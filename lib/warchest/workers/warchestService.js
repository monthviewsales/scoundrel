...
const { pruneTargetsWithVectorStoreCleanup } = require('../../services/targetPruning');
...
const metricsLogger = typeof baseLogger.metrics === 'function'
  ? baseLogger.metrics()
  : baseLogger;
const dataLogger = typeof baseLogger.solanaTrackerData === 'function'
...
// HUD rendering helpers moved to lib/warchest/workers/warchest/hudRenderer.js
...
async function refreshPnlPositionsForWallet(wallet) {
  ...
  try {
    ...
    for (const row of rows || []) {
      ...
      const amt = toNum(normalized.current_token_amount ?? normalized.currentTokenAmount);
      if (!(amt > 0)) continue;
...
function createWalletTokenRefreshScheduler({
  ...
  const refreshBalances = typeof refreshTokenBalances === 'function'
    ? refreshTokenBalances
    : refreshTokenBalancesForWallet;
  const refreshPositions = typeof refreshPnlPositions === 'function'
    ? refreshPnlPositions
...
async function main() {
  const { wallets, mode } = parseArgs(process.argv, logger);
  ...
  try {
    ...
    logger.warn(`[HUD] Failed to prune targets on startup: ${err?.message || err}`);
  }
  ...
  const walletIdentifiers = resolvedWallets
    .map((wallet) => wallet.alias || wallet.walletAlias || wallet.walletAddress || wallet.wallet || wallet.pubkey || null)
    .filter(Boolean);
  ...
  // In HUD mode, keep stdout reserved for Ink. Route log-like writes to a HUD log file.
  ...
  hudStore = createHudStore(() => ({
    ...
    sellOps: JSON.parse(JSON.stringify(sellOpsOrchestrator ? sellOpsOrchestrator.getState() : { byWallet: {} })),
    transactions: txFeed.slice(0, HUD_MAX_TX),
    session: sessionState && sessionState.id
      ? {
  ...
  const targetListIntervalMs = parseIntervalMs(process.env.WARCHEST_TARGET_LIST_INTERVAL_MS, 300_000);
  const targetScanIntervalMs = parseIntervalMs(process.env.WARCHEST_TARGET_SCAN_INTERVAL_MS, 60_000);
  const targetScanConcurrency = Math.max(1, parseNumber(process.env.WARCHEST_TARGET_SCAN_CONCURRENCY, 5));

  const buyOpsEvalIntervalMs = parseIntervalMs(process.env.WARCHEST_BUYOPS_EVAL_INTERVAL_MS, 60_000);
  const buyOpsEvalConcurrency = Math.max(1, parseNumber(process.env.WARCHEST_BUYOPS_EVAL_CONCURRENCY, 6));
  const buyOpsMinScore = parseNumber(process.env.WARCHEST_BUYOPS_MIN_SCORE, 65);
  const buyOpsEvalTimeoutMs = parseNumber(process.env.WARCHEST_BUYOPS_EVAL_TIMEOUT_MS, 20_000);

  const WORKER_HEARTBEAT_STALE_MS = 180_000;
  const WORKER_STARTUP_GRACE_MS = 120_000;
  const WORKER_RESTART_COOLDOWN_MS = 120_000;
  const sellOpsStaleMs = Math.max(WORKER_HEARTBEAT_STALE_MS, pollIntervalMs * 3);
  ...
  const onTargetOpsProgress = (msg) => {
    ...
    if (!targetOpsStarted) {
      ...
      logger.info(`[HUD] TargetOps worker started status=${status}${detailText}`);
      pushServiceAlert(
        serviceAlerts,
        'info',
    ...
    const shouldLog = status !== 'alive' || now - targetOpsLastLoggedAt >= 60_000;
    if (shouldLog) {
      targetOpsLastLoggedAt = now;
      logger.info(`[HUD] TargetOps heartbeat status=${status}${detailText}`);
      pushServiceAlert(serviceAlerts, 'info', `TargetOps heartbeat status=${status}${detailText}`);
      emitHudChange();
    }
  };
  ...
  function startTargetOpsWorker(reason) {
    if (targetOpsWorkerHandle && typeof targetOpsWorkerHandle.stop === 'function') {
    ...
    try {
      ...
      pushServiceAlert(serviceAlerts, 'error', `TargetOps worker failed: ${msg}`);
    }
  }
  ...
  try {
    ...
    eventFollower.onEvent((event) => {
      if (!event || event.type !== 'targetList') return;
      const counts = event.counts || {};
      const volume = counts.volume != null ? counts.volume : 'n/a';
      const trending = counts.trending != null ? counts.trending : 'n/a';
      pushServiceAlert(serviceAlerts, 'info', `Target list updated (vol=${volume}, trend=${trending})`, {
  ...
  try {
    ...
    if (warlordAIClient && warlordAIClient.pid) {
      logger.info(`[HUD] WarlordAI worker started (pid=${warlordAIClient.pid})`);
    } else {
  ...
  const healthTimer = setInterval(() => {
    ...
    Promise.resolve()
      .then(async () => {
        ...
        const now = Date.now();
        const shouldRestartWorker = (lastHeartbeatAt, startedAt, lastRestartAt, staleMs) => {
          if (lastRestartAt && now - lastRestartAt < WORKER_RESTART_COOLDOWN_MS) return false;
          if (startedAt && now - startedAt < WORKER_STARTUP_GRACE_MS) return false;
          if (!lastHeartbeatAt) {
        ...
        if (buyOpsWorkerHandle && shouldRestartWorker(buyOpsLastHeartbeatAt, buyOpsStartedAt, buyOpsRestartedAt, WORKER_HEARTBEAT_STALE_MS)) {
          buyOpsRestartedAt = now;
          logger.warn('[HUD] BuyOps heartbeat stale; restarting worker.');
          pushServiceAlert(serviceAlerts, 'warn', 'BuyOps heartbeat stale; restarting worker.');
          startBuyOpsWorker('watchdog');
        }

        if (
          targetOpsWorkerHandle &&
          shouldRestartWorker(targetOpsLastHeartbeatAt, targetOpsStartedAt, targetOpsRestartedAt, WORKER_HEARTBEAT_STALE_MS)
        ) {
          targetOpsRestartedAt = now;
          logger.warn('[HUD] TargetOps heartbeat stale; restarting worker.');
          pushServiceAlert(serviceAlerts, 'warn', 'TargetOps heartbeat stale; restarting worker.');
          startTargetOpsWorker('watchdog');
        }

        if (sellOpsOrchestrator && typeof sellOpsOrchestrator.restartWallet === 'function') {
          ...
          for (const wallet of resolvedWallets) {
            ...
            const lastHeartbeatAt = hb && Number.isFinite(Number(hb.ts)) ? Number(hb.ts) : 0;
            const lastRestartAt = sellOpsRestartedAtByWallet.get(alias) || 0;
            const startedAt = serviceStartedAt;

            if (shouldRestartWorker(lastHeartbeatAt, startedAt, lastRestartAt, sellOpsStaleMs)) {
              sellOpsRestartedAtByWallet.set(alias, now);
              logger.warn(`[HUD] SellOps heartbeat stale (${alias}); restarting worker.`);
              pushServiceAlert(serviceAlerts, 'warn', `SellOps heartbeat stale (${alias}); restarting worker.`);
  ...
  function shutdown() {
    ...
    try {
      if (sellOpsOrchestrator && typeof sellOpsOrchestrator.stopAll === 'function') {
        sellOpsOrchestrator.stopAll();
      }
    } catch {}
    // Stop BuyOps worker (best-effort)
    try {
      buyOpsWorkerHandle?.stop?.();
    } catch {}
    // Stop TargetOps worker (best-effort)
    try {
      targetOpsWorkerHandle?.stop?.();
    } catch {}
