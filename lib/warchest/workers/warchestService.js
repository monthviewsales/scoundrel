...
const sellOpsState = {
  byWallet: {}, // alias -> { heartbeat, evalByMint }
};

// Keep last displayed decision per wallet+mint to avoid spamming recentEvents.
const sellOpsLastDisplayed = new Map(); // key `${alias}:${mint}` -> { decision, ts }
...
function upsertSellOpsHeartbeat(alias, hb) {
  ...
  if (!sellOpsState.byWallet[alias]) {
    sellOpsState.byWallet[alias] = { heartbeat: null, evalByMint: {} };
  }
...
function shouldDisplaySellOpsEvent(alias, mint, decision) {
  const key = `${alias}:${mint}`;
  const prev = sellOpsLastDisplayed.get(key) || null;
  const now = Date.now();

  // Always display the first time, and when decision changes.
  if (!prev || prev.decision !== decision) {
    sellOpsLastDisplayed.set(key, { decision, ts: now });
    return true;
  }

  // Otherwise, rate-limit to once every 5 minutes per mint.
  if (now - prev.ts > 5 * 60 * 1000) {
    sellOpsLastDisplayed.set(key, { decision, ts: now });
    return true;
  }
            ...
            const handle = forkWorkerWithPayload(workerPath, {
              ...
              onProgress: (msg) => {
                ...
                if (msg.type === 'sellOps:evaluation') {
                  ...
                  // Brief HUD-visible event (rate-limited).
                  ...
                  if (mint && shouldDisplaySellOpsEvent(alias, mint, decision)) {
                    pushRecentEvent(state[alias], `SellOps ${symbol} ${decision} (${regime})`, hudStore);
                    pushServiceAlert(serviceAlerts, 'info', `SellOps eval (${alias}) ${symbol} -> ${decision} (${regime})`);
                  }
