...
const sellOpsState = {
  byWallet: {}, // alias -> { heartbeat, evalByMint, fastByMint, autopsies }
};
...
function ensureSellOpsWalletState(alias) {
  if (!alias) return null;
  if (!sellOpsState.byWallet[alias]) {
    sellOpsState.byWallet[alias] = { heartbeat: null, evalByMint: {}, fastByMint: {}, autopsies: [] };
  }
  return sellOpsState.byWallet[alias];
}
...
function computeStopLossDistancePct(roiPct, hardStopLossPct) {
  const roi = Number(roiPct);
  const hardStop = Number(hardStopLossPct);
  if (!Number.isFinite(roi) || !Number.isFinite(hardStop)) return null;
  return roi + Math.abs(hardStop);
}

function computeTrailingDistancePct(priceUsd, stopUsd) {
  const price = Number(priceUsd);
  const stop = Number(stopUsd);
  if (!Number.isFinite(price) || !Number.isFinite(stop) || price <= 0) return null;
  return ((price - stop) / price) * 100;
}

function buildSellOpsLine({
  recommendation,
  strategyName,
  qualifyWorst,
  qualifyFailed,
  ...
  trailingDistanceToArmPct,
  trailingArmed,
}) {
  const rec = recommendation || 'hold';
  const stratPart = strategyName ? ` ${strategyName}` : '';
  const qPart = qualifyFailed > 0
    ? ` gates=${qualifyFailed} sev=${qualifyWorst || 'none'}${gateFail ? ` gate=${gateFail}` : ''}`
    : ' qualify=pass';

  const roiText = formatPct(roiPct);
  const slText = formatPct(stopLossDistancePct);
  const trailText = formatPct(trailingStopDistancePct);
  const armText = formatPct(trailingDistanceToArmPct);
  const extras = [];

  if (roiText) extras.push(`roi=${roiText}`);
  if (slText) extras.push(`sl=${slText}`);
  if (trailingArmed && trailText) extras.push(`trail=${trailText}`);
  if (!trailingArmed && armText) extras.push(`arm=${armText}`);

  return `SellOps ${rec}${stratPart}${qPart}${extras.length ? ` ${extras.join(' ')}` : ''}`;
}

function upsertSellOpsHeartbeat(alias, hb) {
  if (!alias) return;
  const state = ensureSellOpsWalletState(alias);
  if (state) state.heartbeat = hb || null;
}

function upsertSellOpsEvaluation(alias, mint, payload) {
  if (!alias || !mint) return;
  const state = ensureSellOpsWalletState(alias);
  if (state) state.evalByMint[mint] = payload;
}
 ...
 * @property {{ts:number,recommendation:string,strategyName:string|null,qualifyWorst:string,qualifyFailed:number,regime:string|null,gateFail:string|null,roiPct:number|null,hardStopLossPct:number|null,stopLossDistancePct:number|null,trailingStopDistancePct:number|null,trailingDistanceToArmPct:number|null,trailingArmed:boolean|null}|null} sellOps
            ...
            const handle = forkWorkerWithPayload(workerPath, {
              ...
              onProgress: (msg) => {
                ...
                if (msg.type === 'sellOps:evaluation') {
                  ...
                  if (mint && state?.[alias] && Array.isArray(state[alias].tokens)) {
                    ...
                    if (tokenRow) {
                      ...
                      const roiPct = p?.metrics?.roiUnrealizedPct ?? null;
                      const stopLossDistancePct = p?.stops?.stopLossDistancePct ?? null;
                      const trailingStopDistancePct = p?.stops?.trailing?.distancePct ?? null;
                      const trailingDistanceToArmPct = p?.stops?.trailing?.distanceToArmPct ?? null;
                      const trailingArmed = Boolean(p?.stops?.trailing?.armed);
                      const hardStopLossPct = p?.stops?.hardStopLossPct ?? null;
                      ...
                      tokenRow.sellOpsLine = buildSellOpsLine({
                        recommendation,
                        strategyName,
                        qualifyWorst,
                        qualifyFailed,
                ...
                if (msg.type === 'sellOps:alert') {
                  ...
                  const alias = p?.walletAlias || w.alias;
                  const mint = p?.mint || null;
                  const level = p?.level || 'warn';
                  const code = p?.code || 'alert';
                  const message = p?.message || code;
                  const symbol =
                    state?.[alias]?.tokens?.find((t) => t?.mint === mint)?.symbol ||
                    (mint ? mint.slice(0, 4) : 'mint');

                  const line = `SellOps ${level} (${alias}) ${symbol} ${message}`;
                  pushServiceAlert(serviceAlerts, level === 'error' ? 'error' : 'warn', line, { code });
                ...
                if (msg.type === 'sellOps:trailingStop:armed' || msg.type === 'sellOps:trailingStop:high') {
                  const p = msg.payload || null;
                  const alias = p?.walletAlias || w.alias;
                  const mint = p?.mint || null;
                  if (!mint) return;
                  const stateWallet = ensureSellOpsWalletState(alias);
                  if (stateWallet) {
                    stateWallet.fastByMint[mint] = { ...(p || {}), type: msg.type };
                  }

                  const tokenRow = state?.[alias]?.tokens?.find((t) => t?.mint === mint) || null;
                  if (tokenRow) {
                    const existing = tokenRow.sellOps || {};
                    const roiPct = Number.isFinite(Number(p?.roiPct)) ? Number(p.roiPct) : existing.roiPct;
                    const hardStopLossPct = p?.hardStopLossPct ?? existing.hardStopLossPct ?? null;
                    const trailingStopDistancePct = computeTrailingDistancePct(p?.priceUsd, p?.stopUsd) ?? existing.trailingStopDistancePct;
                    const rawArmDistance =
                      p?.activationPct != null && roiPct != null ? Number(p.activationPct) - roiPct : null;
                    const trailingDistanceToArmPct = rawArmDistance != null && rawArmDistance > 0
                      ? rawArmDistance
                      : existing.trailingDistanceToArmPct;
                    ...
                    const nextSellOps = {
                      ...
                      roiPct,
                      hardStopLossPct,
                      stopLossDistancePct: computeStopLossDistancePct(roiPct, hardStopLossPct) ?? existing.stopLossDistancePct,
                      trailingStopDistancePct,
                      trailingDistanceToArmPct,
                      trailingArmed: true,
                    };

                    tokenRow.sellOps = nextSellOps;
                    tokenRow.sellOpsLine = buildSellOpsLine({
                      recommendation: nextSellOps.recommendation,
                      strategyName: nextSellOps.strategyName,
                      qualifyWorst: nextSellOps.qualifyWorst,
                      qualifyFailed: nextSellOps.qualifyFailed,
                      gateFail: nextSellOps.gateFail,
                      roiPct: nextSellOps.roiPct,
                      stopLossDistancePct: nextSellOps.stopLossDistancePct,
                      trailingStopDistancePct: nextSellOps.trailingStopDistancePct,
                      trailingDistanceToArmPct: nextSellOps.trailingDistanceToArmPct,
                      trailingArmed: nextSellOps.trailingArmed,
                    });
                  }
                ...
                if (msg.type === 'sellOps:trailingStop:trigger' || msg.type === 'sellOps:stopLoss:trigger') {
                  const p = msg.payload || null;
                  const alias = p?.walletAlias || w.alias;
                  const mint = p?.mint || null;
                  if (!mint) return;
                  ...
                  const symbol =
                    state?.[alias]?.tokens?.find((t) => t?.mint === mint)?.symbol ||
                    (mint ? mint.slice(0, 4) : 'mint');
                  const reason = msg.type === 'sellOps:stopLoss:trigger' ? 'stop-loss' : 'trailing-stop';
                  const line = `SellOps ${reason} (${alias}) ${symbol}`;

                  pushServiceAlert(serviceAlerts, 'warn', line);
                  if (state?.[alias]) {
                    pushRecentEvent(state[alias], line, hudStore);
                  }
                  ...
                  if (tokenRow) {
                    ...
                    const nextSellOps = {
                      ...
                      stopLossDistancePct: computeStopLossDistancePct(roiPct, hardStopLossPct) ?? existing.stopLossDistancePct,
                      trailingStopDistancePct,
                      trailingArmed: msg.type === 'sellOps:trailingStop:trigger' ? true : existing.trailingArmed,
                    };
