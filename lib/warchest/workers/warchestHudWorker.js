...
function writeHudRefreshRequest(targetPath, reason) {
  ...
  try {
    ...
    // Best-effort only; HUD can still render from the last snapshot.
  }
}
...
async function main() {
  ...
  const refreshPath = resolvePath(null, DEFAULT_HUD_REFRESH_PATH);
  writeHudRefreshRequest(refreshPath, "hud-startup");
