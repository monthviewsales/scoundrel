...
const { createHubEventFollower, DEFAULT_EVENT_PATH, DEFAULT_STATUS_PATH } = require('../events');
const { applyHubEventToState } = require('../hudEvents');
...
function parseArgs(argv) {
  ...
  return { wallets, mode, hubEventsPath, hubStatusPath, followHub };
}
...
async function main() {
  const { wallets, mode, hubEventsPath: argEventsPath, hubStatusPath: argStatusPath, followHub: argFollowHub } =
    parseArgs(process.argv);

  const hubEventsPath =
    argEventsPath || process.env.WARCHEST_HUB_EVENTS_PATH || process.env.TX_MONITOR_EVENT_PATH || DEFAULT_EVENT_PATH;
  const hubStatusPath = argStatusPath || process.env.WARCHEST_HUB_STATUS_PATH || DEFAULT_STATUS_PATH;
  const followHub =
    argFollowHub || process.env.HUD_FOLLOW_HUB === '1' || String(process.env.HUD_FOLLOW_HUB).toLowerCase() === 'true';
  ...
  const externalClosers = [];
  if (followHub) {
    const follower = createHubEventFollower({ eventPath: hubEventsPath, statusPath: hubStatusPath });
    follower.onEvent((ev) => {
      applyHubEventToState(state, ev);
    });
    follower.onStatus((snapshot) => {
      if (!snapshot || !snapshot.health) return;

      logger.info('[HUD] Received hub health snapshot.');

      // When following the hub, use the reported ws.slot to keep the shared
      // chain state in sync so the HUD header shows a live slot/WS status.
      const ws = snapshot.health.ws;
      if (ws && typeof ws.slot === 'number') {
        updateFromSlotEvent({ slot: ws.slot });
      }
    });
    externalClosers.push(() => follower.close());
  }
  ...
  function shutdown() {
    Promise.resolve()
      .then(() => close())
      .then(() => {
        externalClosers.forEach((fn) => {
          try {
            fn();
          } catch (err) {
            const msg = err && err.message ? err.message : err;
            logger.warn(`[HUD] Error during follower shutdown: ${msg}`);
          }
        });
      })
      .catch((err) => {
