 ...
 * @param {Function} [options.onProgress] - Optional callback invoked for `{type:'progress'}` worker messages.
...
function forkWorkerWithPayload(workerPath, options) {
  const { payload, timeoutMs, env, requestId: explicitRequestId, lockTag, lockDir, onProgress } = options || {};
