...
/**
 * Safely serialize payloads for logging/metrics.
 *
 * @param {any} payload
 * @returns {any}
 */
...
function truncateTargetNotes(payload, maxLen = 200) {
  if (!payload || typeof payload !== 'object') return payload;
  if (Array.isArray(payload)) return payload;
  const target = payload.target;
  if (!target || typeof target !== 'object') return payload;
  const notes = target.notes;
  if (typeof notes !== 'string' || notes.length <= maxLen) return payload;

  return {
    ...payload,
    target: {
      ...target,
      notes: `${notes.slice(0, maxLen)}...`,
      notesTruncated: true,
    },
  };
}
...
function createLifecycleLogger(workerName, loggerInstance, metricsReporter) {
  const baseLogger = loggerInstance || logger;
  const workerLogger = typeof logger.worker === 'function' ? logger.worker() : logger;
  const loggers = [];
  if (loggerInstance) loggers.push(loggerInstance);
  const hudOrInk =
    process.env.SC_HUD_MODE === '1' ||
    process.env.WARCHEST_HUD === '1' ||
    process.env.SC_INK_MODE === '1' ||
    (Array.isArray(process.argv)
      && (process.argv.includes('--hud') || process.argv.includes('--ink')));
  if (workerLogger && !loggers.includes(workerLogger) && (!loggerInstance || !hudOrInk)) {
    // In HUD/Ink mode, keep worker lifecycle logs away from stdout when a scoped logger is provided.
    loggers.push(workerLogger);
  }
  const name = workerName || 'worker';
  const txMonitorQuiet = name === 'txMonitor' && !isTxMonitorVerbose();
  const metricsLogger = baseLogger && typeof baseLogger.metrics === 'function'
    ? baseLogger.metrics()
    : baseLogger;
  const defaultMetricsReporter = metricsLogger && typeof metricsLogger.debug === 'function'
    ? (event) => {
        const serialized = safeSerializePayload(event);
        metricsLogger.debug(JSON.stringify(serialized));
      }
    : null;
  const metrics = typeof metricsReporter === 'function' ? metricsReporter : defaultMetricsReporter;
  ...
  function emit(level, event, requestId, extra) {
    ...
    if (loggers.length) {
      ...
      const line = `[${name}] ${event} ${JSON.stringify(serialized)}`;
      for (const log of loggers) {
        if (log && typeof log[actualLevel] === 'function') {
          log[actualLevel](line);
        }
      }
    }
  ...
  return {
    start(requestId, payload) {
      const bodyRaw = txMonitorQuiet ? pickTxMonitorBrief(payload) : safeSerializePayload(payload);
      const body = truncateTargetNotes(bodyRaw);
 ...
 * @param {boolean} [opts.inkMode] - When true, set `SC_INK_MODE=1` for the worker.
 * @param {boolean} [opts.hudMode] - When true, set `SC_HUD_MODE=1` for the worker.
...
function buildWorkerEnv(opts) {
  const { rpcEndpoint, dataEndpoint, walletIds, bootyBoxPath, inkMode, hudMode, extraEnv } = opts || {};
  const env = { ...extraEnv };
...
function createPidTag(tag, dir) {
  ...
  if (fs.existsSync(lockPath)) {
    ...
    const existingPid = existing && Number.isFinite(Number(existing.pid))
      ? Number(existing.pid)
      : null;
    ...
    if (!existingPid || !isProcessAlive(existingPid)) {
      try {
        ...
        // If cleanup fails, fall through to the error path below.
      }
    } else {
      throw new Error(`PID tag already exists for ${tag}: ${JSON.stringify(existing)}`);
    }
  }
 ...
 *   The returned promise also includes `.stop(reason, options)` and `.child` for long-running workers.
 */
function forkWorkerWithPayload(workerPath, options) {
  ...
  function requestStop(reason, options = {}) {
    if (stopRequested) return;
    stopRequested = true;
    const graceMs = Number.isFinite(Number(options.graceMs)) ? Number(options.graceMs) : 5000;
    const signal = options.signal || 'SIGTERM';
    const force = options.force === true;

    if (typeof child.send === 'function' && child.connected) {
      try {
        child.send({ type: 'stop', payload: { reason: reason || null } });
      } catch (err) {
  ...
  const workerPromise = new Promise((resolve, reject) => {
    ...
    function onMessage(msg) {
      ...
      const isProgressLike = msg.type === 'progress' || (
        typeof msg.type === 'string' && (
          msg.type.startsWith('sellOps:') ||
          msg.type.startsWith('buyOps:') ||
          msg.type.startsWith('targetOps:')
        )
      );
      if (isProgressLike) {
  ...
  workerPromise.stop = requestStop;
  workerPromise.child = child;
  workerPromise.pid = child.pid;
