...
function pushServiceAlert(alerts, level, message, meta) {
  ...
  const cleaned = String(message || '')
    ...
    .trim();
  const maxLen = 240;
  const clipped = cleaned.length > maxLen
 ...
 * @param {string[]} argv
 * @param {object} [logger]
 * @returns {{wallets: Array<{alias: string, pubkey: string, color: string|null}>, mode: string, hudStatePath: string|null}}
 */
function parseArgs(argv, logger) {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === '--wallet') {
      ...
      mode = 'hud';
    } else if (arg === '--hud-state' || arg === '--hud-status' || arg === '--hub-status') {
  ...
  return { wallets, mode, hudStatePath };
}
