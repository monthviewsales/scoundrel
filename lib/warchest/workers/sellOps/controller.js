...
const { resolveAvgCostUsd, getTrailingStopConfig } = require('./stopLogicLoader');
...
function summarizeStrategyLabel(input) {
  ...
  return list.length > max ? `multi(${head}+${list.length - max})` : `multi(${head})`;
}
...
function computeWeightedAvgUsdFromTrades(trades) {
  ...
  for (const trade of trades) {
    const tokenAmountRaw = Number(trade?.token_amount);
    const tokenAmount = Number.isFinite(tokenAmountRaw) ? Math.abs(tokenAmountRaw) : null;
 ...
 */
async function backfillEntryPriceUsd({ db, dataClient, positions, walletAlias, log }) {
  if (!db || typeof db.prepare !== 'function') {
    log?.warn?.('[sellOps] entry_price_usd backfill skipped: db handle missing prepare()');
  ...
  for (const pos of missing) {
    ...
    if (pnlPrice) {
      stmtUpdate.run({ position_id: positionId, entry_price_usd: pnlPrice, last_updated_at: now });
    ...
    if (tradePrice) {
      stmtUpdate.run({ position_id: positionId, entry_price_usd: tradePrice, last_updated_at: now });
  ...
  if (pendingByMint.size > 0 && dataClient && typeof dataClient.getMultipleTokenPrices === 'function') {
    try {
      const priceResp = await dataClient.getMultipleTokenPrices(Array.from(pendingByMint.keys()));
      for (const [mint, list] of pendingByMint.entries()) {
        ...
        for (const pos of list) {
          ...
          stmtUpdate.run({ position_id: pos.positionId, entry_price_usd: priceUsd, last_updated_at: now });
      ...
      log?.warn?.(
        `[sellOps] entry_price_usd backfill price fetch failed wallet=${walletAlias}: ${err?.message || err}`
      );
    }
  }
...
function createSellOpsController(payload, tools = {}, log = sellOpsLogger) {
  ...
  const symbolByTradeUuid = new Map(); // tradeUuid -> token symbol
        ...
        try {
          ...
          for (const pos of previousOpenPositions.values()) {
            ...
            if (!costUsd || !Number.isFinite(Number(costUsd)) || Number(costUsd) <= 0) {
              missingCostSkips += 1;
              const lastAlertTs = missingCostAlertByTradeUuid.get(tradeUuid) || 0;
              if (now - lastAlertTs >= MISSING_COST_ALERT_DEBOUNCE_MS) {
                const mintLabel = symbolByTradeUuid.get(tradeUuid) || pos?.symbol || (mint ? mint.slice(0, 4) : 'coin');
                emitToParent('sellOps:alert', {
                  ts: now,
                  walletAlias: wallet.alias,
                  tradeUuid,
                  mint,
        ...
        if (!rows.length) {
          ...
          for (const summary of summaries) {
            ...
            const observeOnlyDefault =
              workerEnv.SELL_OPS_OBSERVE_ONLY === 'true' || workerEnv.NODE_ENV === 'test';
            const enginePayload = {
              ...(payload || {}),
              observeOnly: payload?.observeOnly != null ? payload.observeOnly : observeOnlyDefault,
            };
