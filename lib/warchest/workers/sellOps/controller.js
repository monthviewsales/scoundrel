...
const { getStrategyDocs } = require("../strategyRegistry");
const {
  evaluatePosition,
...
const { createNoopLogger, normalizeScopedLogger } = require("../opsUtils");
const {
  resolveAvgCostUsd,
...
function fmtUsd(n) {
  ...
  // Large numbers (keep existing compact formatting)
  if (abs >= 1_000_000) return `${sign}$${(abs / 1_000_000).toFixed(2)}m`;
  if (abs >= 1_000) return `${sign}$${(abs / 1_000).toFixed(1)}k`;

  // Typical dollars and cents
  if (abs >= 1) return `${sign}$${abs.toFixed(2)}`;

  // Small values: show more precision so token prices/costs aren't always $0.00
  if (abs >= 0.01) return `${sign}$${abs.toFixed(4)}`;
  if (abs >= 0.0001) return `${sign}$${abs.toFixed(6)}`;
  if (abs >= 0.000001) return `${sign}$${abs.toFixed(8)}`;

  // Extremely small: scientific notation (still with $ prefix)
...
function pickTopGateFailures(evaluation, limit = 3) {
  ...
  // Use gate result shape from decisionEngine: .pass === false is a failure
  const failed = results.filter((r) => r && r.pass === false);
  if (!failed.length) return [];
  return failed.slice(0, limit).map((r) => {
    const id = r.id || "gate?";
    // Defensive: prefer severityOnFail, fallback to r.severityOnFail if present
    const sev = r.severityOnFail || r.severityOnFail || "fail";
    const reason =
...
function buildFriendlyEvaluationSummary(snapshot) {
  ...
  const riskTag =
    worst && worst !== "none" ? String(worst).toUpperCase() : "OK";
  const headline =
    `${sym} | ${fmtPct(roiPct)} uPnL=${fmtUsd(unrealUsd)} ` +
    `| rec=${String(rec).toUpperCase()} | risk=${riskTag}`;
  ...
  // Explicitly call out degrade/exit posture even if no gate failures
  if (worst && worst !== "none" && !gateFails.length) {
    whyParts.push(`qualify=${worst}`);
  }
 ...
 */
async function runSwapWithMonitor(payload, workerEnv, log) {
 ...
 */
function createSellOpsController(payload, tools = {}, logOverride) {
  const wallet = normalizeWallet(payload.wallet || payload);
  const pollIntervalMs = payload.pollIntervalMs || DEFAULT_POLL_MS;

  const baseLogger = logOverride || tools.logger || logger || createNoopLogger();
  const log = normalizeScopedLogger(baseLogger, "sellOps");
  ...
  // INFO-level fast-loop evaluation summary per tradeUuid (throttled)
  const lastFastEvalLogByTradeUuid = new Map(); // tradeUuid -> last INFO eval log ts
  const FAST_EVAL_LOG_INTERVAL_MS = 15_000;
      ...
      async function fastTick() {
        ...
        try {
          ...
          for (const summary of previousOpenPositions.values()) {
            ...
            // DEBUG-LEVEL LOGGING OF GATES DIAGNOSTICS
            if (evalResult?.evaluation?.gatesDebug?.length) {
              log.debug(
                `[sellOps][gates] wallet=${wallet.alias} trade_uuid=${tradeUuid} mint=${mint} ` +
                  `strategy=${
                    evalResult.evaluation?.strategy?.name || "n/a"
                  } ` +
                  JSON.stringify(evalResult.evaluation.gatesDebug)
              );
            }
            ...
            const roiPctEvalRaw = evaluation?.derived?.roiUnrealizedPct;
            const roiPctEval = Number.isFinite(Number(roiPctEvalRaw))
              ? Number(roiPctEvalRaw)
              : null;
            const roiPctLive =
              Number.isFinite(Number(costUsd)) &&
              Number(costUsd) > 0 &&
              Number.isFinite(Number(priceUsdNow))
                ? ((Number(priceUsdNow) - Number(costUsd)) / Number(costUsd)) *
                  100
                : null;
            const roiPctNow = Number.isFinite(Number(roiPctLive))
              ? Number(roiPctLive)
              : roiPctEval;
            if (
              roiPctEval == null &&
              Number.isFinite(Number(roiPctNow)) &&
              evaluation &&
              typeof evaluation === "object"
            ) {
              if (
                !evaluation.derived ||
                typeof evaluation.derived !== "object"
              ) {
                evaluation.derived = {};
              }
              evaluation.derived.roiUnrealizedPct = roiPctNow;
            }
            ...
            // TODO(timeStops): Implement HYBRID manage.timeStops (no_followthrough, momentum_decay)
            // in this fast loop using persisted counters in snapshot.riskControls.timeStops.
            log.info(
              `[sellOps] stop-config wallet=${wallet.alias} trade_uuid=${tradeUuid} strategy=${stratName} ` +
                `activationPct=${fmtPct(
                  cfg.activationPct,
                  2
                )} trailPct=${fmtPct(cfg.trailPct, 2)} ` +
                `hardStop=${fmtPct(cfg.hardStopLossPct, 2)} pollMs=${
                  cfg.pollMs
                } ` +
                `breachConfirmations=${cfg.breachConfirmations} debounceMs=${cfg.actionDebounceMs}`
            );
            ...
            if (!decisionReason && Number.isFinite(Number(roiPctNow))) {
              if (
                ...
                // INFO log for trailing armed
                log.info(
                  `[sellOps] trailing armed wallet=${
                    wallet.alias
                  } trade_uuid=${tradeUuid} symbol=${
                    inferredSym || "n/a"
                  } mint=${mint} ` +
                    `roi=${fmtPct(roiPctNow, 2)} activation=${fmtPct(
                      state.activationPct,
                      2
                    )} trail=${fmtPct(state.trailPct, 2)} ` +
                    `high=${fmtUsd(state.highWaterUsd)} stop=${fmtUsd(
                      state.stopUsd
                    )} price=${fmtUsd(priceUsdNow)}`
                );
              }
              ...
              if (state.active) {
                ...
                if (eligible && debounceOk) {
                  ...
                  // INFO log for trailing trigger
                  log.info(
                    `[sellOps] trailing TRIGGER wallet=${
                      wallet.alias
                    } trade_uuid=${tradeUuid} symbol=${
                      inferredSym || "n/a"
                    } mint=${mint} ` +
                      `roi=${fmtPct(roiPctNow, 2)} trail=${fmtPct(
                        state.trailPct,
                        2
                      )} ` +
                      `high=${fmtUsd(state.highWaterUsd)} stop=${fmtUsd(
                      ...
                      `breachCount=${state.breachCount}/${breachConfirmations}`
                  );
                }
              }
            }
            ...
            // INFO: periodic per-position state line so we can see ROI and stop/trailing distances.
            try {
              const lastTs = lastFastEvalLogByTradeUuid.get(tradeUuid) || 0;
              if (now - lastTs >= FAST_EVAL_LOG_INTERVAL_MS) {
                lastFastEvalLogByTradeUuid.set(tradeUuid, now);

                const uPnlUsd =
                  evaluation?.pnl?.unrealized_usd ??
                  evaluation?.pnl?.unrealizedUsd ??
                  null;
                const trailingActive =
                  snapshot.riskControls?.trailing?.active === true;
                const stopUsd = snapshot.riskControls?.trailing?.stopUsd;
                const highUsd = snapshot.riskControls?.trailing?.highWaterUsd;
                const priceUsd = snapshot.riskControls?.trailing?.priceUsd;

                // Enhance observability: show cost basis and ROI/cost source for live testing
                const costBasisUsd = Number.isFinite(Number(costUsd))
                  ? Number(costUsd)
                  : null;
                const costSrc =
                  Number.isFinite(Number(evaluation?.pnl?.avg_cost_usd)) &&
                  Number(evaluation.pnl.avg_cost_usd) > 0
                    ? "pnl"
                    : Number.isFinite(Number(summary?.entryPriceUsd)) &&
                      Number(summary.entryPriceUsd) > 0
                    ? "entry"
                    : "missing";
                const roiSrc = Number.isFinite(Number(roiPctLive))
                  ? "live"
                  : "derived";
                log.info(
                  `[sellOps] fast-eval wallet=${
                    wallet.alias
                  } trade_uuid=${tradeUuid} symbol=${
                    inferredSym || "n/a"
                  } mint=${mint} ` +
                    `roi=${fmtPct(roiPctNow, 2)}(src=${roiSrc}) cost=${fmtUsd(
                      costBasisUsd
                    )}(src=${costSrc}) uPnL=${fmtUsd(uPnlUsd)} ` +
                    `trailing=${trailingActive ? "ON" : "off"} stop=${fmtUsd(
                      stopUsd
                    )} high=${fmtUsd(highUsd)} price=${fmtUsd(priceUsd)} ` +
                    `hardStop=-${hsAbs != null ? fmtPct(hsAbs, 2) : "n/a"}`
                );
              }
            } catch (_) {
      ...
      try {
        ...
        if (!rows.length) {
          ...
          for (const summary of summaries) {
            ...
            if (evalResult?.evaluation?.gatesDebug?.length) {
              log.debug(
                `[sellOps][gates] wallet=${wallet.alias} trade_uuid=${
            ...
            if (
              ...
              log.info(
                `[sellOps] position missing strategyName; persisting inferred strategy position_id=${
                  summary.positionId || "n/a"
                } trade_uuid=${summary.tradeUuid || "n/a"} mint=${
                  summary.mint || "n/a"
                } inferred=${inferredStrategyName}`
              );
              try {
                ...
                if (result && result.updated) {
                  ...
                  log.info(
                    `[sellOps] position strategyName updated position_id=${
                      summary.positionId || "n/a"
                    } trade_uuid=${
                      summary.tradeUuid || "n/a"
                    } strategy=${inferredStrategyName}`
                  );
                }
              } catch (err) {
                log.warn(
