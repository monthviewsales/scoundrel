...
/**
 * Convert a value to a finite number, treating null/undefined/empty as missing.
 * @param {any} value
 * @returns {number|null}
 */
...
function evalGate(evaluation, gate) {
  ...
  if (type === "warnings_forbidden_absent") {
    ...
    if (v !== params.value) {
      pass = false;
      reasons.push(
        `expected:${params.path}==${String(params.value)} got:${String(v)}`
      );
    }
  } else if (type === "number_lte") {
    const v = toFiniteNumber(getPath(evaluation, params.path));
    if (v == null || v > Number(params.max)) {
      pass = false;
      reasons.push(
        `expected:${params.path}<=${params.max} got:${v == null ? "n/a" : v}`
      );
    }
  } else if (type === "number_gte") {
    const v = toFiniteNumber(getPath(evaluation, params.path));
    if (v == null || v < Number(params.min)) {
      pass = false;
      reasons.push(
        `expected:${params.path}>=${params.min} got:${v == null ? "n/a" : v}`
      );
    }
  } else if (type === "pnl_lte") {
    const roi = toFiniteNumber(getPath(evaluation, "derived.roiUnrealizedPct"));
    const maxPnlPct = Number(params.maxPnlPct);
    if (roi == null || roi > maxPnlPct) {
      pass = false;
      reasons.push(
        `expected:roiUnrealizedPct<=${maxPnlPct} got:${
          roi == null ? "n/a" : roi
        }`
      );
    }
...
function evalQualify(strategy, evaluation) {
  ...
  for (const f of failed) {
    ...
    if ((SEVERITY_RANK[sev] ?? 1) > (SEVERITY_RANK[worstSeverity] ?? 0))
      worstSeverity = sev;
  }
 ...
 * @param {any} evaluation
 * @returns {{ strategy: object, source: 'position'|'inferred', qualify: ReturnType<typeof evalQualify> }}
 */
function chooseStrategy(position, docs, evaluation) {
  const name = position?.strategyName
    ? String(position.strategyName).toUpperCase()
    : null;
  if (name) {
    if (name.includes("FLASH")) {
      return {
        strategy: docs.flash,
        source: "position",
        qualify: evalQualify(docs.flash, evaluation),
      };
    }
    if (name.includes("CAMPAIGN")) {
      return {
        strategy: docs.campaign,
        source: "position",
        qualify: evalQualify(docs.campaign, evaluation),
      };
    }
    if (name.includes("HYBRID")) {
      return {
        strategy: docs.hybrid,
        source: "position",
        qualify: evalQualify(docs.hybrid, evaluation),
      };
    }
  }
  ...
  if (!flashQ.failed.length)
    return { strategy: docs.flash, source: "inferred", qualify: flashQ };

  const hybridQ = evalQualify(docs.hybrid, evaluation);
  if (!hybridQ.failed.length)
    return { strategy: docs.hybrid, source: "inferred", qualify: hybridQ };
