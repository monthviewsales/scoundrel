...
function evalGate(evaluation, gate) {
  ...
  if (type === 'warnings_forbidden_absent') {
    ...
    if (v == null || v > Number(params.max)) {
      pass = false;
      reasons.push(`expected:${params.path}<=${params.max} got:${v == null ? 'n/a' : v}`);
    }
  } else if (type === 'number_gte') {
    const v = toFiniteNumber(getPath(evaluation, params.path));
    if (v == null || v < Number(params.min)) {
      pass = false;
      reasons.push(`expected:${params.path}>=${params.min} got:${v == null ? 'n/a' : v}`);
    }
    ...
    if (roi == null || roi > maxPnlPct) {
      pass = false;
      reasons.push(`expected:roiUnrealizedPct<=${maxPnlPct} got:${roi == null ? 'n/a' : roi}`);
    }
  } else {
...
function chooseStrategy(position, docs, evaluation) {
  ...
  if (name) {
    if (name.includes('FLASH')) {
      return { strategy: docs.flash, source: 'position', qualify: evalQualify(docs.flash, evaluation) };
    }
    if (name.includes('CAMPAIGN')) {
      return { strategy: docs.campaign, source: 'position', qualify: evalQualify(docs.campaign, evaluation) };
    }
    if (name.includes('HYBRID')) {
      return { strategy: docs.hybrid, source: 'position', qualify: evalQualify(docs.hybrid, evaluation) };
    }
  }
