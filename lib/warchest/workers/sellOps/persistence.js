 ...
 */
function persistSellOpsEvaluation({ bootyBox, summary, snapshot, hudPayload, logger, walletAlias }) {
  if (!bootyBox || typeof bootyBox.insertSellOpsEvaluation !== 'function') return;
  ...
  try {
    ...
    logger.warn(
      `[sellOps] persist tick failed wallet=${walletAlias} trade_uuid=${summary.tradeUuid || 'n/a'} mint=${summary.mint || 'n/a'}: ${
        err?.message || err
      }`
    );
  }
