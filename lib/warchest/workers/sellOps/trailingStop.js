...
function getTrailingStopConfig(strategy) {
  ...
  const manageTrailing = (manage && (manage.trailingStop || manage.trailing_stop || manage.trailing)) || {};
  const defaultsTrailing = (defaults && (defaults.trailingStop || defaults.trailing_stop || defaults.trailing)) || {};
  ...
  const activationPct =
    readNumeric(manageTrailing.activationPct, manageTrailing.minProfitToEnablePct, defaultsTrailing.activationPct, defaultsTrailing.minProfitToEnablePct) ??
    DEFAULT_TRAIL_ACTIVATION_PCT;
  const trailPct = readNumeric(manageTrailing.trailPct, defaultsTrailing.trailPct) ?? DEFAULT_TRAIL_PCT;
  const pollMs = readNumeric(manageTrailing.pollMs, defaultsTrailing.pollMs) ?? DEFAULT_TRAIL_POLL_MS;
  const breachConfirmations =
    readNumeric(manageTrailing.breachConfirmations, defaultsTrailing.breachConfirmations) ?? DEFAULT_BREACH_CONFIRMATIONS;
  const actionDebounceMs = readNumeric(manageTrailing.actionDebounceMs, defaultsTrailing.actionDebounceMs) ?? DEFAULT_ACTION_DEBOUNCE_MS;
  const hardStopLossPct =
    readNumeric(manageTrailing.hardStopLossPct, defaultsTrailing.hardStopLossPct, defaults.hardStopLossPct) ?? DEFAULT_HARD_STOP_LOSS_PCT;
