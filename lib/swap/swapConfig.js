...
const logger = require('../logger').child({ scope: 'swapConfig' });
...
const DEFAULT_CONFIG = {
  ...
  swapDiscountKey: "",         // Legacy swap key (if needed); not used for swap auth
...
async function loadConfig() {
  if (process.env.SWAP_CONFIG_JSON) {
    try {
      const override = JSON.parse(process.env.SWAP_CONFIG_JSON);
      return { ...DEFAULT_CONFIG, ...(override || {}) };
    } catch (err) {
      logger.warn('[swapConfig] failed to parse SWAP_CONFIG_JSON override', {
  ...
  logger.debug('[swapConfig] load', { configPath });
  ...
  if ((cfg.swapApiKey === undefined || cfg.swapApiKey === '') && cfg.swapAPIKey !== undefined) {
    cfg.swapApiKey = cfg.swapAPIKey;
    updated = true;
  }
  if (cfg.swapAPIKey !== undefined) {
    if (cfg.swapDiscountKey === undefined || cfg.swapDiscountKey === '') {
      cfg.swapDiscountKey = cfg.swapAPIKey;
    }
    logger.warn('[swapConfig] removing legacy swapAPIKey', { configPath });
...
async function saveConfig(cfg) {
  const configPath = getConfigPath();
  await fs.ensureDir(path.dirname(configPath), { mode: 0o700 });
  if (cfg && cfg.swapAPIKey !== undefined) {
    if (cfg.swapDiscountKey === undefined) {
      cfg.swapDiscountKey = cfg.swapAPIKey;
    }
    logger.warn('[swapConfig] removing legacy swapAPIKey on save', { configPath });
    delete cfg.swapAPIKey;
  }
