...
function normalizeRaptorPriorityFee(priorityFee) {
  if (priorityFee == null) return null;
  const raw = String(priorityFee).trim();
  if (!raw) return null;
...
async function quoteAndSwap({
  ...
  const normalizedPriorityFee = normalizeRaptorPriorityFee(priorityFee);
  const body = {
    userPublicKey,
    inputMint,
    outputMint,
  ...
  if (normalizedPriorityFee != null) body.priorityFee = String(normalizedPriorityFee);
...
/**
 * Request swap instructions from Raptor (quote must be supplied).
 *
 * @param {Object} args
 * @param {string} args.baseUrl
 * @param {string} args.apiKey
 * @param {string} args.userPublicKey
 * @param {object} args.quoteResponse
 * @param {string|null} [args.txVersion]
 * @param {string|null} [args.priorityFee]
 * @param {boolean} [args.wrapUnwrapSol]
 * @returns {Promise<object>}
 */
async function buildSwapInstructions({
  ...
  if (!baseUrl) throw new Error('swapRaptor.buildSwapInstructions: missing baseUrl');
  if (!apiKey) throw new Error('swapRaptor.buildSwapInstructions: missing apiKey');
  if (!quoteResponse || typeof quoteResponse !== 'object') {
    throw new Error('swapRaptor.buildSwapInstructions: missing quoteResponse');
  }

  const url = resolveEndpoint(baseUrl, '/swap-instructions');
  const normalizedPriorityFee = normalizeRaptorPriorityFee(priorityFee);
  const body = {
    userPublicKey,
    quoteResponse,
    wrapUnwrapSol: Boolean(wrapUnwrapSol),
  };
  if (txVersion) body.txVersion = String(txVersion);
  if (normalizedPriorityFee != null) body.priorityFee = String(normalizedPriorityFee);

  if (typeof raptorDebugLogger === 'function') {
    raptorDebugLogger('swap.raptor.swapInstructions.request', {
      url: url.toString(),
      body,
    });
  }
  ...
  if (!res.ok) {
    const snippet = text ? text.slice(0, 500) : '';
    const err = new Error(`swapRaptor.buildSwapInstructions: HTTP ${res.status} ${res.statusText} (${ms}ms) ${snippet}`);
    err.status = res.status;
    err.body = snippet;
    throw err;
  }

  const json = parseJsonResponse({ text, label: 'swapRaptor.buildSwapInstructions', ms });
  if (typeof raptorDebugLogger === 'function') {
    raptorDebugLogger('swap.raptor.swapInstructions.response', json);
  }
  return json;
}
...
async function executeSwapRaptor({
  ...
  if (debugLogging) {
    logger.debug('swap.raptor.transaction.received', {
      swapTransaction: payload.swapTransaction,
      swapTransactionLength: payload.swapTransaction.length,
    });
  }
  ...
  if (debugLogging) {
    logger.debug('swap.raptor.transaction.signed', {
      signedTransaction: signedWireB64,
      signedTransactionLength: signedWireB64.length,
    });
  }
