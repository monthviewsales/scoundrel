...
async function buildSwapTx({
  ...
  const shouldRetry = (err) => {
    if (!err) return false;
    if (err.status && Number(err.status) >= 500) return true;
    if (err.status && Number(err.status) === 429) return true;
    if (/fetch failed/i.test(err.message || '')) return true;
    if (/timeout/i.test(err.message || '')) return true;
    const code = err.code || err?.cause?.code;
    return code && /ETIMEDOUT|ECONNRESET|ECONNREFUSED|EAI_AGAIN|UND_ERR_CONNECT_TIMEOUT/i.test(String(code));
  };
  ...
  let lastError = null;
  for (let attempt = 0; attempt < MAX_SWAP_API_ATTEMPTS; attempt += 1) {
    ...
    try {
      ...
      lastError = err;
      const retryable = shouldRetry(err) && attempt < MAX_SWAP_API_ATTEMPTS - 1;
      if (!retryable) throw err;
      const delayMs = computeDelay(attempt);
      logger.warn('swap.build.retry', {
        attempt: attempt + 1,
        delayMs,
  ...
  throw lastError || new Error('swapV3.buildSwapTx: failed after retries');
}
...
async function executeSwapV3({
  ...
  if (priceImpactPercent != null && priceImpactPercent > 2) {
    (inkMode ? logger.debug : logger.warn)('swap.priceImpact.high', {
      side,
      fromMint,
      toMint,
      fromAmount: String(fromAmount),
  ...
  if (maxPriceImpactPercent != null && priceImpactPercent != null && priceImpactPercent > maxPriceImpactPercent) {
    const e = new Error(`swapV3: price impact ${priceImpactPercent}% exceeds max ${maxPriceImpactPercent}%`);
  ...
  if (preflight) {
    if (typeof rpc.simulateTransaction === 'function') {
      ...
      if (typeof onProgress === 'function') {
        onProgress('swap.preflight.start', {
          side,
          fromMint,
          toMint,
          fromAmount: String(fromAmount),
      ...
      if (typeof onProgress === 'function') {
        onProgress('swap.preflight.unavailable', { reason: 'rpc.simulateTransaction not present' });
      }
    }
  }
