...
const READ_ONLY_SETTINGS = [
  ...
  },
  {
    key: 'swapAPIKey',
    label: 'Swap API key',
    description: 'SolanaTracker swap API key (managed outside the config editor).',
    redact: true,
  },
];
...
function createSwapConfigComponents(ink, TextInput) {
  ...
  function EditValueScreen({ def, inputValue, onChange, onSubmit, onCancel, error, busy }) {
    useInput((input, key) => {
