...
const { upsertWalletSecret, deleteWalletSecret } = require('../wallets/keystore');
...
function createCommandTuiComponents(ink, TextInput) {
  ...
  function AddWalletPanel({ onAdded, onCancel, bus }) {
    ...
    const [keySource, setKeySource] = React.useState('keychain');
    ...
    useInput((input, key) => {
      ...
      if (stage === 'key-source') {
        if (input === 'k') setKeySource('keychain');
        if (input === 'e') setKeySource('env');
        if (input === 'n') setKeySource('none');
        if (key.return) {
          if (keySource === 'env') {
            setStage('env');
    ...
    async function handleSubmit() {
      ...
      const signing = isSigning && keySource !== 'none';
      if (signing && keySource === 'env' && !envVar.trim()) {
        setStatus('Env var name is required.');
        return;
      }
      if (signing && keySource === 'keychain' && !secret.trim()) {
      ...
      try {
        ...
        const wallet = await walletRegistry.addWallet({
          ...
          keySource: signing ? keySource : 'none',
          keyRef: signing && keySource === 'env' ? envVar.trim() : null,
          usageType: 'funding',
        ...
        if (signing && keySource === 'keychain') {
          try {
            ...
            const stored = await upsertWalletSecret({ walletId, secret: secret.trim() });
            ...
            await deleteWalletSecret({ keyRef: wallet.keyRef, walletId: wallet.walletId });
    ...
    return h(
      ...
      stage === 'key-source'
        ? h(
            Box,
            { marginTop: 1, flexDirection: 'column' },
            h(Text, null, `Key source: ${keySource}`),
            h(Text, { dimColor: true }, 'k=keychain, e=env var, n=none • Enter to continue.')
          )
        : null,
      stage === 'env'
        ? h(
            Box,
            { marginTop: 1, flexDirection: 'column' },
            h(Text, null, 'Env var name:'),
            h(TextInput, {
              value: envVar,
              onChange: setEnvVar,
              onSubmit: () => handleSubmit(),
              placeholder: 'WALLET_KEY_ALIAS',
            })
          )
        : null,
      stage === 'secret'
        ? h(
            Box,
            { marginTop: 1, flexDirection: 'column' },
            h(Text, null, 'Paste private key:'),
            h(TextInput, {
              value: secret,
              onChange: setSecret,
              onSubmit: () => handleSubmit(),
              placeholder: 'base58 or id.json array',
            })
          )
        : null,
  ...
  function SwapPanel({ mint, opts = {}, bus, onExit }) {
    ...
    function formatSwapErrorMessage(raw) {
      const cleaned = String(raw || '').replace(/\s+/g, ' ').trim();
      if (!cleaned || cleaned === '[object Object]') return 'Swap failed.';
      const impactMatch = cleaned.match(/price impact\s+([\d.]+)%\s+exceeds\s+max\s+([\d.]+)%/i);
      if (impactMatch) {
        return `Swap blocked: price impact ${impactMatch[1]}% exceeds max ${impactMatch[2]}%. Adjust maxPriceImpact in swap config if needed.`;
      }
      return cleaned;
    }

    function resolveSwapError(err) {
      if (!err) return { message: 'Swap failed.' };
      const errContext = err?.code === 'ERR_UNHANDLED_ERROR' && err?.context
        ? err.context
        : null;
      let root = err;
      if (errContext && typeof errContext === 'object') {
        root = errContext.error || errContext.err || errContext;
      } else if (err && typeof err === 'object' && err.error) {
        root = err.error;
      }

      const fieldSource = Array.isArray(err?.fields)
        ? err.fields
      ...
      if (fieldSource) {
        const fieldSummary = fieldSource
          .map((fieldErr) => {
          ...
          .join('; ');
        if (fieldSummary) {
          return { message: `Invalid swap input: ${fieldSummary}` };
        }
      }

      const message = root?.message || err?.message || String(root || err);
      return { message: formatSwapErrorMessage(message) };
    }
    ...
    async function runSwap(submittedAmount) {
      try {
        ...
        const resolved = resolveSwapError(err);
        const message = resolved.message || 'Swap failed.';
      ...
      return h(
        ...
        details
          ? h(
            Text,
            { dimColor: true },
            `Mint: ${details.mint}${details.symbol ? ` | Symbol: ${details.symbol}` : ''}`
          )
          : null,
  ...
  function TestPanel({ run, bus }) {
    const [step, setStep] = React.useState('running');
    const [status, setStatus] = React.useState('Running checks...');
    const [summary, setSummary] = React.useState(null);
    const [error, setError] = React.useState(null);

    const renderCheckLine = (label, ok, detail) => {
      const statusLabel = ok ? 'OK' : 'MISSING';
    ...
    React.useEffect(() => {
      ...
      (async () => {
        try {
          ...
          if (!mounted) return;
          if (!result) {
            setError('No self-check results returned.');
            setStep('error');
            return;
          }
          setSummary(result);
          setStatus(result.ok ? 'Self-check complete.' : 'Self-check complete (issues found).');
          setStep('done');
        } catch (err) {
          if (!mounted) return;
          setError(err?.message || String(err));
    ...
    if (step === 'running') {
      return h(Spinner, { label: status || 'Running checks' });
    }

    if (step === 'error') {
      return h(Text, { color: 'red' }, error || 'Self-check failed.');
    }

    if (step === 'done') {
      const env = summary?.env || {};
      const coreFiles = Array.isArray(summary?.coreFiles) ? summary.coreFiles : [];
      const swapConfig = summary?.swapConfig || {};
      const db = summary?.db || {};
      const wallets = summary?.wallets || {};
      const walletCountLabel =
        wallets.count == null ? 'unknown' : String(wallets.count);
      const walletDetail = wallets.error
        ? `(${wallets.error})`
        : `(${walletCountLabel})`;
      ...
      return h(
        Box,
        { flexDirection: 'column' },
        h(Text, null, status || 'Self-check complete.'),
        h(Text, { dimColor: true }, `Node: ${env.nodeVersion || 'unknown'}`),
        h(Text, { dimColor: true }, `CWD: ${env.cwd || 'unknown'}`),
        renderCheckLine('OPENAI_API_KEY', env.openaiKey === true),
        renderCheckLine('SOLANATRACKER_API_KEY', env.solanaTrackerKey === true),
        renderCheckLine('xAI_API_KEY', env.xaiKey === true),
        h(Text, { dimColor: true }, 'Core files:'),
        ...coreFiles.map((entry) =>
          renderCheckLine(entry.path, entry.present === true)
        ),
        h(Text, { dimColor: true }, 'Swap config:'),
        h(Text, { dimColor: true }, `Path: ${swapConfig.path || 'unknown'}`),
        renderCheckLine(
          'Config',
          swapConfig.ok === true,
          swapConfig.override ? '(SWAP_CONFIG_JSON override)' : null
        ),
        h(Text, { dimColor: true }, 'Database:'),
        h(Text, { dimColor: true }, `Path: ${db.path || 'unknown'}`),
        renderCheckLine(
          'BootyBox sqlite',
          db.ok === true,
          db.error ? `(${db.error})` : null
        ),
        renderCheckLine('Wallets in DB', wallets.ok === true, walletDetail),
        h(Text, { dimColor: true }, 'Press q to exit.')
      );
    }
  ...
  function FilePrunePanel({ run, bus, onExit }) {
    const [step, setStep] = React.useState('running');
    ...
    React.useEffect(() => {
      ...
      const onProgress = (payload) => {
        ...
        if (event === 'openai-fileprune:found') {
          const total = Number(data.total) || 0;
          const dryRun = data.dryRun === true;
          const current = dryRun ? total : 0;
          setProgressState({
            total,
            current,
            dryRun,
          ...
          const line = total
            ...
            : 'No matching files.';
          setStatus(line);
          if (bus) bus.emit({ text: line });
          return;
        }

        if (event === 'openai-fileprune:delete') {
          const total = Number(data.total) || 0;
          ...
          if (total) {
            setStatus(`Deleting ${current} of ${total} files...`);
          }
        }
      };
      ...
      (async () => {
        try {
          ...
          if (!mounted) return;
          if (!result) {
            setError('File prune failed.');
            setStep('error');
            return;
          }
          setSummary(result);
          if (result.skipped) {
            const reason = result.reason ? `Skipped (${result.reason}).` : 'Skipped.';
            setStatus(reason);
          ...
          if (bus) bus.emit({ text: 'File prune complete.' });
          setStep('done');
        } catch (err) {
          if (!mounted) return;
          setError(err?.message || String(err));
    ...
    if (step === 'running') {
      ...
      const label = total > 0 ? `${current}/${total}` : null;
      const subtitle = progressState.found
        ? `Found ${total} matching files.`
      ...
      return h(
        Box,
        { flexDirection: 'column' },
        h(Spinner, { label: status || 'Pruning files' }),
        h(Text, { dimColor: true }, subtitle),
        total > 0 ? h(ProgressBar, { value: ratio, label, width: 26 }) : null
      );
    }

    if (step === 'done') {
      const matched = summary && summary.matched != null ? summary.matched : null;
      const deleted = summary && summary.deleted != null ? summary.deleted : null;
      const errors = summary && summary.errors != null ? summary.errors : null;
      const detail =
        matched != null
          ? `Matched ${matched}, deleted ${deleted || 0}, errors ${errors || 0}.`
          : null;

      return h(
        Box,
        { flexDirection: 'column' },
        h(Text, null, status || 'Complete.'),
  ...
  function TargetListPanel({ run, bus, onExit }) {
    const [step, setStep] = React.useState('running');
    const [status, setStatus] = React.useState('Fetching target list...');
    const [error, setError] = React.useState(null);
    ...
    React.useEffect(() => {
      ...
      const onProgress = (payload) => {
        ...
        const event = payload.event || null;
        const data = payload.data && typeof payload.data === 'object' ? payload.data : {};
        if (event !== 'targetlist:counts') return;
        const totals = data.totals || {};
        const totalRaw = Number.isFinite(totals.raw) ? totals.raw : null;
        const totalFiltered = Number.isFinite(totals.filtered) ? totals.filtered : null;
        setCounts({ totalRaw, totalFiltered });
        setStatus('Target list fetched.');
        if (bus) bus.emit({ text: 'Target list counts updated.' });
      };
      ...
      (async () => {
        try {
          ...
          if (result && result.totals) {
            const totalRaw = Number.isFinite(result.totals.raw) ? result.totals.raw : null;
            const totalFiltered = Number.isFinite(result.totals.filtered) ? result.totals.filtered : null;
            setCounts((prev) => ({
              totalRaw: totalRaw != null ? totalRaw : prev.totalRaw,
              totalFiltered: totalFiltered != null ? totalFiltered : prev.totalFiltered,
            }));
          }
          if (result && result.detached) {
            setStatus(`Detached (pid=${result.pid}).`);
          } else {
            setStatus('Complete.');
          }
          setStep('done');
    ...
    const totalRawLabel = counts.totalRaw != null ? String(counts.totalRaw) : '...';
    const totalFilteredLabel = counts.totalFiltered != null ? String(counts.totalFiltered) : '...';

    if (step === 'running') {
      return h(
        Box,
        { flexDirection: 'column' },
        h(Spinner, { label: status || 'Fetching target list' }),
        h(Text, { dimColor: true }, `Downloaded: ${totalRawLabel}`),
        h(Text, { dimColor: true }, `Passed token bouncer: ${totalFilteredLabel}`)
      );
    }
  ...
  function CommandTuiApp({ command, args, options, run }) {
    ...
    } else if (command === 'openai-fileprune') {
      content = h(FilePrunePanel, {
        run,
        bus: eventBus,
        onExit: exit,
      });
    } else if (command === 'targetlist') {
      content = h(TargetListPanel, {
        run,
        bus: eventBus,
        onExit: exit,
      });
