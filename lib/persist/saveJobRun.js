...
async function saveJobRun(params) {
  ...
  await query(
    `INSERT INTO sc_job_runs (job_run_id, job, context, input, response_raw)
     VALUES (:job_run_id, :job, CAST(:context AS JSON), CAST(:input AS JSON), CAST(:response_raw AS JSON))`,
    {
      job_run_id: jobRunId,
      job,
