 ...
 *   - final:    prompt + response merged payload (SAVE_ENRICHED only)
 *
 * Naming convention
 *   - <prefix>-<runId>.json
 ...
 *   - SAVE_ENRICHED / SAVE_ENHANCED behaves like SAVE_PROMPT + SAVE_RESPONSE + SAVE_FINAL
 *
 * NODE_ENV=development is a full override and enables all saves.
 *
 * @returns {{ saveRaw: boolean, savePrompt: boolean, saveResponse: boolean, saveFinal: boolean, env: string, legacy: { saveParsed: boolean, saveEnriched: boolean, saveEnhanced: boolean } }}
 */
function getArtifactConfig() {
  ...
  const saveFinal = legacySaveEnriched;
...
function createArtifactWriter({ command, segments = [], runId, logger } = {}) {
  ...
  function isStageEnabled(stage) {
    const { saveRaw, savePrompt, saveResponse, saveFinal } = getArtifactConfig();
    if (stage === 'raw') return !!saveRaw;
    if (stage === 'prompt') return !!savePrompt;
    if (stage === 'response') return !!saveResponse;
    if (stage === 'final') return !!saveFinal;
    return false;
  }

  function normalizeStage(stage) {
    const s = String(stage || '').trim().toLowerCase();
    if (s === 'raw' || s === 'prompt' || s === 'response' || s === 'final') return s;
   ...
   *
   * @param {'raw'|'prompt'|'response'|'final'} stage
   * @param {string} prefix
   * @param {any} data
  ...
  function write(stage, prefix, data) {
    ...
    let safePrefix = sanitizeSegment(prefix, 'artifact');
    if (
      normalizedStage === 'final'
      && safePrefix !== safeCommand
      && !safePrefix.startsWith(`${safeCommand}-`)
    ) {
      safePrefix = `${safeCommand}-${safePrefix}`;
    }
    ...
    if (process.env.NODE_ENV === 'development') {
      logger?.debug?.(`[jsonArtifacts] wrote ${normalizedStage} → ${fullPath}`);
    }
    return fullPath;
  }
