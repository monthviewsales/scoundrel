...
function createGetTransaction(rpc) {
  ...
  async function fetchOne(signature, opts) {
    ...
    try {
      const finalOpts = {
        // Support v0 (and legacy) transactions. This is required by Solana RPC
        // when decoding versioned transactions. Callers can override this if
        // needed by passing their own maxSupportedTransactionVersion.
        maxSupportedTransactionVersion: 0,
        // Default to confirmed commitment; callers can override this as well.
        commitment: 'confirmed',
        ...opts,
      };

      const response = await resolveRpcResult(rpc.getTransaction(signature, finalOpts));
