...
function createTransactionsPanel(ink) {
  ...
  return function TransactionsPanel({ transactions, maxItems = 10 }) {
    ...
    return h(
      ...
      ...items.map((tx) => {
        const emoji = tx && tx.statusEmoji ? tx.statusEmoji : "🟡";
        const statusColor =
        ...
        const slot = tx && tx.slot != null ? ` slot ${tx.slot}` : "";
        const tokens =
          tx && tx.tokens != null
            ? ` tokens ${formatAmount(Number(tx.tokens), 4) ?? tx.tokens}`
            : "";
        const sol =
          tx && tx.sol != null
            ? ` SOL ${formatAmount(Number(tx.sol), 6) ?? tx.sol}`
            : "";
        const err = tx && tx.errMessage ? ` ⚠️ ${tx.errMessage.slice(0, 60)}` : "";
        const price =
          tx && tx.coin && Number.isFinite(Number(tx.coin.priceUsd))
            ? ` @ $${Number(tx.coin.priceUsd).toFixed(6)}`
            : "";
        const changes =
          tx && tx.coin && tx.coin.events
            ? ` Δ1m:${formatPct(tx.coin.events["1m"])} Δ5m:${formatPct(tx.coin.events["5m"])} Δ15m:${formatPct(tx.coin.events["15m"])} Δ30m:${formatPct(tx.coin.events["30m"])}`
            : "";
        const holders = tx && tx.coin && Number.isFinite(Number(tx.coin.holders)) ? ` holders:${tx.coin.holders}` : "";
        const age =
          tx && tx.observedAt
            ? ` ${formatDurationMs(Math.max(0, now - Number(tx.observedAt)))} ago`
            : "";
        const line = `${emoji} ${side}${coinName ? ` ${coinName}` : ""}${mint ? ` (${mint})` : ""}${slot}${price}${tokens}${sol}${changes}${holders}${age}${err}`;
        return h(Text, { key: `${tx.txid || Math.random()}`, color: statusColor }, line);
      })
    );
  };
}
