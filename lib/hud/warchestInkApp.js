...
function createTransactionsPanel(ink) {
  ...
  return function TransactionsPanel({ transactions, maxItems = 10, tokenLookup = {} }) {
    ...
    function usdPerTokenFor(tx) {
      ...
      const looked = mint && tokenLookup && tokenLookup[mint] ? tokenLookup[mint] : null;
      const p = looked && looked.priceUsd != null ? Number(looked.priceUsd) : NaN;
    ...
    return h(
      ...
      ...items.map((tx, idx) => {
        ...
        const symFromTx = tx && tx.coin && (tx.coin.symbol || tx.coin.name) ? String(tx.coin.symbol || tx.coin.name) : "";
        const looked = mintFull && tokenLookup && tokenLookup[mintFull] ? tokenLookup[mintFull] : null;
        const symFromLookup = looked && looked.symbol ? String(looked.symbol) : "";
        ...
        const line = `${hhmm} ${sideLabel.padEnd(4, " ")} ${identCol} ${tokenPart}  ${arrow} ${solPart}${usdPart}`.trimEnd();
        const stableKey = (() => {
...
function createWalletCard(ink, RecentActivityList) {
  ...
  function TokenRow({ token, isStable, pnl, isFocused, tickerOffset, tickerWidthMax, now }) {
    ...
    const posLineRaw = token && token.positionLine ? String(token.positionLine) : null;
    const posRoiPct = token && token.position && token.position.roiPct != null ? Number(token.position.roiPct) : null;
    const posColor =
      Number.isFinite(posRoiPct) ? (posRoiPct > 0 ? "green" : posRoiPct < 0 ? "red" : undefined) : undefined;
    ...
    const badgeParts = [];
    if (token && token.curvePct != null && Number.isFinite(Number(token.curvePct))) {
      const txt = `Curve ${Number(token.curvePct).toFixed(1)}%`;
      badgeParts.push({ key: "curve", text: txt, color: curveColor(token.curvePct), dim: curveColor(token.curvePct) === undefined });
    }
    if (token && token.riskScore != null && Number.isFinite(Number(token.riskScore))) {
      const txt = `Risk ${Number(token.riskScore).toFixed(0)}`;
      badgeParts.push({ key: "risk", text: txt, color: riskColor(token.riskScore), dim: false });
    }
    if (token && token.top10Pct != null && Number.isFinite(Number(token.top10Pct))) {
      const txt = `Top10 ${Number(token.top10Pct).toFixed(1)}%`;
      badgeParts.push({ key: "top10", text: txt, color: top10Color(token.top10Pct), dim: false });
    }
    ...
    const sellOpsLineRaw = token && token.sellOpsLine ? String(token.sellOpsLine) : null;

    const sellOpsAgeMs =
      sellOps && Number.isFinite(Number(sellOps.ts)) && now != null ? Math.max(0, Number(now) - Number(sellOps.ts)) : null;
    const sellOpsAgeText = sellOpsAgeMs != null ? formatDurationMs(sellOpsAgeMs) : null;

    // Prefer the friendly fields (headline/details/metricsLine) when present.
    const sellOpsHeadline =
      sellOps && sellOps.headline ? String(sellOps.headline) : sellOpsLineRaw ? String(sellOpsLineRaw) : null;
    const sellOpsDetails = sellOps && sellOps.details ? String(sellOps.details) : null;
    const sellOpsMetricsLine = sellOps && (sellOps.metricsLine || sellOps.metrics_line) ? String(sellOps.metricsLine || sellOps.metrics_line) : null;

    // Compact one-liner for non-focused rows; richer multi-line for the focused row.
    const sellOpsCompact = (() => {
      if (!sellOps) return null;
      const rec = sellOps.recommendation ? String(sellOps.recommendation).toUpperCase() : null;
      const worst = sellOps.worstSeverity ? String(sellOps.worstSeverity) : null;
      const warnN = Number.isFinite(Number(sellOps.warningsCount)) ? Number(sellOps.warningsCount) : null;
      const parts = [];
    ...
    const sellOpsRecColor = sellOpsColor(sellOps && sellOps.recommendation ? sellOps.recommendation : null);
    ...
    return h(
        ...
        badgeParts.length
          ? h(
              ...
              ...badgeParts.flatMap((p, i) => {
                ...
                segs.push(
                  h(Text, { key: p.key, color: p.color, dimColor: p.dim === true }, p.text)
                );
                return segs;
              }),
      ...
      (() => {
        ...
        if (isFocused) {
          ...
          const headWithAge = sellOpsAgeText ? `${head}  |  ${sellOpsAgeText} ago` : head;
          ...
          const rc = sellOps && sellOps.riskControls ? sellOps.riskControls : null;
          let riskLine = null;
          ...
          if (rc && rc.trailing && rc.trailing.active) {
            const t = rc.trailing;
            if (Number.isFinite(Number(t.stopUsd)) && Number.isFinite(Number(t.priceUsd))) {
              const delta = Number(t.priceUsd) - Number(t.stopUsd);
              const deltaTxt = formatUsdCompact(delta);
              riskLine = riskLine
                ? `${riskLine}  |  Trail stop ${formatUsdCompact(t.stopUsd)} (Δ ${deltaTxt})`
          ...
          const riskLineColor = (() => {
            ...
            if (!rc || !Number.isFinite(Number(rc.hardStopLossPct))) return undefined;
              ...
              lines.push(
                h(
                  ...
                  h(
                    Text,
                    { dimColor: true },
                    padOrTrim(detail, width)
                  )
                )
              );
            }
          } else if (riskLine || detailParts.length) {
            const combined = [riskLine, ...detailParts].filter(Boolean).join("  |  ");
            lines.push(
              h(
                Box,
                { key: "sellops-detail", flexDirection: "row" },
                h(
                  Text,
                  { dimColor: true },
                  padOrTrim(combined, width)
                )
              )
            );
          } else {
            // Debug-friendly fallback: show what we DID receive when the richer fields are missing.
            const rec = sellOps && sellOps.recommendation ? String(sellOps.recommendation) : "n/a";
            const worst = sellOps && sellOps.worstSeverity ? String(sellOps.worstSeverity) : "n/a";
            const decision = sellOps && sellOps.decision ? String(sellOps.decision) : "n/a";
            const hasRc = !!(sellOps && sellOps.riskControls);
            const hasReasons = Array.isArray(sellOps && sellOps.reasons) && sellOps.reasons.length > 0;
  ...
  function TokenTable({ walletAlias, tokens, stableMints, pnlByMint = {}, tokenPage = 0, tokensPerPage = 10, focus, tickerOffset, stdoutColumns, now }) {
    if (!tokens || tokens.length === 0) {
    ...
    return h(
      ...
      ...page.items.map((entry, idx) => {
        ...
        const isFocused = focusMatch || (focus && focus.enabled === true && page.total === 1 && absoluteIndex === 0);
  ...
  return function WalletCard({ wallet, stableMints, lastSolPriceUsd, tokenPage = 0, tokensPerPage = 10, focus, tickerOffset, stdoutColumns, now }) {
    ...
    return h(
      ...
      h(
        ...
        (() => {
          ...
          const solText = `${fmtNum(effectiveSolBalance, 3)}${isLow ? " ⚠" : ""}`;
 ...
 */
  function WarchestApp({ hudStore, stableMints, tokenPage = 0, tokensPerPage = 10 }) {
      ...
      for (const alias of Object.keys(state || {})) {
        ...
        for (const t of tokens) {
          ...
          if (!out[mint]) {
            ...
            if ((!Number.isFinite(curP) || curP <= 0) && Number.isFinite(nextP) && nextP > 0) {
              cur.priceUsd = t.priceUsd;
            }
            if (!cur.symbol && (t.symbol || t.name)) cur.symbol = t.symbol || t.name;
          }
        }
      }
      return out;
    ...
    React.useEffect(() => {
      ...
      if (!stdinState.isRawModeSupported || !stdinState.stdin.isTTY) return undefined;
      ...
      try {
        if (typeof stdinState.stdin.resume === "function") stdinState.stdin.resume();
      } catch (_) {
    ...
    }, [stdinState?.stdin, stdinState?.setRawMode, stdinState?.isRawModeSupported]);
    ...
    return h(
      ...
      h(
        ...
        { dimColor: true },
        (() => {
          const wa = focus && focus.walletAlias ? String(focus.walletAlias) : 'n/a';
          const wi = focus && Number.isFinite(focus.walletIndex) ? String(focus.walletIndex) : 'n/a';
          const ti = focus && Number.isFinite(focus.tokenIndex) ? String(focus.tokenIndex) : 'n/a';
          return `Keys: ↑/↓ (or j/k) token  |  ←/→ (or h/l) wallet  |  Focus: ${wa} (w=${wi}, t=${ti})`;
        })()
      ),
