...
function createWalletCard(ink, RecentActivityList) {
  ...
  function TokenRow({ token, isStable, pnl, isFocused, tickerOffset }) {
    ...
    const sym = padOrTrim((token.symbol || "").slice(0, 24), COLUMN_WIDTHS.symbol);
    ...
    const tickerLine = isFocused
      ...
      : padOrTrim(tickerText, tickerWidth);
  ...
  function TokenTable({ walletAlias, tokens, stableMints, pnlByMint = {}, tokenPage = 0, tokensPerPage = 10, focus, tickerOffset }) {
    if (!tokens || tokens.length === 0) {
  ...
  return function WalletCard({ wallet, stableMints, lastSolPriceUsd, tokenPage = 0, tokensPerPage = 10, focus, tickerOffset }) {
  ...
  function WarchestApp({ hudStore, stableMints, tokenPage = 0, tokensPerPage = 10 }) {
    ...
    React.useEffect(() => {
      ...
      setFocus((prev) => {
        ...
        return { ...prev, enabled: true, walletIndex, walletAlias, tokenIndex };
      });
    }, [aliases.join("|"), state]);

    const focusedTokensLen = (() => {
      if (!focus || focus.enabled !== true || !focus.walletAlias) return 0;
      const wallet = state[focus.walletAlias];
      const tokens = wallet && Array.isArray(wallet.tokens) ? wallet.tokens : [];
      return tokens.length;
    })();

    // Focus-only ticker: advance offset only when the focused wallet has tokens.
    // IMPORTANT: do NOT depend on the entire `state` object or the interval will be recreated on every redraw.
    React.useEffect(() => {
      if (!focus || focus.enabled !== true || !focus.walletAlias) return undefined;
      if (!focusedTokensLen) return undefined;

      const id = setInterval(() => setTickerOffset((n) => n + 1), 180);
      return () => clearInterval(id);
    }, [focus && focus.enabled, focus && focus.walletAlias, focusedTokensLen]);

    // Keyboard navigation: arrow keys move focus through wallet tokens.
