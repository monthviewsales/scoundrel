...
function createWalletCard(ink, RecentActivityList) {
  ...
  function TokenRow({ token, isStable, pnl, isFocused, tickerOffset, tickerWidthMax, now }) {
    ...
    const sellOpsLine =
      sellOpsLineRaw && sellOpsAgeText ? `${sellOpsLineRaw}  |  ${sellOpsAgeText} ago` : sellOpsLineRaw;
    ...
    return h(
      ...
      // 3rd line: SellOps intel, if present
      sellOpsLine
        ? h(
            Box,
            { flexDirection: "row" },
            h(Text, { dimColor: !isFocused, color: isFocused ? sellOpsRecColor : undefined }, padOrTrim(sellOpsLine, tickerWidth || fullWidth))
          )
        : null
    );
  }
