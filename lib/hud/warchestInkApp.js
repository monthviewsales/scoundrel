  ...
  return function TransactionsPanel({ transactions, maxItems = 10 }) {
    ...
    return h(
      ...
      ...items.map((tx, idx) => {
        ...
        const line = `${emoji} ${side}${coinName ? ` ${coinName}` : ""}${mint ? ` (${mint})` : ""}${slot}${price}${tokens}${sol}${changes}${holders}${timeLabel}${err}`;
        const stableKey = (() => {
        ...
        return h(Text, { key: stableKey || `${line}-${idx}`, color: statusColor }, line);
      })
    );
  };
}
