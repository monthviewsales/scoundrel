        ...
        messages: [
            {
                ...
                content: [
                    ...
                    "- Select the TWO most profitable CLOSED trades and return them under signature_trades (maxItems=2), each with: mint, buy{ts,price_usd,size_sol}, sell{ts,price_usd,size_sol}, pnl_usd, hold_minutes.",
                    "  - If exact USD values are unavailable, set fields to null (schema allows null).",
                    "  - Only include trades that have both a buy and a sell leg within the provided rows.",
                    "",
                    "Return ONLY valid TraderProfile JSON according to the schema (strict)."
                ].join("\n")
            },
            { role: "user", content: JSON.stringify(payload) }
        ],
