...
const runDevscanFlow = createAnalysisFlow({
  ...
  persist: async ({ payload, analysis, buildResult }) => {
    try {
      ...
      const name = buildResult.normalized.mint
        ? `mint:${buildResult.normalized.mint}`
        : buildResult.normalized.developerWallet
          ? `dev:${buildResult.normalized.developerWallet}`
          : `devtokens:${buildResult.normalized.developerTokensWallet}`;
      await persistProfileSnapshot({
        BootyBox,
        profileId,
        name,
        wallet: buildResult.normalized.developerWallet || buildResult.normalized.developerTokensWallet || null,
        source: 'devscan',
...
async function runDevscan(options) {
  ...
  const flowResult = await runDevscanFlow({
    ...normalized,
    apiKey,
  });
