...
const runDevscanFlow = createAnalysisFlow({
  ...
  analyze: async ({ payload }) => analyzeDevscan({
    payload,
    model: process.env.xAI_API_MODEL,
  ...
  persist: async ({ payload, analysis, buildResult }) => {
    ...
    try {
      ...
      await queueVectorStoreUpload({
        ...
        attributes: {
          source: 'devscan',
          mint: normalized.mint || tokenData?.mintAddress || null,
          tokenName: tokenData?.name || null,
          tokenSymbol: tokenData?.symbol || null,
          developerWallet: normalized.developerWallet || normalized.developerTokensWallet || developerData?.wallet || null,
          developerName: developerData?.name || developerTokensData?.name || null,
        },
