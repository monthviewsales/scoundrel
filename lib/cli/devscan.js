  ...
  persist: async ({ payload, analysis, buildResult }) => {
    ...
    try {
      ...
      if (buildResult?.artifacts) {
        const prefixBase = normalized.mint || normalized.developerWallet || normalized.developerTokensWallet || 'final';
        finalPath = buildResult.artifacts.write('final', `${prefixBase}_final`, finalPayload);
      }
