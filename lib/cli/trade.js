      ...
      items.length
        ? h(
            ...
            ...items.map((p, idx) => {
              ...
              const suffix = p && p.data && typeof p.data === 'object' && p.data.txid ? ` txid=${shorten(p.data.txid)}` : '';
              return h(Text, { key: `${idx}-${label}` }, `${stamp ? `[${stamp}] ` : ''}${label}${suffix}`);
            }),
          )
        : h(Text, { dimColor: true }, 'Waiting for worker…'),
      done ? h(Box, { marginTop: 1 }, h(Text, { dimColor: true }, 'Done.')) : null,
    );
  }
