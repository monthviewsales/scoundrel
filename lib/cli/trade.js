...
module.exports = async function tradeCli(mint, opts) {
  ...
  const payload = {
    ...
    mint: trimmedMint,
    amount,
    walletAlias: walletAliasOrAddress,
    slippagePercent,
    priorityFee,
    useJito,
    dryRun,
  ...
  const env = buildWorkerEnv({
    rpcEndpoint: process.env.SOLANATRACKER_RPC_HTTP_URL || process.env.SOLANA_RPC_URL,
  ...
  if (!result) {
    logger.info('[scoundrel] trade completed, but worker returned no summary.');
    return;
  }
