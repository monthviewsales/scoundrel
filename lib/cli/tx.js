...
async function runTx({ signature, cmd }) {
    ...
    if (swapMode) {
        ...
        try {
            const walletResolver = wallets && wallets.resolver && typeof wallets.resolver.createWalletResolver === 'function'
                ? wallets.resolver.createWalletResolver()
                : null;

            if (walletResolver) {
                const resolved = await walletResolver.resolveAliasOrAddress(opts.wallet);

                if (!resolved || !resolved.wallet || !resolved.wallet.pubkey) {
                    logger.error(`[scoundrel] Unable to resolve wallet "${opts.wallet}" to a public key.`);
                    process.exit(1);
                }

                focusWalletAlias = resolved.wallet.alias || opts.wallet;
                focusWalletPubkey = resolved.wallet.pubkey;

                // Prefer an explicit walletId field if present, otherwise fall back to common id shapes.
                if (Object.prototype.hasOwnProperty.call(resolved.wallet, 'walletId')) {
                    focusWalletId = resolved.wallet.walletId;
                } else if (Object.prototype.hasOwnProperty.call(resolved.wallet, 'wallet_id')) {
                    focusWalletId = resolved.wallet.wallet_id;
                } else if (Object.prototype.hasOwnProperty.call(resolved.wallet, 'id')) {
                    focusWalletId = resolved.wallet.id;
                }
            } else {
                // Fallback: treat the provided option as a raw address.
                focusWalletAlias = opts.wallet;
                focusWalletPubkey = opts.wallet;
            }
        } catch (err) {
            logger.error(`[scoundrel] Failed to resolve wallet "${opts.wallet}": ${err.message}`);
            process.exit(1);
        }
    }
    ...
    const { recoverSwapInsightFromTransaction } = require('../services/txInsightService');
    ...
    try {
        ...
        for (let idx = 0; idx < summaries.length; idx += 1) {
            ...
            const signatureForSummary = (summary && summary.signature) ? summary.signature : signatures[idx] || signatures[0];
