...
} = require("../solana/addressValidation");
...
function applyInsightToSummary(summary, insight, coin) {
  if (!summary || !insight) return summary;

  const tokenDelta = Number(insight.tokenDeltaNet);
  const solDelta = Number(insight.solDeltaNet);
  const tokens = Number.isFinite(tokenDelta) ? Math.abs(tokenDelta) : null;
  const sol = Number.isFinite(solDelta) ? solDelta : null;

  if (insight.kind === "transfer" && !summary.label) {
    summary.label = "transfer";
  }
  if (insight.side && !summary.side) summary.side = insight.side;

  if (insight.mint && !summary.mint) summary.mint = insight.mint;
  if (insight.walletAddress && !summary.walletAddress) summary.walletAddress = insight.walletAddress;
  if (insight.walletAlias && !summary.walletAlias) summary.walletAlias = insight.walletAlias;
  if (insight.walletId && !summary.walletId) summary.walletId = insight.walletId;

  if (tokens != null && summary.tokens == null) summary.tokens = tokens;
  if (sol != null && summary.sol == null) summary.sol = sol;

  if (insight.priceSolPerToken != null && summary.priceSolPerToken == null) {
    summary.priceSolPerToken = insight.priceSolPerToken;
  }
  if (insight.priceUsdPerToken != null && summary.priceUsdPerToken == null) {
    summary.priceUsdPerToken = insight.priceUsdPerToken;
  }

  if (coin && typeof coin === "object") {
    if (coin.symbol && !summary.symbol) summary.symbol = coin.symbol;
    if (coin.name && !summary.name) summary.name = coin.name;
  }
...
function renderTxSummary(summary) {
  ...
  if (s.err) {
    ...
    if (typeof s.err === "string") {
      ...
      try {
        errStr = JSON.stringify(s.err, (key, val) =>
          typeof val === "bigint" ? val.toString() : val
        );
      } catch (_) {
 ...
 */
function writeTxSessionArtifacts({
  txid,
  sessionPayload,
  swapMode,
  focusWallet,
  ...
  if (!run || !run.artifacts || typeof run.artifacts.write !== "function") {
    throw new Error(
      "jsonArtifacts writer is unavailable for tx session artifacts."
    );
  }

  // Match autopsy convention: use response/* for outputs.
  const savedPath = run.artifacts.write(
    "response",
    "txSession",
    sessionPayload
  );
...
async function runTxSessionTui({
  title,
  txid,
  payload,
  canPersistSwap,
  ...
  const menu = [
    ...
    ...(canPersistSwap
      ? [{ key: "persist", label: "Upsert swap into BootyBox (persist)" }]
      : []),
    { key: "exit", label: "Exit" },
  ];
  ...
  return new Promise((resolve, reject) => {
    function App() {
      ...
      return h(
        ...
        status
          ? h(Box, { marginTop: 1 }, h(Text, { color: "yellow" }, status))
          : null,
    ...
    try {
      restoreScreen = prepareTuiScreen();
...
async function determineFocusWallet(initialIdentifier) {
  ...
  if (!resolver) {
    throw new Error(
      "wallet resolver is unavailable; ensure BootyBox is initialised"
    );
  }

  const rawIdentifier =
    initialIdentifier && typeof initialIdentifier === "string"
      ? initialIdentifier.trim()
      : "";
  if (rawIdentifier) {
    const firstPass = await resolveWithResolver(resolver, rawIdentifier);
    if (firstPass) return firstPass;
    if (looksLikeSolanaAddress(rawIdentifier)) {
      throw createInvalidAddressError(rawIdentifier, "wallet");
    }
    throw new Error(`Wallet "${rawIdentifier}" not found in registry.`);
  }
...
async function runTx({ signature, cmd }) {
  ...
  if (swapMode) {
    if (!opts.wallet || !opts.mint) {
      const errMsg =
        "[scoundrel] tx swap mode requires both --wallet and --mint.";
      logger.error(errMsg);
      if (isTui) {
        throw new Error(errMsg);
      }
    ...
    opts.mint = assertValidMintAddress(opts.mint);
  ...
  } = require("../txInspector/inspectTransaction");
  const {
  ...
  try {
    ...
    const result = await inspectTransaction(
      isBatch ? signatures : signatures[0],
      {
    ...
    const sessionPayload = {
      ...
      focusWallet: swapMode
        ? {
            alias: focusWalletAlias || opts.wallet,
            pubkey: focusWalletPubkey || opts.wallet,
            id: focusWalletId || null,
          }
        : null,
    ...
    for (let idx = 0; idx < summaries.length; idx += 1) {
      ...
      const signatureForSummary =
        summary && summary.signature
          ? summary.signature
          : signatures[idx] || signatures[0];

      const sessionRow = {
        signature: signatureForSummary,
        summary,
      };
      ...
      if (!swapMode) {
        if (summary && summary.raw) {
          try {
            ...
            const insight = await recoverSwapInsightFromTransaction(
              signatureForSummary,
              summary.raw,
              {}
            );
            ...
            let coin = null;
            if (insight && insight.mint) {
              try {
                // eslint-disable-next-line no-await-in-loop
                coin = await BootyBox.getCoinByMint(insight.mint);
              } catch (_) {
                coin = null;
              }
            }
            applyInsightToSummary(summary, insight, coin);
          } catch (err) {
            logger.debug(
              `[tx.cli] insight recovery failed for ${signatureForSummary}: ${err?.message || err}`
            );
          }
        }
      ...
      if (summary && summary.status && summary.status !== "ok") {
        ...
        logger.debug(
          `[tx.cli] failed-transaction branch for ${signatureForSummary}: ${errSummary}`
        );
        // eslint-disable-next-line no-console
        ...
        if (
          summary.networkFeeLamports != null ||
          ...
          // eslint-disable-next-line no-console
          console.log(
            "  Note: No token balance change detected; only network fee was paid."
          );
          if (feeLamportsRaw != null || feeSolRaw != null) {
            // eslint-disable-next-line no-console
            console.log(
              `  Network fee: ${feeLamportsRaw ?? "N/A"} lamports (${
                feeSolRaw ?? "N/A"
              } SOL)`
            );
          }
        ...
        if (
          persistMode &&
          focusWalletId &&
          typeof BootyBox.recordScTradeEvent === "function"
        ) {
          ...
          if (summary.blockTime != null) {
            try {
              const bt =
                typeof summary.blockTime === "bigint"
          ...
          const feeSolValue = Number.isFinite(feeSolRaw)
            ? feeSolRaw
            : typeof feeLamportsRaw === "number" &&
              Number.isFinite(feeLamportsRaw)
      ...
      if (!persistMode) {
        // eslint-disable-next-line no-console
        console.log(
          "  (read-only) Tip: add --persist to record this swap to BootyBox."
        );
      }
      ...
      if (!insight) {
        // eslint-disable-next-line no-console
        console.log(
          "  No swap-like balance changes detected for this wallet in this transaction."
        );
        // eslint-disable-next-line no-continue
        continue;
      }
      ...
      const feeLamports = Number.isFinite(feeLamportsFromInsight)
        ...
        : Number.isFinite(summary.networkFeeLamports)
        ? summary.networkFeeLamports
        : null;

      const hasTokenChange =
        tokenNet !== 0 || tokenIncrease !== 0 || tokenDecrease !== 0;
      const hasSolChange = solNetSol !== 0;
      const hasAny =
        hasTokenChange ||
        hasSolChange ||
        (typeof feeLamports === "number" && Number.isFinite(feeLamports));
      ...
      if (
        ...
        const tradeEvent = {
          ...
          priceUsdPerToken: null,
          solUsdPrice: null,
          feesSol: Number.isFinite(feeSol)
            ? feeSol
            : typeof feeLamports === "number" && Number.isFinite(feeLamports)
      ...
      if (Number.isFinite(priceSolPerToken)) {
        // eslint-disable-next-line no-console
        console.log(
          `    Price:              ${priceSolPerToken} SOL per token`
        );
      }
      ...
      if (typeof feeLamports === "number" && Number.isFinite(feeLamports)) {
        // eslint-disable-next-line no-console
        console.log(
          `    Network fee:        ${feeLamports} lamports${
            Number.isFinite(feeSol) ? ` (~${feeSol} SOL)` : ""
          }`
        );
      }
    }
    ...
    if (sessionMode) {
      ...
      const persistAction = async () => {
        if (!canPersistSwap) {
          throw new Error(
            "Persist is unavailable (missing walletId or BootyBox writer)."
          );
        }
        ...
        for (const row of rows) {
          ...
          const normalizedSide =
            insight.side === "buy" || insight.side === "sell"
              ? insight.side
              : insight.tokenDeltaNet > 0
              ? "buy"
              : insight.tokenDeltaNet < 0
              ? "sell"
              : null;
