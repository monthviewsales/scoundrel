...
async function runTx({ signature, cmd }) {
    ...
    if (swapMode) {
        ...
        try {
            let resolved = null;
            if (walletResolver) {
                resolved = await walletResolver.resolveAliasOrAddress(opts.wallet);
            }

            if (resolved && resolved.wallet && resolved.wallet.pubkey) {
                assignWalletFromRecord(resolved.wallet);
            } else {
                logger.warn(`[scoundrel] Unable to resolve wallet "${opts.wallet}"; launching selector...`);
                ...
                if (!selection || !selection.walletAddress) {
                    logger.error('[scoundrel] No wallet selected for tx swap context.');
                    process.exit(1);
                }

                const chosen = selection.walletAddress;
                let finalRecord = null;
                if (walletResolver) {
                    try {
                        const resolvedSelection = await walletResolver.resolveAliasOrAddress(chosen);
                        finalRecord = resolvedSelection && resolvedSelection.wallet ? resolvedSelection.wallet : null;
                    } catch (err) {
                        logger.debug && logger.debug('[scoundrel] resolver failed after selector:', err?.message || err);
                    }
                }

                assignWalletFromRecord(finalRecord || {
                    alias: selection.walletLabel || chosen,
                    pubkey: chosen,
                });
            }
        } catch (err) {
            const msg = err && err.message ? err.message : err;
            logger.error(`[scoundrel] Failed to resolve wallet "${opts.wallet}": ${msg}`);
            process.exit(1);
        }
    }
