...
function normalizeDbTradesToAutopsyTrades(rows) {
  ...
  return rows.map((r) => ({
    // Normalize DB trade rows into the same shape the autopsy metric extractors expect.
    // (We intentionally use `time` and `type` to match SolanaTracker responses.)
    tx: r.txid,
    type: r.side,
    side: r.side,
    ...
    time: r.executed_at,
    // Preserve original row for debugging if needed.
    __db: {
      id: r.id,
...
async function resolveWalletAddressFromTrades(dbTrades) {
  ...
  // 1) If the DB row already contains an address-like field (some schemas do), use it.
  const direct = first.wallet_address || first.walletAddress || first.wallet_pubkey || first.walletPubkey;
  if (direct) return String(direct).trim();

  // 2) Best-effort: try BootyBox helpers if they exist.
  const walletId = first.wallet_id;
  const walletAlias = first.wallet_alias;

  try {
    if (walletId && typeof BootyBox.getWalletById === 'function') {
      const w = await BootyBox.getWalletById(walletId);
      const addr = w && (w.address || w.wallet_address || w.pubkey || w.publicKey);
  ...
  try {
    if (walletAlias && typeof BootyBox.getWalletByAlias === 'function') {
      const w = await BootyBox.getWalletByAlias(walletAlias);
      const addr = w && (w.address || w.wallet_address || w.pubkey || w.publicKey);
      if (addr) return String(addr).trim();
    }
  } catch (_) {
 ...
 * @param {string} params.mint          Token mint address.
 * @param {*} [params.client] Optional SolanaTracker data client to reuse; when omitted a
 *   new client is created and closed after the run.
 * @returns {Promise<{ payload: Object, ai: Object, artifactPath: string }>} Artifact path and AI output.
 */
/**
 * Run a trade autopsy for a single wallet + mint campaign or by tradeUuid (DB mode).
 *
 * @param {Object} params
 * @param {string} [params.tradeUuid]   DB mode: trade_uuid to autopsy (loads trades from sc_trades)
 * @param {string} [params.walletLabel] Human-friendly label or "other".
 * @param {string} [params.walletAddress] Base58 wallet pubkey.
 * @param {string} [params.mint]        Token mint address.
 * @param {*} [params.client] Optional SolanaTracker data client to reuse; when omitted a
...
async function runAutopsy({ tradeUuid, walletLabel, walletAddress, mint, client: providedClient }) {
  ...
  // In DB mode we will infer wallet/mint after loading trades.
  ...
  try {
    const buildRes = isDbMode
      ? await buildAutopsyPayloadFromTradeUuid({
          tradeUuid,
          client,
        })
      : await buildAutopsyPayload({
    ...
    // If DB mode inferred wallet/mint, update locals for persistence/output.
    if (isDbMode) {
      resolvedWalletLabel = buildRes.walletLabel;
      resolvedWalletAddress = buildRes.walletAddress;
      resolvedMint = buildRes.mint;
    }

    const runId = buildRes.runId;
    payload = buildRes.payload;

    // Recompute safe strings for artifact naming after inference.
    const walletSafe2 = sanitizeSegment(resolvedWalletLabel || resolvedWalletAddress || 'wallet');
    const mintSafe2 = sanitizeSegment(resolvedMint || 'mint', 'mint');

    ai = await analyzeTradeAutopsy({ payload });
    artifactPath = saveArtifact(payload, ai, { runId, walletSafe: walletSafe2, mintSafe: mintSafe2 });
...
/**
 * Build the full enriched autopsy payload for a tradeUuid (DB mode).
 *
 * @param {Object} params
 * @param {string} params.tradeUuid   The trade_uuid to autopsy (loads trades from sc_trades)
 * @param {*} [params.client]         Optional SolanaTracker data client to reuse.
 * @param {string} [params.runId]     Optional runId; if not provided one is generated.
 * @param {string} [params.baseDir]   Optional base directory for artifacts.
 * @returns {Promise<Object>}         Autopsy build result.
 */
async function buildAutopsyPayloadFromTradeUuid({
  ...
  // NOTE: We may need the wallet address for SolanaTracker enrichment calls.
  ...
  // Refresh token metadata (ensureTokenInfo already upserts into DB)
  const tokenInfo = await ensureTokenInfo({ mint, client: resolvedClient });
  ...
  // Keep OHLCV enrichment for now (even though we don't need to persist it)
  const ochlvWindow = await fetchCandles({
    client: resolvedClient,
    mint,
    startTimestamp,
  ...
  const payload = {
    ...
    campaign: {
      ...
      endTimestamp,
      metrics,
      // carry the trade_uuid so downstream can link back to this campaign
