...
async function runAutopsy({ tradeUuid, walletLabel, walletAddress, mint, client: providedClient }) {
  ...
  try {
    ...
    artifactPath = saveArtifact(payload, ai, { runId: buildResult.runId, walletSafe, mintSafe });
...
async function buildAutopsyPayload({
  ...
  maybeWriteTokenInfoArtifact({ baseDir: resolvedBaseDir, runId: resolvedRunId, tokenInfo });

  const tradeResp = await resolvedClient.getUserTokenTrades(mint, walletAddress);
  maybeWriteTradeArtifact({ baseDir: resolvedBaseDir, runId: resolvedRunId, tradeResp, mint, walletAddress });
  ...
  maybeWriteParsedCampaignArtifact({
    walletLabel,
    walletAddress,
    mint,
    tokenInfo,
  ...
  maybeWriteEnrichedArtifact({ baseDir: resolvedBaseDir, runId: resolvedRunId, payload });
 ...
 * @returns {Promise<Object>}
 */
async function buildAutopsyPayloadFromTradeUuid({ tradeUuid, client, runId, baseDir }) {
  if (!tradeUuid) {
...
async function fetchCandles({ client, mint, startTimestamp, endTimestamp, baseDir, runId }) {
  ...
  maybeWriteCandleArtifact({ baseDir, runId, params, candlesResp });
...
/**
 * Persist the final autopsy artifact alongside the AI output.
 *
 * @param {Object} payload
 * @param {Object} aiResult
 * @param {Object} params
 * @param {string} params.runId
 * @param {string} params.walletSafe
 * @param {string} params.mintSafe
 * @returns {string}
 */
function saveArtifact(payload, aiResult, { runId, walletSafe, mintSafe }) {
  ...
  fs.writeFileSync(outPath, JSON.stringify({ ...payload, ai: aiResult }, null, 2));
  return outPath;
}

/**
 * Store the autopsy payload in the profiles table.
 *
 * @param {Object} params
 * @param {Object} params.payload
 * @param {Object} params.aiResult
 * @param {string} params.autopsyId
...
/**
 * Persist the trade autopsy execution to sc_trade_autopsies if available.
 *
 * @param {Object} params
 * @param {string} params.autopsyId
 * @param {Object} params.payload
 ...
 * @param {string} params.mint
 */
async function persistTradeSnapshot({ autopsyId, payload, ai, walletAddress, mint }) {
  try {
    await BootyBox.init();
...
// Artifact helpers
// ---------------------------------------------------------------------------
function maybeWriteTokenInfoArtifact({ baseDir, runId, tokenInfo }) {
  if (!SAVE_RAW) return;
  try {
...
function maybeWriteTradeArtifact({ baseDir, runId, tradeResp, mint, walletAddress }) {
  if (!SAVE_RAW) return;
  try {
    writeArtifact(baseDir, ['raw'], `userTokenTrades-${runId}.json`, {
...
function maybeWriteParsedCampaignArtifact({
  walletLabel,
  walletAddress,
  mint,
  tokenInfo,
  ...
  if (!SAVE_PARSED) return;
  const parsedSnapshot = {
    wallet: { label: walletLabel, address: walletAddress },
    token: { mint, symbol: tokenInfo?.symbol, name: tokenInfo?.name, decimals: tokenInfo?.decimals },
    campaign: { trades: mintTrades, startTimestamp, endTimestamp, metrics },
  };
  try {
...
function maybeWriteEnrichedArtifact({ baseDir, runId, payload }) {
  if (!SAVE_ENRICHED) return;
  try {
...
function maybeWriteCandleArtifact({ baseDir, runId, params, candlesResp }) {
  if (!SAVE_RAW) return;
  try {
    writeArtifact(baseDir, ['raw'], `ohlcv-${runId}.json`, { request: params, response: candlesResp });
  } catch (err) {
