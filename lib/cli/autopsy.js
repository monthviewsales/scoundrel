 ...
 */
async function runAutopsy({ walletLabel, walletAddress, mint, client: providedClient }) {
  if (!walletAddress || !mint) {
  ...
  try {
    const buildRes = await buildAutopsyPayload({
      walletLabel,
      walletAddress,
      mint,
      client,
    ...
    artifactPath = saveArtifact(payload, ai, { runId, walletSafe, mintSafe });
