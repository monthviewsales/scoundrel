...
const runAutopsyFlow = createAnalysisFlow({
  ...
  persist: async ({ payload, analysis, buildResult }) => {
    ...
    await queueVectorStoreUpload({
      ...
      attributes: {
        source: 'autopsy',
        wallet: campaignWallet.address || null,
        traderName: campaignWallet.label || payload?.meta?.traderName || null,
        traderAlias: payload?.meta?.traderAlias || null,
        mint: campaignToken.mint || buildResult?.mint || payload?.meta?.mint || null,
        tokenName: campaignToken.name || null,
        tokenSymbol: campaignToken.symbol || null,
      },
