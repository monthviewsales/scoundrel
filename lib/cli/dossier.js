...
const runDossierFlow = createAnalysisFlow({
  ...
  build: async ({ options, createArtifacts }) => {
    ...
    logger.debug('[dossier] merged sizes before analyze:', {
      trades: Array.isArray(rawTrades) ? rawTrades.length : -1,
      chart: Array.isArray(rawChart) ? rawChart.length : -1,
    });
  ...
  persist: async ({ payload, analysis, buildResult }) => {
    try {
      ...
      if (buildResult?.artifacts) {
        const traderPrefix = buildResult.meta.traderName || buildResult.meta.traderAlias || 'final';
        finalPath = buildResult.artifacts.write('final', `${traderPrefix}_final`, finalPayload);
      }
