  ...
  try {
    ...
    for (const t of rawTrades) {
      ...
      if (!isBase58Mint(mint)) {
        if (isDev) logger.debug(`[dossier] skipping non-base58 mint: ${mint}`);
        continue;
      }
    ...
    for (const mint of lastMints) {
      try {
        ...
        if (isDev) logger.debug(`[dossier] mint ${mint} user-trades fetched: ${count}`);
    ...
    try {
      ...
      if (isDev) logger.debug('[dossier] saved job run (sc_job_runs)');
    } catch (e) {
    ...
    try {
      ...
      if (isDev) logger.debug(`[dossier] persisted analysis ${analysisId}`);

      // Archive the full dossier snapshot in DB (like autopsy).
      try {
        ...
        if (isDev) logger.debug('[dossier] archived snapshot (sc_profiles)');
      } catch (e) {
        logger.warn('[dossier] failed to archive snapshot (sc_profiles):', e?.message || e);
      }

      // Disk artifacts are controlled by jsonArtifacts (SAVE_* and NODE_ENV override). No extra cleanup here.
    } catch (persistErr) {
