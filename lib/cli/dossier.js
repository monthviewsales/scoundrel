...
async function harvestWallet({ wallet, traderName, startTime, endTime, limit = DEFAULT_LIMIT, concurrency = 6, includeOutcomes = false, featureMintCount = FEATURE_MINT_COUNT_DEFAULT, runAnalysis = true }) {
  ...
  try {
    ...
    for (const t of rawTrades) {
      ...
      if (!isBase58Mint(mint)) { if (ENV === 'development') logger.warn('[dossier] skipping non-base58 mint:', mint); continue; }
    ...
    if (ENV === 'development') {
      if (lastMints.length === 0) {
        ...
        logger.info(`[dossier] last ${lastMints.length}/${featureMintCount} mints:`, lastMints.join(', '));
      }
    }
    ...
    for (const mint of lastMints) {
      try {
        if (ENV === 'development') {
          logger.info('[dossier] fetching user-token-trades', {
            mint,
            owner: wallet,
            sortDirection: 'DESC',
          });
        ...
        if (ENV === 'development') logger.info(`[dossier] mint ${mint} user-trades fetched: ${count}`);
        ...
        if (SAVE_RAW) {
          writeDataArtifact([
            'raw',
            'mints',
          ], `${mint}-minttrades-${runId}.json`, resp);
        }
    ...
    // Merged payload archive:
    // - If SAVE_ENRICHED=true, we will archive merged into sc_profiles (DB) after analysis.
    // - Otherwise, write the merged payload to disk for traceability.
    let mergedPath = null;
    if (!SAVE_ENRICHED) {
      mergedPath = writeDataArtifact([
        'merged',
      ], `merged-${runId}.json`, merged);
      logger.info(`[dossier] wrote merged payload → ${mergedPath}`);
    }
    ...
    if (SAVE_ENRICHED) {
      ...
      if (ENV === 'development') {
        logger.info(`[dossier] wrote enriched features → ${enrichedPath}`);
      }
    }
    ...
    try {
      ...
      if (ENV === 'development') logger.info('[dossier] saved job run (sc_job_runs)');
    } catch (e) {
      logger.warn('[dossier] failed to save job run:', e?.message || e);
    }
    ...
    try {
      ...
      await BootyBox.recordWalletAnalysis({
        ...
        merged: SAVE_ENRICHED ? merged : null,
        responseRaw: openAiResult,
        jsonVersion: openAiResult?.version || null,
      });

      if (ENV === 'development') logger.info(`[dossier] persisted analysis ${analysisId}`);

      // Archive the full dossier snapshot in DB (like autopsy) when SAVE_ENRICHED is enabled.
      if (SAVE_ENRICHED) {
        try {
          await BootyBox.upsertProfileSnapshot({
            ...
            wallet,
            profile: { merged, ai: openAiResult },
            source: 'dossier',
          });
          if (ENV === 'development') logger.info('[dossier] archived snapshot (sc_profiles)');
        } catch (e) {
          logger.warn('[dossier] failed to archive snapshot (sc_profiles):', e?.message || e);
        }
      }
      ...
      try {
        ...
        if (ENV === 'development') logger.info(`[dossier] wrote profile → ${profilePath}`);
      } catch (e) {
        logger.warn('[dossier] failed to write profile JSON:', e?.message || e);
      }
      ...
      if (ENV === 'development') {
        if (ENV === 'development') logger.info('[dossier] dev mode: keeping SolanaTracker artifacts on disk');
      } else {
        try {
          ...
          logger.warn('[dossier] cleanup warning:', cleanupErr?.message || cleanupErr);
        }
      }
    } catch (persistErr) {
