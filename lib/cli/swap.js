 ...
 *
 * This helper temporarily no-ops common logger + console methods and returns
 * a restore function.
 *
 * @param {object} log
 * @returns {() => void}
...
function muteStdoutLogging(log) {
  ...
  const originalLogger = {
    info: log && typeof log.info === 'function' ? log.info : null,
    warn: log && typeof log.warn === 'function' ? log.warn : null,
    error: log && typeof log.error === 'function' ? log.error : null,
    debug: log && typeof log.debug === 'function' ? log.debug : null,
    verbose: log && typeof log.verbose === 'function' ? log.verbose : null,
    silly: log && typeof log.silly === 'function' ? log.silly : null,
  };
  ...
  // Silence Winston-style logger methods if present.
  if (log && typeof log === 'object') {
    if (typeof log.info === 'function') log.info = noop;
    if (typeof log.warn === 'function') log.warn = noop;
    if (typeof log.error === 'function') log.error = noop;
    if (typeof log.debug === 'function') log.debug = noop;
    if (typeof log.verbose === 'function') log.verbose = noop;
    if (typeof log.silly === 'function') log.silly = noop;
  }
  ...
  return function restore() {
    ...
    if (log && typeof log === 'object') {
      if (originalLogger.info) log.info = originalLogger.info;
      if (originalLogger.warn) log.warn = originalLogger.warn;
      if (originalLogger.error) log.error = originalLogger.error;
      if (originalLogger.debug) log.debug = originalLogger.debug;
      if (originalLogger.verbose) log.verbose = originalLogger.verbose;
      if (originalLogger.silly) log.silly = originalLogger.silly;
    }
  };
}
...
module.exports = async function tradeCli(mint, opts) {
  ...
  const shouldUseInk = Boolean(process.stdout.isTTY && process.stdin.isTTY);
  ...
  if (shouldUseInk) {
    logger.info(queuedMessage);
  } else {
    console.log(chalk.cyan(queuedMessage));
  }
  ...
  try {
    workerResponse = await hub.runSwap(payload, {
      env,
      timeoutMs: 120_000,
      captureOutput: Boolean(progressUi),
      onProgress: (p) => {
  ...
  if (!monitorResult && monitorPayload) {
    ...
    if (monitorDetach) {
      try {
        ...
        if (progressUi) progressUi.emitProgress({ event: 'swap:monitor:detached' });
      } catch (err) {
        logger.warn(`[scoundrel] swap monitor detach failed: ${err?.message || err}`);
      }
    } else {
      if (progressUi) progressUi.emitProgress({ event: 'swap:monitor:start' });
      try {
        monitorResult = await hub.runTxMonitor(monitorPayload, {
          env: monitorEnv,
          timeoutMs: 120_000,
        ...
        logger.warn(`[scoundrel] swap monitor failed: ${err?.message || err}`);
      } finally {
        if (progressUi) {
