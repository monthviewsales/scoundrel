...
function buildSwapInputError(errors) {
  const list = Array.isArray(errors) ? errors : [];
  const summary = list
    .map((err) => {
      if (!err || typeof err !== 'object') return null;
      const field = err.field ? String(err.field) : 'input';
      const message = err.message ? String(err.message) : 'invalid';
      return `${field}: ${message}`;
    })
    .filter(Boolean)
    .join('; ');
  const error = new Error(summary ? `Invalid swap input: ${summary}` : 'Invalid swap input.');
  error.fields = list;
  return error;
}
...
async function createSwapProgressInkRenderer(context) {
  ...
  const ink = await import('ink');
  const React = require('react');
  const { createTxMonitorPanelComponents } = require('../tui/txMonitorPanel');
  const h = React.createElement;
  ...
  function App() {
    ...
    const progressEvents = items.length
      ? h(
          Box,
          { flexDirection: 'column', marginTop: 1 },
          ...items.map((p, idx) => {
    ...
    return h(
      ...
      {
        title,
        subtitle: headerLine || null,
        status: txSummary ? null : progressLine,
        summary: txSummary,
        hint: txSummary ? 'Press Enter, q, or Esc to exit' : null,
      },
      txSummary
        ? null
        : h(
  ...
  let instance;
  try {
    instance = render(h(App), { exitOnCtrlC: false });
  } catch (err) {
    cleanup();
    throw err;
  }
...
module.exports = async function tradeCli(mint, opts) {
  ...
  if (!side || (opts.buy && opts.sell)) {
    const errors = [];
    if (!side) {
      errors.push({ field: 'side', message: 'missing --buy or --sell flag' });
    }
    if (opts.buy && opts.sell) {
      errors.push({ field: 'side', message: 'use either --buy or --sell, not both' });
    }
    throw buildSwapInputError(errors);
  }
  ...
  const inputErrors = [];
  let trimmedMint = null;
  let amount = null;
  try {
    trimmedMint = assertValidMintAddress(mint);
  } catch (err) {
    inputErrors.push({ field: 'mint', message: err?.message || String(err) });
  }
  try {
    amount = normalizeAmount(side, rawAmount);
  } catch (err) {
    inputErrors.push({ field: 'amount', message: err?.message || String(err) });
  }
  if (inputErrors.length) {
    throw buildSwapInputError(inputErrors);
  }

  let resolvedWallet;
  try {
    resolvedWallet = await resolveWalletRecord(walletAliasOrAddress);
  } catch (err) {
    throw buildSwapInputError([{ field: 'wallet', message: err?.message || String(err) }]);
  }
