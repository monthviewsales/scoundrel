...
const { listAutoAttachedWarchestWallets, getDefaultFundingWallet } = require('../wallets/walletRegistry');
const {
  DEFAULT_EVENT_PATH,
  DEFAULT_HUD_STATE_PATH,
  DEFAULT_HUD_REFRESH_PATH,
  DEFAULT_STATUS_PATH,
} = require('../warchest/events');
const { buildWorkerEnv, forkWorkerWithPayload } = require('../warchest/workers/harness');

const LEGACY_DATA_DIR = path.join(__dirname, '..', '..', 'data', 'warchest');
const LEGACY_PID_PATH = path.join(LEGACY_DATA_DIR, 'warchest.pid');
const SERVICE_WORKER_SCRIPT = path.join(__dirname, '..', 'warchest', 'workers', 'warchestService.js');
const HUD_WORKER_SCRIPT = path.join(__dirname, '..', 'warchest', 'workers', 'warchestHudWorker.js');
const POSITION_HEAL_WORKER_SCRIPT = path.join(
  __dirname,
  '..',
  'warchest',
  'workers',
...
/**
 * Persist a HUD refresh request so a running service can resync snapshots.
 *
 * @param {object} opts
 ...
 * @returns {void}
 */
function writeHudRefreshRequest(opts = {}) {
  try {
    const targetPath = DEFAULT_HUD_REFRESH_PATH;
    ...
    const payload = {
      ...
      reason: opts.reason || 'manual-heal',
      wallets: Array.isArray(opts.walletSpecs) ? opts.walletSpecs : [],
    };
    ...
    logger.warn('[warchest] failed to write HUD refresh request:', err?.message || err);
  }
}
...
function buildServiceArgs({ walletSpecs, hudStatePath }) {
  const args = [
  ...
  if (hudStatePath) {
    args.push('--hud-state', hudStatePath);
  }

  return args;
}

function buildHudArgs({ walletSpecs, followHub, hubEventsPath, hudStatePath }) {
  const args = [
    HUD_WORKER_SCRIPT,
    ...buildWalletArgs(walletSpecs),
  ];
  ...
  if (followHub !== false) {
    ...
    args.push('--hud-state', hudStatePath || DEFAULT_HUD_STATE_PATH);
  }
...
function spawnWorker(args, env, label) {
  return new Promise((resolve, reject) => {
    ...
    child.on('exit', (code, signal) => {
      ...
      const name = label ? String(label) : 'worker';
      logger.info(`[warchest] ${name} exited (${reason})`);
      resolve({ code, signal });
    });
  });
}
...
/**
 * Start the warchest service worker in foreground mode.
 ...
 * @param {string} [opts.hudStatePath] - Optional override for HUD state snapshot path
 * @returns {Promise<void>}
 */
async function start(opts) {
  const { walletSpecs = [], hudStatePath } = opts || {};
  clearLegacyPid();
  ...
  logger.info('[warchest] launching warchest service worker (headless)');

  const env = buildWorkerEnv({ hudMode: false, inkMode: false });
  await spawnWorker(args, env, 'warchest service');
}
...
/**
 * Restart the service worker by clearing legacy artifacts and launching a fresh foreground worker.
 *
 * @param {Object} opts
 * @param {string[]} [opts.walletSpecs]
 * @param {string} [opts.hudStatePath]
 * @returns {Promise<void>}
 */
...
/**
 * Run a one-off position heal against current wallet holdings.
 *
 * @param {Object} opts
 * @param {string[]} [opts.walletSpecs] - Array of wallet specs alias:pubkey:color
 * @returns {Promise<void>}
...
async function heal(opts) {
  ...
  const resolvedWalletSpecs = await resolveWalletSpecsFromConfig(walletSpecs);
  if (!resolvedWalletSpecs || resolvedWalletSpecs.length === 0) {
    logger.warn('[warchest] heal aborted: no wallets available to attach.');
    return;
  }

  const payload = { walletSpecs: resolvedWalletSpecs, reason: 'manual' };
  const env = buildWorkerEnv({ hudMode: false, inkMode: false });

  const res = await forkWorkerWithPayload(POSITION_HEAL_WORKER_SCRIPT, {
    payload,
    env,
    timeoutMs: 120_000,
  });
  ...
  logger.info(`[warchest] position heal complete: ${formatPositionHealSummary(result)}`);

  writeHudRefreshRequest({
    reason: 'manual-heal',
    walletSpecs: resolvedWalletSpecs,
  });
}
...
async function hud(opts) {
  const { walletSpecs = [], followHub = true, hubEventsPath, hudStatePath } = opts || {};
  ...
  const env = buildWorkerEnv({ hudMode: true, inkMode: true, extraEnv: { WARCHEST_HUD: '1' } });
  await spawnWorker(args, env, 'warchest HUD');
}
