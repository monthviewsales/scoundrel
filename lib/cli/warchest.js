...
const WORKER_SCRIPT = path.join(__dirname, '..', 'warchest', 'workers', 'warchestService.js');
...
function spawnHudWorker(args, env) {
...
async function start(opts) {
  ...
  const env = buildWorkerEnv({ hudMode: hud, inkMode: hud });
  await spawnHudWorker(args, env);
}
...
async function hud(opts) {
  ...
  const env = buildWorkerEnv({ hudMode: true, inkMode: true });
  await spawnHudWorker(args, env);
}
