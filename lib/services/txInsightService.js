 ...
 */
async function recoverPriceFromTransactionv2(txid, txOverride = null) {
  let tx = txOverride;
  if (!tx) {
  ...
  try {
    ...
    for (const pre of preTokenBalances) {
      if (!pre) continue;
      const key = `${pre.mint || ''}:${pre.owner || ''}:$${pre.accountIndex ?? ''}`;
      preMap.set(key, pre);
    }

    for (const post of postTokenBalances) {
      if (!post) continue;
      const key = `${post.mint || ''}:${post.owner || ''}:$${post.accountIndex ?? ''}`;
