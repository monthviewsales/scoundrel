...
async function recoverSwapInsightFromTransaction(txid, txOverride = null, opts = {}) {
  ...
  try {
    ...
    // Determine which wallet perspective to use (explicit argument or fee payer).
    ...
    // Meta drives balance/fee deltas; without it we're blind.
    const meta = tx.meta;
    if (!meta) {
      logger.warn(
    ...
    // Token balance entries often reference owners indirectly via index.
    ...
    // Token balances are tracked separately from SOL so we aggregate per mint below.
    ...
    // Prefer owner-relative deltas when we know which wallet initiated the swap.
    if (normalizedWallet) {
      ...
      // Fallback path compares each token account before/after when no wallet context exists.
    ...
    if (deltasByMint.size === 0 && solSpent > 0 && solReceived === 0) {
      logger.info(
    ...
    // Identify primary input/output token flows while ignoring non-tradable numeraires.
    let outputMint = null;
    let outputDelta = 0;
    let inputMint = null;
    let inputDelta = 0;
    ...
    // Derive SOL-per-token using whichever direction (buy/sell) had valid deltas.
    ...
    // Positive delta => tokens acquired; negative => tokens sold.
