...
function hasRealTokenData(info) {
  ...
  const pools = Array.isArray(info?.pools)
    ? info.pools
    : Array.isArray(token?.pools)
      ? token.pools
      : [];
  const events = info?.events || token?.events || null;
  const risk = info?.risk || token?.risk || null;
  ...
  const hasEvents = Array.isArray(events) ? events.length > 0 : hasNonEmptyObject(events);
  const hasRisk = Array.isArray(risk) ? risk.length > 0 : hasNonEmptyObject(risk);
  const hasPools = pools.some(Boolean);
  const hasTradeStats =
    info?.buys != null ||
    info?.sells != null ||
    info?.txns != null ||
...
/**
 * Fill missing metadata in a token info payload with cached DB fields.
 * This preserves API-provided data while preventing null overwrites.
 *
 * @param {object|null|undefined} info
 * @param {object|null|undefined} cached
 * @returns {object|null|undefined}
 */
function mergeTokenInfoWithCache(info, cached) {
  if (!info || !cached) return info;

  const cachedToken =
    cached && typeof cached === 'object'
      ? (cached.token && typeof cached.token === 'object' ? cached.token : cached)
      : null;

  if (!cachedToken || typeof cachedToken !== 'object') return info;

  const merged = { ...info };
  const hasNestedToken = merged.token && typeof merged.token === 'object';
  const token = hasNestedToken ? { ...merged.token } : null;
  const target = token || merged;

  const shouldFill = (value) => {
    if (value == null) return true;
    return typeof value === 'string' && value.trim() === '';
  };

  const fillFromCache = (key) => {
    if (!shouldFill(target[key])) return;
    if (cachedToken[key] != null) {
      target[key] = cachedToken[key];
    }
  };

  ['mint', 'symbol', 'name', 'decimals', 'image', 'uri'].forEach(fillFromCache);

  if (token) {
    merged.token = token;
    if (merged.symbol == null && token.symbol != null) merged.symbol = token.symbol;
    if (merged.name == null && token.name != null) merged.name = token.name;
    if (merged.decimals == null && token.decimals != null) merged.decimals = token.decimals;
    if (merged.image == null && token.image != null) merged.image = token.image;
    if (merged.uri == null && token.uri != null) merged.uri = token.uri;
    if (merged.mint == null && token.mint != null) merged.mint = token.mint;
  }
...
function buildCoinUpsertPayload(info, mintOverride) {
  ...
  return {
    ...
    status: 'complete',
    token,
    pools: (Array.isArray(info?.pools) ? info.pools : Array.isArray(token?.pools) ? token.pools : [])
      .filter((p) => p && typeof p === 'object')
      .map((p) => {
        // BootyBox now keys pools by address (pools.id = pool address).
        // SolanaTracker commonly provides this as `poolId`.
        const poolId =
          (typeof p.poolId === 'string' && p.poolId.trim()) ||
          (typeof p.id === 'string' && p.id.trim()) ||
          (typeof p.poolAddress === 'string' && p.poolAddress.trim()) ||
          (typeof p.address === 'string' && p.address.trim()) ||
          null;

        // Preserve original fields but ensure `poolId` is present for downstream upserts.
  ...
  for (const row of rows) {
    ...
    try {
      ...
      const missingColumn =
        ...
        /no such column/i.test(String(err?.message || '')) ||
        /has no column named/i.test(String(err?.message || ''));
      ...
      const context = {
        ...
        sqliteCode: err?.code || undefined,
      };
      if (err?.sqlMessage) context.sqlMessage = err.sqlMessage;
      if (err?.sql) context.sql = err.sql;
      if (err?.name) context.errName = err.name;

      const hint = missingColumn ? ' (DB schema mismatch; run migrations / update sqliteSchema.js)' : '';
      ...
      if (emit) {
        const errMsg = payload.err || 'unknown error';
        const codeMsg = payload.code ? ` code=${payload.code}` : '';
        // Put the important bits in the message so they show up in console even if metadata is suppressed.
        logFn(
          `Failed to persist token info${hint} mint=${mint}${codeMsg} err=${errMsg}`,
          payload
        );

        // In non-HUD mode, also emit a debug stack (goes to file transports if configured).
        if (!hud && err?.stack) {
          logger.debug('Persist token info stack', { mint, stack: err.stack });
        }
      }
    }
  }
...
async function ensureTokenInfo({ mint, client, forceRefresh = false }) {
  ...
  if (dbReady) {
    try {
      ...
      if (!forceRefresh && cached && (cached.symbol || cached.name)) {
        return cached;
      }
    } catch (err) {
  ...
  const mergedInfo = mergeTokenInfoWithCache(info, cached);

  // Build the payload in the shape BootyBox expects:
  const payload = buildCoinUpsertPayload(mergedInfo, mint);

  if (dbReady) {
    // Reuse the shared upsert helper so error handling stays consistent.
    const result = await upsertTokenInfoPayload(payload);

    // Surface a concise summary in normal CLI output when persistence failed.
    if (result && result.enabled && result.failed > 0) {
      logger.warn(
        'Token info persistence had failures',
        {
          mint,
