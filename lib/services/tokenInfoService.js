...
async function ensureTokenInfo({ mint, client }) {
  ...
  const payload = {
    ...
    lastUpdated: parseNumber(
      info?.lastUpdated ??
        info?.last_updated ??
        info?.updatedAt ??
        info?.updated_at,
    ),
    lastEvaluated: parseNumber(
      info?.lastEvaluated ?? info?.last_evaluated,
    ) ?? 0,
    price: parseNumber(
      info?.price ?? info?.priceUsd ?? info?.price_usd,
    ),
    liquidity: parseNumber(
      info?.liquidity ??
        info?.liquidityUsd ??
        info?.liquidity_usd,
    ),
