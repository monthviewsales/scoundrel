...
function buildDevscanMetadataArtifact({ mint, source, metadataRow, fetchedAt } = {}) {
  ...
  const rawCode = error?.code || response?.status || response?.statusCode || null;
  const rawMessage = error?.message || response?.message || null;
  const codeString = rawCode != null ? String(rawCode) : '';
  const isMintNotFound = /MINT_NOT_FOUND|TOKEN_NOT_FOUND/i.test(codeString);

  let devscanResponseStatus = 'api_error';
  if (response && response.success === true) {
    devscanResponseStatus = 'success';
  } else if (isMintNotFound) {
    devscanResponseStatus = 'mint_not_found';
  }

  const responseError = devscanResponseStatus === 'success'
    ? null
    : {
        code: isMintNotFound ? 'mint_not_found' : (rawCode != null ? rawCode : 'api_error'),
        message: rawMessage || (isMintNotFound ? 'mint not found' : 'devscan error'),
      };
  ...
  return {
    ...
    devscanResponseStatus,
    error: responseError,
    data: data
      ? {
          creator: data.creator ?? null,
          creatorWallet: data.creatorWallet ?? null,
          mainDeveloperWallet: data.mainDeveloperWallet ?? null,
          status: data.status ?? null,
          launchPlatform: data.launchPlatform ?? null,
          fnfDeveloperWallet: data.fnfDeveloperWallet ?? null,
          fnfName: data.fnfName ?? null,
          isFNFToken: data.isFNFToken ?? null,
          isCurrentlyLive: data.isCurrentlyLive ?? null,
          dexpaid: data.dexpaid ?? data.dexPaid ?? null,
          dexboosts: data.dexboosts ?? null,
