 ...
 */
function buildFromMintMap(mintMap, featureMintCount){
  const rows = [];
  for (const [mint, list] of Object.entries(mintMap||{})){
    ...
    const legs = Array.isArray(realized.perLeg)
      ? realized.perLeg.filter(v => typeof v === 'number' && isFinite(v))
      : [];

    const wins = legs.filter(x => x > 0);
    const losses = legs.filter(x => x < 0);

    const maxGainPct = legs.length ? Math.max(...legs) : null;
    const maxLossPct = legs.length ? Math.min(...legs) : null;
    const avgGainPctWinners = wins.length ? mean(wins) : null;
    const avgLossPctLosers = losses.length ? mean(losses) : null;

    const sumBuyQty  = buys.reduce((a,t)=>a + (Number(t.amount)||0), 0);
    const sumSellQty = sells.reduce((a,t)=>a + (Number(t.amount)||0), 0);
    let residualSizePct = null;
    if (sumBuyQty > 0) {
      residualSizePct = Math.abs(sumBuyQty - sumSellQty) / sumBuyQty;
    }
    const hasBag = residualSizePct != null && residualSizePct > 0.02;

    const manyTrades = (buys.length + sells.length) >= 8;
    const longHold = (realized.medianHoldMins ?? 0) > 60;
    const badOutcome = (realized.medianGainPct ?? 0) < -40;
    const goodOutcome = (maxGainPct ?? 0) > 200;

    const isStoryCoin = !!(manyTrades && longHold && badOutcome);
    const isMegaWinner = !!goodOutcome;
  ...
  // Add wallet-level realized winners/losers and bagging metrics
  const allWins = allLegs.filter(x => x > 0);
  const allLosses = allLegs.filter(x => x < 0);
  const realizedAvgGainPctWinners = allWins.length ? mean(allWins) : null;
  const realizedAvgLossPctLosers = allLosses.length ? mean(allLosses) : null;

  const residuals = picked
    .map(r => r.residualSizePct)
    .filter(v => typeof v === 'number' && isFinite(v));
  const bagConcentration = residuals.length ? mean(residuals) : null;
  const openBagCount = picked.filter(r => r.hasBag).length;
  const storyBagCount = picked.filter(r => r.isStoryCoin).length;
  ...
  const overall = {
    ...
    realizedWinRate: legsWinRate,
    realizedAvgGainPctWinners,
    realizedAvgLossPctLosers,
    bagConcentration,
    openBagCount,
    storyBagCount,
